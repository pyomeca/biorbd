...
internal_forces::WrappingObject::WrappingObject()
    : utils::Vector3d(), m_RT(std::make_shared<utils::RotoTrans>()) {
...
internal_forces::WrappingObject::WrappingObject(
    ...
    : utils::Vector3d(x, y, z), m_RT(std::make_shared<utils::RotoTrans>()) {
  *m_typeOfNode = utils::NODE_TYPE::WRAPPING_OBJECT;
}
...
internal_forces::WrappingObject::WrappingObject(const utils::Vector3d &other)
