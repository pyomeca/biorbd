...
void internal_forces::WrappingHalfCylinder::DeepCopy(
    const internal_forces::WrappingHalfCylinder& other) {
...
void internal_forces::WrappingHalfCylinder::wrapPoints(
    ...
    utils::Vector3d& p2,
    utils::Scalar* length) {
  // This function takes the position of the wrapping and finds the location
  // where muscle 1 and 2 leave the wrapping object
  ...
  // if there is a straight line in between two points not passing throught the
  // cylinder
  ...
  // If asked, compute the distance distance traveled on the periphery of the
  // cylinder Apply pythagorus to the cercle arc
...
void internal_forces::WrappingHalfCylinder::wrapPoints(
    ...
    utils::Vector3d& p2,
    utils::Scalar* length) {
  // This function takes a model and a position of the model and returns the
  // location where muscle 1 et 2 leave the wrapping object
...
const utils::Scalar& internal_forces::WrappingHalfCylinder::length() const {
  return *m_length;
}
...
void internal_forces::WrappingHalfCylinder::findTangentToCircle(
  ...
  utils::Scalar p_dot =
      static_cast<Eigen::Vector2d>(p.block(0, 0, 2, 1))
          .dot(static_cast<Eigen::Vector2d>(p.block(0, 0, 2, 1)));
#else
  utils::Scalar p_dot = p.block(0, 0, 2, 1).dot(p.block(0, 0, 2, 1));
#endif

  const utils::Vector2d& Q0(radius() * radius() / p_dot * p.block(0, 0, 2, 1));
  utils::Matrix2d tp(utils::Matrix2d::Zero());
  tp(0, 1) = -1;
  tp(1, 0) = 1;

  const utils::Vector2d& T(
      radius() / p_dot * std::sqrt(p_dot - radius() * radius()) * tp *
      p.block(0, 0, 2, 1));
...
void internal_forces::WrappingHalfCylinder::selectTangents(
  ...
  p_tan = IF_ELSE_NAMESPACE::if_else(
      IF_ELSE_NAMESPACE::ge((*p1.m_p2)(0), (*p1.m_p1)(0)), *p1.m_p2, *p1.m_p1);
#else
...
bool internal_forces::WrappingHalfCylinder::findVerticalNode(
  ...
  if (!checkIfWraps(
    ...
    for (size_t i = 0; i < 3; ++i) {
      (*pointsToWrap.m_p1)(i) =
          static_cast<utils::Scalar>(static_cast<double>(NAN));
      (*pointsToWrap.m_p2)(i) =
          static_cast<utils::Scalar>(static_cast<double>(NAN));
    }
    return false;
  } else {
    // Make sure the z component won't cause any problem in the rotation
    // computation
  ...
  // Strategy : Find the matrix of the passage between the aligned points in x
  // and the cylinder. Find the location where the points cross the cylinder
  ...
  (*pointsToWrap.m_p1)(2) =
      utils::Scalar(wrapA(0) - globB(0)) / utils::Scalar(globA(0) - globB(0)) *
          ((*pointsInGlobal.m_p1)(2) - (*pointsInGlobal.m_p2)(2)) +
      (*pointsInGlobal.m_p2)(2);
  (*pointsToWrap.m_p2)(2) =
      utils::Scalar(wrapB(0) - globB(0)) / utils::Scalar(globA(0) - globB(0)) *
          ((*pointsInGlobal.m_p1)(2) - (*pointsInGlobal.m_p2)(2)) +
      (*pointsInGlobal.m_p2)(2);
...
bool internal_forces::WrappingHalfCylinder::checkIfWraps(
  ...
  // If we are on top of the wrap, it is impossible to determine because the
  // wrap is not a cylinder but a half-cylinder
  if (((*pointsInGlobal.m_p1)(1) > 0 && (*pointsInGlobal.m_p2)(1) > 0) ||
      ((*pointsInGlobal.m_p1)(1) < 0 && (*pointsInGlobal.m_p2)(1) < 0)) {
    return false;
  }

  // If we have a height* smaller than the radius, there is a numerical
  // aberation
  ...
  // If the straight line between the two points go through the cylinder,there
  // is a wrap
...
utils::Scalar internal_forces::WrappingHalfCylinder::computeLength(
  ...
  utils::Scalar arc =
      std::acos(
          ((*p.m_p1)(0) * (*p.m_p2)(0) + (*p.m_p1)(1) * (*p.m_p2)(1)) /
          std::sqrt(
              ((*p.m_p1)(0) * (*p.m_p1)(0) + (*p.m_p1)(1) * (*p.m_p1)(1)) *
              ((*p.m_p2)(0) * (*p.m_p2)(0) + (*p.m_p2)(1) * (*p.m_p2)(1)))) *
      radius();

  return std::sqrt(
      arc * arc +
      ((*p.m_p1)(2) - (*p.m_p2)(2)) * ((*p.m_p1)(2) - (*p.m_p2)(2)));
}
