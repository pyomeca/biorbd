...
internal_forces::actuator::ActuatorGauss3p::ActuatorGauss3p()
...
internal_forces::actuator::ActuatorGauss3p::ActuatorGauss3p(
    const internal_forces::actuator::ActuatorGauss3p& other)
...
internal_forces::actuator::ActuatorGauss3p
internal_forces::actuator::ActuatorGauss3p::DeepCopy() const {
  internal_forces::actuator::ActuatorGauss3p copy;
  copy.DeepCopy(*this);
  return copy;
}

void internal_forces::actuator::ActuatorGauss3p::DeepCopy(
    const internal_forces::actuator::ActuatorGauss3p& other) {
...
utils::Scalar internal_forces::actuator::ActuatorGauss3p::torqueMax() {
  utils::Error::raise(
...
utils::Scalar internal_forces::actuator::ActuatorGauss3p::torqueMax(
  ...
  Tw = IF_ELSE_NAMESPACE::if_else(
      IF_ELSE_NAMESPACE::ge(speed, 0),
      C / (*m_wc + speed) - Tc,
      E / (we - speed) + *m_Tmax);
#else
  if (speed >= 0) {
  ...
  utils::Scalar A =
      *m_amin +
      (*m_amax - *m_amin) / (1 + utils::Scalar(exp(-(speed - *m_w1) / *m_wr)));

  // Torque angle
  utils::Scalar Ta =
      exp(-(*m_qopt - pos) * (*m_qopt - pos) / (2 * *m_r * *m_r));

  // Calculation of the max torque
  return Tw * A * Ta;
}

void internal_forces::actuator::ActuatorGauss3p::setType() {
  *m_type = internal_forces::actuator::TYPE::GAUSS3P;
}
