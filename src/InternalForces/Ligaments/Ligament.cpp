...
internal_forces::ligaments::Ligament::Ligament()
    : internal_forces::Compound(),
      ...
      m_type(
          std::make_shared<internal_forces::ligaments::LIGAMENT_TYPE>(
              internal_forces::ligaments::LIGAMENT_TYPE::NO_LIGAMENT_TYPE)),
      m_characteristics(
          std::make_shared<
              internal_forces::ligaments::LigamentCharacteristics>()),
    ...
    : internal_forces::Compound(name),
      ...
      m_type(
          std::make_shared<internal_forces::ligaments::LIGAMENT_TYPE>(
              internal_forces::ligaments::LIGAMENT_TYPE::NO_LIGAMENT_TYPE)),
      m_characteristics(
          std::make_shared<internal_forces::ligaments::LigamentCharacteristics>(
              characteristics)),
      m_Fl(std::make_shared<utils::Scalar>()),
      m_damping(std::make_shared<utils::Scalar>()) {
  setType();
...
void internal_forces::ligaments::Ligament::DeepCopy(
    const internal_forces::ligaments::Ligament &other) {
...
internal_forces::ligaments::Ligament::type() const {
  return *m_type;
}
...
void internal_forces::ligaments::Ligament::updateOrientations(
  ...
  m_position->updateKinematics(
      ligamentPointsInGlobal, jacoPointsInGlobal, nullptr);
}
void internal_forces::ligaments::Ligament::updateOrientations(
    std::vector<utils::Vector3d> &ligamentPointsInGlobal,
    utils::Matrix &jacoPointsInGlobal,
  ...
  // Update de la position des insertions et origines
  m_position->updateKinematics(
      ligamentPointsInGlobal, jacoPointsInGlobal, &Qdot);
}
...
const internal_forces::Geometry &
internal_forces::ligaments::Ligament::position() const {
  return *m_position;
}
...
internal_forces::ligaments::Ligament::ligamentsPointsInGlobal(
    rigidbody::Joints &updatedModel,
    const rigidbody::GeneralizedCoordinates &Q,
    bool updateLigamentKinematics) {
  if (updateLigamentKinematics)
    m_position->updateKinematics(updatedModel, *m_pathChanger, &Q);
