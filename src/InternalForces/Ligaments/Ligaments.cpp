...
internal_forces::ligaments::Ligaments::Ligaments()
    : m_ligaments(
          std::make_shared<std::vector<
              std::shared_ptr<internal_forces::ligaments::Ligament>>>()) {}
...
internal_forces::ligaments::Ligaments
internal_forces::ligaments::Ligaments::DeepCopy() const {
...
void internal_forces::ligaments::Ligaments::DeepCopy(
  ...
  for (size_t i = 0; i < other.m_ligaments->size(); ++i) {
    if ((*other.m_ligaments)[i]->type() ==
        internal_forces::ligaments::LIGAMENT_TYPE::LIGAMENT_CONSTANT) {
      (*m_ligaments)[i] =
          std::make_shared<internal_forces::ligaments::LigamentConstant>(
              (*other.m_ligaments)[i]);
      return;
    } else if (
        (*other.m_ligaments)[i]->type() ==
        internal_forces::ligaments::LIGAMENT_TYPE::LIGAMENT_SPRING_LINEAR) {
      (*m_ligaments)[i] =
          std::make_shared<internal_forces::ligaments::LigamentSpringLinear>(
              (*other.m_ligaments)[i]);
      return;
    } else if (
        (*other.m_ligaments)[i]->type() ==
        internal_forces::ligaments::LIGAMENT_TYPE::
            LIGAMENT_SPRING_SECOND_ORDER) {
      (*m_ligaments)[i] = std::make_shared<
...
internal_forces::ligaments::Ligaments::ligament(size_t idx) {
  utils::Error::check(
      idx < nbLigaments(), "Idx asked is higher than number of ligaments");
  return *(*m_ligaments)[idx];
}

const internal_forces::ligaments::Ligament&
internal_forces::ligaments::Ligaments::ligament(size_t idx) const {
  utils::Error::check(
      idx < nbLigaments(), "Idx asked is higher than number of ligaments");
  return *(*m_ligaments)[idx];
}
...
void internal_forces::ligaments::Ligaments::addLigament(
  ...
  if (ligamentTp.type() ==
    ...
    m_ligaments->push_back(
      ...
      ligamentTp.type() ==
      internal_forces::ligaments::LIGAMENT_TYPE::LIGAMENT_SPRING_SECOND_ORDER) {
    m_ligaments->push_back(
        std::make_shared<internal_forces::ligaments::LigamentSpringSecondOrder>(
            ligamentTp));
  } else {
...
internal_forces::ligaments::Ligaments::ligamentNames() const {
...
internal_forces::ligaments::Ligaments::ligamentsJointTorque(
  ...
  if (updateKin < 2) {
    utils::Error::raise(
        utils::String(
            "When using Casadi, this method must set updateKin to true. ") +
        "Alternatively, you can call ligamentsJointTorque with the pre-updated "
        "model.");
  }
  rigidbody::Joints
#else
  rigidbody::Joints&
      ...
      updatedModel =
          dynamic_cast<rigidbody::Joints&>(*this).UpdateKinematicsCustom(
              updateKin >= 2 ? &Q : nullptr, updateKin >= 2 ? &Qdot : nullptr);
...
internal_forces::ligaments::Ligaments::ligamentsJointTorque(
    ...
    const rigidbody::GeneralizedVelocity& Qdot,
    bool updateLigamentKinematics) {
  return ligamentsJointTorque(
      ligamentForces(updatedModel, Q, Qdot, updateLigamentKinematics));
}
...
utils::Vector internal_forces::ligaments::Ligaments::ligamentForces(
  ...
  for (size_t j = 0; j < nbLigaments(); ++j) {
    forces(static_cast<unsigned int>(j)) =
        ((*m_ligaments)[j]->force(updatedModel, Q, updateLigamentKinematics));
  }
...
utils::Vector internal_forces::ligaments::Ligaments::ligamentForces(
  ...
  if (updateKin < 2) {
    utils::Error::raise(
        utils::String(
            "When using Casadi, this method must set updateKin to true. ") +
        "Alternatively, you can call ligamentForces with the pre-updated "
        "model.");
  }
  rigidbody::Joints
#else
...
utils::Vector internal_forces::ligaments::Ligaments::ligamentForces(
  ...
  utils::Vector forces(nbLigaments());
  for (size_t j = 0; j < nbLigaments(); ++j) {
    forces(static_cast<unsigned int>(j)) = ((*m_ligaments)[j]->force(
        updatedModel, Q, Qdot, updateLigamentKinematics));
  }
...
utils::Matrix internal_forces::ligaments::Ligaments::ligamentsLengthJacobian()
  ...
  for (size_t j = 0; j < nbLigaments(); ++j) {
    tp.block(
        ...
        0,
        1,
        static_cast<unsigned int>(model.nbDof())) =
...
utils::Matrix internal_forces::ligaments::Ligaments::ligamentsLengthJacobian(
  ...
  if (updateKin < 2) {
    utils::Error::raise(
        utils::String(
            "When using Casadi, this method must set updateKin to true. ") +
        "Alternatively, you can call ligamentsLengthJacobian with the "
        "pre-updated model.");
  }
  rigidbody::Joints
#else
...
int internal_forces::ligaments::Ligaments::ligamentID(
    const utils::String& nameToFind) {
  for (size_t i = 0; i < m_ligaments->size(); ++i) {
...
void internal_forces::ligaments::Ligaments::updateLigaments(
    ...
    const rigidbody::GeneralizedVelocity& Qdot) {
  for (size_t j = 0; j < nbLigaments(); ++j) {
    ligament(j).updateOrientations(
        ligamentPointsInGlobal[j], jacoPointsInGlobal[j], Qdot);
  }
}
...
void internal_forces::ligaments::Ligaments::updateLigaments(
  ...
  for (size_t j = 0; j < nbLigaments(); ++j) {
    ligament(j).updateOrientations(
        ligamentPointsInGlobal[j], jacoPointsInGlobal[j]);
  }
}