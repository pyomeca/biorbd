...
internal_forces::muscles::State internal_forces::muscles::State::DeepCopy()
    const {
...
void internal_forces::muscles::State::DeepCopy(
    const internal_forces::muscles::State& other) {
  *m_stateType = *other.m_stateType;
  *m_excitation = *other.m_excitation;
  ...
  *m_activation = val;
  ...
  if (val < 0) {
    if (!turnOffWarnings) {
      utils::Error::warning(
          0,
          "Activation is " + utils::String::to_string(val) +
    ...
    *m_activation = 0;
    ...
    if (!turnOffWarnings) {
      utils::Error::warning(
          0,
          "Activation " + utils::String::to_string(val) +
...
internal_forces::muscles::STATE_TYPE internal_forces::muscles::State::type()
    const {
  return *m_stateType;
}
