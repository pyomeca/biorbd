...
internal_forces::muscles::Muscles::Muscles(
    const internal_forces::muscles::Muscles& other)
    : m_mus(other.m_mus) {}

internal_forces::muscles::Muscles::~Muscles() {}

internal_forces::muscles::Muscles internal_forces::muscles::Muscles::DeepCopy()
    const {
...
void internal_forces::muscles::Muscles::DeepCopy(
    const internal_forces::muscles::Muscles& other) {
...
void internal_forces::muscles::Muscles::addMuscleGroup(
  ...
  if (m_mus->size() > 0) {
    utils::Error::check(
        getMuscleGroupId(name) == -1, "Muscle group already defined");
  }

  m_mus->push_back(
      internal_forces::muscles::MuscleGroup(name, originName, insertionName));
}

int internal_forces::muscles::Muscles::getMuscleGroupId(
...
std::vector<utils::String> internal_forces::muscles::Muscles::muscleNames()
    const {
...
internal_forces::muscles::Muscles::muscleGroup(size_t idx) {
  utils::Error::check(
      idx < nbMuscleGroups(),
...
const internal_forces::muscles::MuscleGroup&
internal_forces::muscles::Muscles::muscleGroup(size_t idx) const {
  utils::Error::check(
      idx < nbMuscleGroups(),
      "Idx asked is higher than number of muscle groups");
  return (*m_mus)[idx];
...
const internal_forces::muscles::MuscleGroup&
internal_forces::muscles::Muscles::muscleGroup(
    const utils::String& name) const {
...
internal_forces::muscles::Muscles::muscularJointTorque(const utils::Vector& F) {
...
internal_forces::muscles::Muscles::muscularJointTorque(
    ...
    const rigidbody::GeneralizedVelocity& Qdot,
    bool updateMuscleParameters) {
  return muscularJointTorque(
      muscleForces(updatedModel, emg, Q, Qdot, updateMuscleParameters));
}
...
utils::Vector internal_forces::muscles::Muscles::muscleForces(
  ...
  for (size_t i = 0; i < m_mus->size(); ++i) {  // muscle group
    for (size_t j = 0; j < (*m_mus)[i].nbMuscles(); ++j) {
      forces(static_cast<unsigned int>(cmpMus), 0) =
          ((*m_mus)[i].muscle(j).force(*emg[cmpMus]));
      ++cmpMus;
    }
  }
...
utils::Matrix internal_forces::muscles::Muscles::musclesLengthJacobian() {
  ...
  for (size_t i = 0; i < nbMuscleGroups(); ++i)
    for (size_t j = 0; j < ((*m_mus)[i]).nbMuscles(); ++j) {
      tp.block(
          static_cast<unsigned int>(cmpMus++),
          0,
          1,
...
utils::Matrix internal_forces::muscles::Muscles::musclesLengthJacobian(
  ...
  if (updateKin < 2) {
    utils::Error::raise(
        ...
        "Alternatively, you can call musclesLengthJacobian with the "
        "pre-updated model.");
  }
  rigidbody::Joints
#else
...
void internal_forces::muscles::Muscles::updateMuscles(
  ...
  if (!updateKin) {
    utils::Error::raise(
        utils::String(
            "When using Casadi, this method must set updateKin to true. ") +
        "Alternatively, you can call updateMuscles with the pre-updated "
        "model.");
  }
  rigidbody::Joints
#else
  ...
  for (auto group : *m_mus)  // muscle  group
    for (size_t j = 0; j < group.nbMuscles(); ++j) {
      group.muscle(j).updateOrientations(
          musclePointsInGlobal[cmpMuscle], jacoPointsInGlobal[cmpMuscle], Qdot);
      ++cmpMuscle;
    }
}
...
void internal_forces::muscles::Muscles::updateMuscles(
  ...
  for (auto group : *m_mus)  // muscle group
    for (size_t j = 0; j < group.nbMuscles(); ++j) {
      group.muscle(j).updateOrientations(
          musclePointsInGlobal[cmpMuscle], jacoPointsInGlobal[cmpMuscle]);
      ++cmpMuscle;
    }
}
