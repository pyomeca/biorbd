...
internal_forces::muscles::MuscleGeometry
internal_forces::muscles::MuscleGeometry::DeepCopy() const {
  internal_forces::muscles::MuscleGeometry copy;
  copy.DeepCopy(*this);
  return copy;
}

void internal_forces::muscles::MuscleGeometry::DeepCopy(
    const internal_forces::muscles::MuscleGeometry& other) {
...
const utils::Scalar&
internal_forces::muscles::MuscleGeometry::musculoTendonLength() const {
  utils::Error::check(
      *m_isGeometryComputed,
      "Geometry must be computed at least before calling length()");
  ...
  *m_muscleTendonLength = 0;
  ...
  if (pathModifiers != nullptr && pathModifiers->nbWraps() != 0) {
    ...
    utils::Vector3d pi_wrap(
        0, 0, 0);  // point on the wrapping related to insertion
    utils::Vector3d po_wrap(
        0, 0, 0);  // point on the wrapping related to origin
    ...
    *m_muscleTendonLength =
    ...
    for (size_t i = 0; i < m_pointsInGlobal->size() - 1; ++i) {
      *m_muscleTendonLength +=
          ((*m_pointsInGlobal)[i + 1] - (*m_pointsInGlobal)[i]).norm();
    }
  }
  *m_muscleLength =
      (*m_muscleTendonLength - characteristics->tendonSlackLength()) /
