...
internal_forces::muscles::Muscle::Muscle()
    : internal_forces::Compound(),
      ...
      m_type(
          std::make_shared<internal_forces::muscles::MUSCLE_TYPE>(
              internal_forces::muscles::MUSCLE_TYPE::NO_MUSCLE_TYPE)),
      m_characteristics(
          std::make_shared<internal_forces::muscles::Characteristics>()),
...
internal_forces::muscles::Muscle::Muscle(
    ...
    : internal_forces::Compound(name),
      m_position(
          std::make_shared<internal_forces::muscles::MuscleGeometry>(position)),
      m_type(
          std::make_shared<internal_forces::muscles::MUSCLE_TYPE>(
              internal_forces::muscles::MUSCLE_TYPE::NO_MUSCLE_TYPE)),
      m_characteristics(
    ...
    : internal_forces::Compound(name, pathModifiers),
      ...
      m_type(
          std::make_shared<internal_forces::muscles::MUSCLE_TYPE>(
              internal_forces::muscles::MUSCLE_TYPE::NO_MUSCLE_TYPE)),
      m_characteristics(
          std::make_shared<internal_forces::muscles::Characteristics>(c)),
      m_state(std::make_shared<internal_forces::muscles::State>()) {
  setState(emg);

  utils::Error::check(
      pathModifiers.nbWraps() <= 1,
...
void internal_forces::muscles::Muscle::DeepCopy(
    const internal_forces::muscles::Muscle &other) {
...
internal_forces::muscles::MUSCLE_TYPE internal_forces::muscles::Muscle::type()
    const {
  return *m_type;
}
...
void internal_forces::muscles::Muscle::updateOrientations(
  ...
  m_position->updateKinematics(
      updatedModel, *m_characteristics, *m_pathChanger, &Q, nullptr);
}
void internal_forces::muscles::Muscle::updateOrientations(
    rigidbody::Joints &updatedModel,
    const rigidbody::GeneralizedCoordinates &Q,
  ...
  // Update de la position des insertions et origines
  m_position->updateKinematics(
      updatedModel, *m_characteristics, *m_pathChanger, &Q, &Qdot);
}
void internal_forces::muscles::Muscle::updateOrientations(
...
const internal_forces::muscles::MuscleGeometry &
internal_forces::muscles::Muscle::position() const {
  return *m_position;
}
...
const utils::Scalar &internal_forces::muscles::Muscle::activationDot(
  ...
  std::shared_ptr<internal_forces::muscles::StateDynamics> state_copy =
      std::dynamic_pointer_cast<internal_forces::muscles::StateDynamics>(
          m_state);
  utils::Error::check(
      state_copy != nullptr,
...
void internal_forces::muscles::Muscle::computeForce(
    const internal_forces::muscles::State &emg) {
...
internal_forces::muscles::Characteristics &
...
void internal_forces::muscles::Muscle::setState(
  ...
  if (emg.type() == internal_forces::muscles::STATE_TYPE::BUCHANAN) {
    ...
    utils::Error::raise(
        utils::String(internal_forces::muscles::STATE_TYPE_toStr(emg.type())) +
        " is not a valid type for setState");
  }
  *m_state = emg;
}
const internal_forces::muscles::State &internal_forces::muscles::Muscle::state()
    const {
  return *m_state;
}
