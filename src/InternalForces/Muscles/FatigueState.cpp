...
internal_forces::muscles::FatigueState::FatigueState(
    ...
    : m_activeFibers(std::make_shared<utils::Scalar>(active)),
      ...
      m_type(std::make_shared<internal_forces::muscles::STATE_FATIGUE_TYPE>()) {
  setType();
}

internal_forces::muscles::FatigueState::FatigueState(
    const internal_forces::muscles::FatigueState& other)
...
internal_forces::muscles::FatigueState::DeepCopy() const {
  internal_forces::muscles::FatigueState copy;
  copy.DeepCopy(*this);
  return copy;
}

void internal_forces::muscles::FatigueState::DeepCopy(
    const internal_forces::muscles::FatigueState& other) {
  *m_activeFibers = *other.m_activeFibers;
  *m_fatiguedFibers = *other.m_fatiguedFibers;
  *m_restingFibers = *other.m_restingFibers;
...
void internal_forces::muscles::FatigueState::setState(
    utils::Scalar active,
  ...
  //
  // In order to get the quantity of active fibers to 0 or 1, it has to come
  // from the input command. The input command manage fiber recruitment from
  // resting to active. Hence any exceeding because of integration can be
  // corrected by putting back exceeding active fibers quantity into resting
  // fibers quantity.
  //
  if (active < 0) {
    if (!turnOffWarnings) {
  ...
  //
  // In order to get the quantity of resting fibers to 0, it has to come from
  // the input command. The input command manage fiber recruitment from resting
  // to active. Hence any exceeding because of integration can be corrected by
  // putting back exceeding resting fibers quantity into active fibers quantity.
  //
  if (resting < 0) {
    if (!turnOffWarnings) {
      utils::Error::warning(
    ...
    utils::Error::raise("Resting Fibers Quantity can't be higher than 1");
  }
...
const utils::Scalar& internal_forces::muscles::FatigueState::activeFibers()
    const {
  return *m_activeFibers;
}

const utils::Scalar& internal_forces::muscles::FatigueState::fatiguedFibers()
    const {
  return *m_fatiguedFibers;
}

const utils::Scalar& internal_forces::muscles::FatigueState::restingFibers()
    const {
  return *m_restingFibers;
}

internal_forces::muscles::STATE_FATIGUE_TYPE
internal_forces::muscles::FatigueState::getType() const {
  return *m_type;
}

void internal_forces::muscles::FatigueState::setType() {
  *m_type = internal_forces::muscles::STATE_FATIGUE_TYPE::SIMPLE_STATE_FATIGUE;
}
