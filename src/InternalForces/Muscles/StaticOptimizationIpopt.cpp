...
internal_forces::muscles::StaticOptimizationIpopt::StaticOptimizationIpopt(
    Model &model,
    ...
    : m_model(model),
      ...
      m_torqueTarget(
          std::make_shared<rigidbody::GeneralizedTorque>(torqueTarget)),
      m_torqueResidual(
          std::make_shared<utils::Vector>(utils::Vector::Zero(*m_nbTorque))),
      ...
      m_finalSolution(std::make_shared<utils::Vector>(utils::Vector(*m_nbMus))),
      m_finalResidual(std::make_shared<utils::Vector>(utils::Vector(*m_nbQ))) {
  if (*m_eps < 1e-12) {
    utils::Error::raise(
        "epsilon for partial derivates approximation is too small ! \nLimit "
        "for epsilon is 1e-12");
  }

  for (auto &s : *m_states) {
    s = std::make_shared<internal_forces::muscles::State>();
  }
...
bool internal_forces::muscles::StaticOptimizationIpopt::get_nlp_info(
    Ipopt::Index &n,
    Ipopt::Index &m,
  ...
  if (*m_nbTorqueResidual) {
    nnz_jac_g =
        (static_cast<int>(*m_nbMus) + 1) * static_cast<int>(*m_nbTorque);
  } else {
...
bool internal_forces::muscles::StaticOptimizationIpopt::get_bounds_info(
    Ipopt::Index n,
    Ipopt::Number *x_l,
    Ipopt::Number *x_u,
    Ipopt::Index m,
...
bool internal_forces::muscles::StaticOptimizationIpopt::get_starting_point(
    Ipopt::Index,
    bool init_x,
    Ipopt::Number *x,
    bool init_z,
...
bool internal_forces::muscles::StaticOptimizationIpopt::eval_f(
    Ipopt::Index n,
  ...
  // Warning, residual norm HAS to be a non-sqrt, otherwise the problem is
  // degenerated. The activation is just a speed matter.
  obj_value = m_activations->norm(*m_pNormFactor, true) +
              *m_torquePonderation * m_torqueResidual->norm(2, true);

  return true;
}

bool internal_forces::muscles::StaticOptimizationIpopt::eval_grad_f(
    Ipopt::Index n,
    const Ipopt::Number *x,
    bool new_x,
    Ipopt::Number *grad_f) {
  ...
  utils::Vector grad_activ(m_activations->normGradient(*m_pNormFactor, true));
...
bool internal_forces::muscles::StaticOptimizationIpopt::eval_g(
    Ipopt::Index n,
    const Ipopt::Number *x,
    bool new_x,
    Ipopt::Index m,
  ...
  for (unsigned int i = 0; i < static_cast<unsigned int>(m); i++) {
    g[i] = GeneralizedTorqueMusc[i] + (*m_torqueResidual)[i] -
           (*m_torqueTarget)[i];
  }

  if (*m_verbose >= 2) {
    std::cout << "GeneralizedTorque_musc = "
              << GeneralizedTorqueMusc.transpose() << std::endl;
...
bool internal_forces::muscles::StaticOptimizationIpopt::eval_jac_g(
    Ipopt::Index n,
    const Ipopt::Number *x,
    bool new_x,
    Ipopt::Index m,
  ...
  if (values == nullptr) {
      ...
      for (unsigned int i = 0; i < *m_nbMus; ++i) {
        ...
        stateEpsilon.push_back(
            std::make_shared<internal_forces::muscles::State>(
                internal_forces::muscles::State(
                    0, (*m_activations)[i] + delta * *m_eps)));
      }
      const rigidbody::GeneralizedTorque &GeneralizedTorqueCalculEpsilon(
          m_model.muscularJointTorque(stateEpsilon));
      for (unsigned int i = 0; i < static_cast<unsigned int>(m); i++) {
        values[k++] =
            (GeneralizedTorqueCalculEpsilon[i] - GeneralizedTorqueMusc[i]) /
            *m_eps;
        if (*m_verbose >= 3) {
          std::cout << std::setprecision(20) << std::endl;
          std::cout << "values[" << k - 1 << "]: " << values[k - 1]
                    << std::endl;
          std::cout << "GeneralizedTorqueCalculEpsilon[" << i
                    << "]: " << GeneralizedTorqueCalculEpsilon[i] << std::endl;
          std::cout << "GeneralizedTorqueMusc[" << i
                    << "]: " << GeneralizedTorqueMusc[i] << std::endl;
        }
      }
    }
    ...
    if (*m_verbose >= 2) {
      ...
      utils::Matrix jacobian(
          utils::Matrix::Zero(*m_nbTorque, static_cast<unsigned int>(n)));
      for (unsigned int j = 0; j < *m_nbMus; j++)
        for (unsigned int i = 0; i < static_cast<unsigned int>(m); i++) {
...
void internal_forces::muscles::StaticOptimizationIpopt::finalize_solution(
    Ipopt::SolverReturn,
    Ipopt::Index,
    const Ipopt::Number *x,
  ...
  *m_finalResidual = *m_torqueResidual;
  ...
  if (*m_verbose >= 1) {
    ...
    std::cout << "Activations = " << m_activations->transpose() << std::endl;
    std::cout << "Muscular torques = "
              << m_model.muscularJointTorque(*m_states).transpose()
              << std::endl;
    std::cout << "GeneralizedTorque target = " << m_torqueTarget->transpose()
              << std::endl;
    if (*m_nbTorqueResidual) {
      std::cout << "Residual torques= " << m_torqueResidual->transpose()
                << std::endl;
    }
    //        // Uncomment to show lagrange multipliers
    //        std::cout << "Solution of the bound multipliers, z_L and z_U" <<
    //        std::endl; for( Ipopt::Index i = 0; i < n; i++ )
    //            std::cout << "z_L[" << i << "] = " << z_L[i] << ", z_U[" << i
    //            << "] = " << z_U[i] << std::endl << std::endl;
  }
}

utils::Vector internal_forces::muscles::StaticOptimizationIpopt::finalSolution()
    const {
  return *m_finalSolution;
}

utils::Vector internal_forces::muscles::StaticOptimizationIpopt::finalResidual()
    const {
  return *m_finalResidual;
}

void internal_forces::muscles::StaticOptimizationIpopt::dispatch(
    const Ipopt::Number *x) {
  for (unsigned int i = 0; i < *m_nbMus; i++) {
    (*m_activations)[i] = x[i];
