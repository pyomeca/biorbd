...
internal_forces::muscles::StaticOptimizationIpoptLinearized::
...
void internal_forces::muscles::StaticOptimizationIpoptLinearized::
    prepareJacobian() {
  ...
  for (unsigned int i = 0; i < *m_nbMus; ++i) {
    ...
    const rigidbody::GeneralizedTorque &GeneralizedTorque(
        m_model.muscularJointTorque(state, *m_Q.get(), *m_Qdot.get()));
...
bool internal_forces::muscles::StaticOptimizationIpoptLinearized::eval_g(
  ...
  if (*m_verbose >= 2) {
    std::cout << "GeneralizedTorque_musc_approximated = " << res.transpose()
              << std::endl;
