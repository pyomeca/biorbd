...
internal_forces::Geometry::Geometry()
    : m_origin(std::make_shared<utils::Vector3d>()),
      ...
      m_originInGlobal(
          std::make_shared<utils::Vector3d>(utils::Vector3d::Zero())),
      m_insertionInGlobal(
          std::make_shared<utils::Vector3d>(utils::Vector3d::Zero())),
...
void internal_forces::Geometry::DeepCopy(
    const internal_forces::Geometry &other) {
...
void internal_forces::Geometry::setOrigin(const utils::Vector3d &position) {
...
const utils::Vector3d &internal_forces::Geometry::insertionInLocal() const {
  return *m_insertion;
}
...
const std::vector<utils::Vector3d> &internal_forces::Geometry::pointsInGlobal()
    const {
  utils::Error::check(
      *m_isGeometryComputed,
      "Geometry must be computed at least once before calling "
...
utils::Matrix internal_forces::Geometry::jacobian(size_t idxViaPoint) const {
  utils::Error::check(
      *m_isGeometryComputed,
      "Geometry must be computed before calling jacobian(i)");
  return m_jacobian->block(
      3 * static_cast<unsigned int>(idxViaPoint), 0, 3, m_jacobian->cols());
}
...
const utils::Vector3d &internal_forces::Geometry::originInGlobal(
  ...
  m_originInGlobal->block(0, 0, 3, 1) =
      model.CalcBodyToBaseCoordinates(Q, m_origin->parent(), *m_origin, false);
  return *m_originInGlobal;
}
...
const utils::Vector3d &internal_forces::Geometry::insertionInGlobal(
    ...
    const rigidbody::GeneralizedCoordinates &Q) {
  // Return the position of the marker in function of the given position
  m_insertionInGlobal->block(0, 0, 3, 1) = model.CalcBodyToBaseCoordinates(
      Q, m_insertion->parent(), *m_insertion, false);
  return *m_insertionInGlobal;
}
...
void internal_forces::Geometry::setPointsInGlobal(
  ...
  m_pointsInLocal
      ->clear();  // In this mode, we don't need the local, because the Jacobian
                  // of the points has to be given as well
...
void internal_forces::Geometry::setPointsInGlobal(
  ...
  if (pathModifiers->nbWraps() != 0) {
    ...
    m_pointsInLocal->push_back(
        utils::Vector3d(
            model.CalcBodyToBaseCoordinates(Q, w.parent(), po_wrap, false),
            "wrap_o",
            w.parent()));
    m_pointsInLocal->push_back(
        utils::Vector3d(
            model.CalcBodyToBaseCoordinates(Q, w.parent(), pi_wrap, false),
            "wrap_i",
            w.parent()));
    ...
    for (size_t i = 0; i < pathModifiers->nbObjects(); ++i) {
      const internal_forces::ViaPoint &node(
          static_cast<internal_forces::ViaPoint &>(pathModifiers->object(i)));
      m_pointsInLocal->push_back(node);
      m_pointsInGlobal->push_back(
          model.CalcBodyToBaseCoordinates(Q, node.parent(), node, false));
    }
    m_pointsInLocal->push_back(insertionInLocal());
...
const utils::Scalar &internal_forces::Geometry::length(
  ...
  *m_length = 0;
  ...
  // because we can't combine, test the first (0) will let us know all the types
  // if more than one
  if (pathModifiers != nullptr && pathModifiers->nbWraps() != 0) {
    ...
    static_cast<internal_forces::WrappingObject &>(pathModifiers->object(0))
        .wrapPoints(po_wrap, pi_wrap, &lengthWrap);
    *m_length =
...
void internal_forces::Geometry::setJacobianDimension(rigidbody::Joints &model) {
  *m_jacobian = utils::Matrix::Zero(
...
void internal_forces::Geometry::jacobian(const utils::Matrix &jaco) {
  utils::Error::check(
      jaco.rows() / 3 == static_cast<int>(m_pointsInGlobal->size()),
      "Jacobian is the wrong size");
  *m_jacobian = jaco;
}
...
void internal_forces::Geometry::jacobian(
    rigidbody::Joints &model,
    const rigidbody::GeneralizedCoordinates &Q) {
  // jacobian is a protected method, so all method that calls it is expected to
  // have updated the kinematics if required
  ...
  for (size_t i = 0; i < m_pointsInLocal->size(); ++i) {
    m_jacobian->block(3 * static_cast<unsigned int>(i), 0, 3, model.dof_count) =
        model.CalcPointJacobian(
            Q,
            (*m_pointsInLocal)[i].parent(),
            (*m_pointsInLocal)[i],
            updateKin);
  }
}
...
void internal_forces::Geometry::computeJacobianLength() {
  *m_jacobianLength = utils::Matrix::Zero(1, m_jacobian->cols());
  ...
  for (size_t i = 0; i < p.size() - 1; ++i) {
    *m_jacobianLength +=
        ((p[i + 1] - p[i]).transpose() * (jacobian(i + 1) - jacobian(i))) /
        (p[i + 1] - p[i]).norm();
  }
}
