        ...
        while(file.read(
            ...
            if (!main_tag.tolower().compare("segment")) {
                ...
                if (!type.tolower().compare("gauss3p")) {
                    ...
                    actuator = new internalforce::actuator::ActuatorGauss3p(int_direction,Tmax,T0,wmax,wc,
                            amin,wr,w1,r,qopt,dofIdx,name);
                } else if (!type.tolower().compare("constant")) {
                    utils::Error::check(isDofSet && isDirectionSet && isTmaxSet,
                                                "Make sure all parameters are defined");
                    actuator = new internalforce::actuator::ActuatorConstant(int_direction,Tmax,dofIdx,
                            name);
                } else if (!type.tolower().compare("linear")) {
                    utils::Error::check(isDofSet && isDirectionSet && isSlopeSet && isT0Set,
                                                "Make sure all parameters are defined");
                    actuator = new internalforce::actuator::ActuatorLinear(int_direction,T0,slope,dofIdx,
                            name);
                    ...
                    utils::Error::check(isDofSet && isDirectionSet && isTmaxSet && isT0Set
                                                ...
                                                && isqopt2Set,
                                                "Make sure all parameters are defined");
                    actuator = new internalforce::actuator::ActuatorGauss6p(int_direction,Tmax,T0,wmax,wc,
                    ...
                    utils::Error::check(isDofSet && isThetaSet && isLambdaSet
                                                ...
                                                "Make sure all parameters are defined");
                    actuator = new internalforce::actuator::ActuatorSigmoidGauss3p(int_direction, theta,
                ...
                // Declaration of the variables
                internalforce::muscles::MUSCLE_TYPE type(internalforce::muscles::MUSCLE_TYPE::NO_MUSCLE_TYPE);
                internalforce::muscles::STATE_TYPE stateType(
                    internalforce::muscles::STATE_TYPE::NO_STATE_TYPE);
                ...
                while(file.read(property_tag) && property_tag.tolower().compare("endmuscle")) {
                    if (!property_tag.tolower().compare("musclegroup")) {
                        ...
                        if (!tp_state.tolower().compare("buchanan")) {
                            stateType = internalforce::muscles::STATE_TYPE::BUCHANAN;
                        } else if (!tp_state.tolower().compare("degroote")) {
                            stateType = internalforce::muscles::STATE_TYPE::DE_GROOTE;
                        } else {
                        ...
                        while(file.read(subproperty_tag)
                            ...
                            if (!subproperty_tag.tolower().compare("type")) {
                                ...
                                if (!tp_fatigue_type.tolower().compare("simple")) {
                                    dynamicFatigueType = internalforce::muscles::STATE_FATIGUE_TYPE::SIMPLE_STATE_FATIGUE;
                                } else if (!tp_fatigue_type.tolower().compare("xia")) {
                                    dynamicFatigueType = internalforce::muscles::STATE_FATIGUE_TYPE::DYNAMIC_XIA;
                                } else {
                ...
                internalforce::muscles::Geometry geo(
                ...
                internalforce::muscles::State stateMax(maxExcitation, maxActivation);
                internalforce::muscles::Characteristics characteristics(optimalLength, maxForce, PCSA,
                ...
                model->muscleGroup(static_cast<unsigned int>(idxGroup)).addMuscle(name,type,geo,
                        characteristics,
                        internalforce::PathModifiers(),stateType,dynamicFatigueType);
                if (stateType == internalforce::muscles::STATE_TYPE::BUCHANAN && shapeFactor != 0) {
                    ...
                    static_cast<internalforce::muscles::StateDynamicsBuchanan&>(state).shapeFactor(
                        shapeFactor);
                }
#else // MODULE_MUSCLES
                ...
                int iMuscleGroup(-1);
                int iMuscle(-1);
                internalforce::ViaPoint position(0,0,0);
                ...
                if (!wrapType.tolower().compare("halfcylinder")) {
                    ...
                    internalforce::WrappingHalfCylinder cylinder(RT,radius,length,name,parent);
