    ...
    Model *model)
{
    ...
    try
    {
        ...
        { // Attempt read into main_tag, return false if it fails
            ...
            if (!main_tag.tolower().compare("segment"))
            {
                ...
                if (version >= 3)
                { // By default for version 3 (not in matrix)
                    RTinMatrix = false;
                }
                bool isRTset(false);
            ...
            else if (!main_tag.tolower().compare("imu") || !main_tag.tolower().compare("mimu") || !main_tag.tolower().compare("customrt"))
            {
                ...
                while (file.read(property_tag) && !(!property_tag.tolower().compare("endimu") || !property_tag.tolower().compare("endmimu") || !property_tag.tolower().compare("endcustomrt")))
                {
                    if (!property_tag.tolower().compare("parent"))
                    {
                        // Dynamically find the parent number
                    ...
                    else if (!property_tag.tolower().compare("frommarkers"))
                    {
                        if (!firstTag)
                        {
                            utils::Error::raise("The tag 'fromMarkers' should appear first in the IMU " + name);
                        }
                        else
                        {
                            fromMarkers = true;
            ...
            else if (!main_tag.tolower().compare("softcontact"))
            {
                ...
                while (file.read(property_tag) && property_tag.tolower().compare("endsoftcontact"))
                {
                    if (!property_tag.tolower().compare("parent"))
                    {
                        // Dynamically find the parent number
            ...
            else if (!main_tag.tolower().compare("actuator"))
            {
                ...
                {
                    utils::Error::check(isDofSet && isDirectionSet && isTmaxSet && isT0Set && iswmaxSet && iswcSet && isaminSet &&
                ...
                {
                    utils::Error::check(isDofSet && isThetaSet && isLambdaSet && isOffsetSet && isrSet && isqoptSet,
                                        "Make sure all parameters are defined");
            ...
            else if (!main_tag.tolower().compare("muscle"))
            {
                ...
                while (file.read(property_tag) && property_tag.tolower().compare("endmuscle"))
                {
                    ...
                    else if (!property_tag.tolower().compare("type"))
                    {
                        ...
                        else if (!tp_type.tolower().compare("hilldegroote") || !tp_type.tolower().compare("degroote"))
                        {
                        ...
                        else if (!tp_type.tolower().compare("hilldegrooteactive") || !tp_type.tolower().compare("degrooteactive"))
                        {
                        ...
                        else if (!tp_type.tolower().compare("hilldegrootefatigable") || !tp_type.tolower().compare("degrootefatigable"))
                        {
                ...
                model->muscleGroup(static_cast<size_t>(idxGroup)).addMuscle(name, type, geo, characteristics, internal_forces::PathModifiers(), stateType, dynamicFatigueType);
            ...
            else if (!main_tag.tolower().compare("ligament"))
            {
                ...
                while (file.read(property_tag) && property_tag.tolower().compare("endligament"))
                {
                    if (!property_tag.tolower().compare("type"))
                    {
                        file.read(type);
            ...
            else if (!main_tag.tolower().compare("passivetorque"))
            {
                ...
                while (file.read(property_tag) && property_tag.tolower().compare("endpassivetorque"))
                {
                    if (!property_tag.tolower().compare("type"))
                    {
                        file.read(type);
    ...
    {
        utils::String error_message("Reading of file \"" + path.filename() + "." + path.extension() +
    ...
    const utils::Path &path)
{
    ...
    for (size_t j = 0; j < nbMark; j++)
    {
        ...
        size_t noMarker;
        file.read(noMarker);
        for (size_t i = 0; i <= nbIntervals; i++)
        { // <= because there is nbIntervals+1 values
...
Reader::readViconForceFile(const utils::String &path)
{
    ...
    for (size_t j = 0; j < force.size(); ++j)
    { // nb platform
        std::vector<utils::SpatialVector> tp;
        for (size_t i = 0; i < force[j].size(); ++i)
        {                                            // timestamp
                            ...
                            int nFramesToGet)
{
    ...
    size_t nbFrames(0);
    if (nFramesToGet != -1)
    { // If it's all of them, the jumps are 1
        while (!file.eof())
        {
        ...
        utils::Error::check(nFramesToGet != 0 && nFramesToGet != 1 && static_cast<size_t>(nFramesToGet) <= nbFrames,
    ...
    const utils::Path &path)
{
    ...
    file.readSpecificTag("version", tp);
    size_t version(static_cast<size_t>(atoi(tp.c_str())));
    utils::Error::check(version == 1 || version == 2, "Version not implemented yet");
