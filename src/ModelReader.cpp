...
void biorbd::Reader::readModelFile(
        ...
        biorbd::Model *model) {  // Ouverture du fichier
    if (!is_readable(path)) {
    ...
    biorbd::utils::Error::check((version == 1 || version == 2
                                 || version == 3 || version == 4),
                                "Version " + main_tag + " is not implemented"
                                                        " yet");
    ...
    try {
        while (file.read(main_tag)) {
            // Attempt read into main_tag, return false if it fails
            ...
            if (!main_tag.tolower().compare("segment")) {
                ...
                if (version == 3) {
                    // Par défaut pour la version 3 (pas en matrice)
                    RTinMatrix = false;
                }
                bool isRTset(false);
                double mass = 0.00000001;
                Eigen::Matrix3d inertia(Eigen::Matrix3d::Identity(3, 3));
                RigidBodyDynamics::Math::Matrix3d RT_R(
                ...
                while (file.read(property_tag)
                    ...
                    if (!property_tag.tolower().compare("parent")) {
                        ...
                        if (!filePath.extension().compare("bioMesh")) {
                            boneMesh = readBoneMeshFileBiorbdBones(
                                        path.folder()
                                        + filePath.relativePath());
                        }
                        else if (!filePath.extension().compare("ply")) {
                            boneMesh = readBoneMeshFilePly(
                                        path.folder()
                                        + filePath.relativePath());
                        }
                        else {
                ...
                biorbd::rigidbody::BoneCaracteristics caract(
                            mass, com, inertia, boneMesh);
                ...
                while (file.read(var)
                       && var.tolower().compare("endvariables")) {
                    if (!var(0).compare("$")) {
                        double value;
                        file.read(value);
                        biorbd::utils::Error::check(
                ...
                while (file.read(property_tag)
                    ...
                    if (!property_tag.tolower().compare("parent")) {
                        ...
                        // Si parent_int est encore égal à zéro
                        // c'est qu'aucun nom a concordé
                        biorbd::utils::Error::check(
                                    model->IsBodyId(parent_int),
                                    "Wrong name in a segment");
                    }
                ...
                model->addMarker(pos, name, parent_str, technical,
                                 anatomical, axesToRemove,
                                 static_cast<int>(parent_int));
            }
            else if (!main_tag.tolower().compare("mimu") && version >= 4) {
                biorbd::utils::Error::raise(
                            "MIMU is no more the right tag, change it to IMU!");
            }
                ...
                if (version == 1) {
                    ...
                    biorbd::utils::Error::check
                            (axis.compare(""), "Axis must be provided");
                ...
                while (file.read(property_tag)
                    ...
                    if (!property_tag.tolower().compare("predecessor")) {
                        ...
                        id_predecessor =
                                model->GetBodyId(predecessor_str.c_str());
                        // Si parent_int est encore égal à zéro
                        // c'est qu'aucun nom a concordé
                        biorbd::utils::Error::check(
                                    model->IsBodyId(id_predecessor),
                                    "Wrong name in a segment");
                    }
                        ...
                        for (unsigned int i = 0; i < 3; ++i)
                            file.read(trans(i));
                        X_predecessor =
                                biorbd::utils::RotoTrans(rot, trans, seq);
                    }
                    ...
                    else if (!property_tag.tolower()
                             .compare("stabilizationparameter")) {
                ...
                model->AddLoopConstraint(id_predecessor, id_successor,
                                         X_predecessor, X_successor,
                                         axis, name, enableStabilization,
                                         stabilizationParam);
            }
            else if (!main_tag.tolower().compare("actuator")) {
    #ifdef MODULE_ACTUATORS
                hasActuators = true;
                // Le nom de l'actuator doit correspondre au numéro
                // du segment sur lequel il s'attache
                biorbd::utils::String name;
                file.read(name);
                ...
                while (file.read(property_tag)
                    ...
                    if (!property_tag.tolower().compare("type")) {
                        ...
                        biorbd::utils::Error::check(
                                    ...
                                    || !str_direction.tolower()
                                    .compare("negative"),
                                    "Direction should be \"positive\" or "
                                    "\"negative\"");
                ...
                biorbd::utils::Error::check(
                            isTypeSet != 0, "Actuator type must be defined");
                biorbd::actuator::Actuator* actuator;

                if (!type.tolower().compare("gauss3p")) {
                    biorbd::utils::Error::check(
                                isDofSet && isDirectionSet && isTmaxSet
                                && isT0Set && iswmaxSet && iswcSet && isaminSet
                                && iswrSet && isw1Set && isrSet && isqoptSet,
                                "Make sure all parameters are defined");
                    actuator = new biorbd::actuator::ActuatorGauss3p(
                                int_direction, Tmax, T0, wmax, wc, amin, wr,
                                w1, r, qopt, dofIdx, name);
                }
                else if (!type.tolower().compare("constant")) {
                    biorbd::utils::Error::check(
                                isDofSet && isDirectionSet && isTmaxSet,
                                "Make sure all parameters are defined");
                    actuator = new biorbd::actuator::ActuatorConstant(
                                int_direction, Tmax, dofIdx, name);
                }
                else if (!type.tolower().compare("linear")) {
                    biorbd::utils::Error::check(
                                isDofSet && isDirectionSet
                                && isPenteSet && isT0Set,
                                "Make sure all parameters are defined");
                    actuator = new biorbd::actuator::ActuatorLinear(
                                int_direction, T0, pente, dofIdx, name);
                }
                    ...
                    biorbd::utils::Error::check(
                                ...
                                && isT0Set && iswmaxSet && iswcSet && isaminSet
                                && iswrSet && isw1Set && isrSet && isqoptSet
                                && isFacteur6pSet && isr2Set && isqopt2Set,
                                        "Make sure all parameters are defined");
                    actuator = new biorbd::actuator::ActuatorGauss6p(
                                int_direction, Tmax, T0, wmax, wc, amin, wr, w1,
                                r, qopt, facteur6p, r2, qopt2, dofIdx, name);
                }
                else {
                    biorbd::utils::Error::raise(
                                "Actuator do not correspond "
                                "to an implemented one");
                    // Next line is for compilation on Windows
                    actuator = new biorbd::actuator::ActuatorConstant(
                                int_direction, Tmax, dofIdx, name);
                }
            ...
            biorbd::utils::Error::raise(
                        "Biorbd was build without the module Actuators "
                        "but the model defines ones");
    #endif  // MODULE_ACTUATORS
            }
                ...
                // Lecture du fichier
                while (file.read(property_tag) && property_tag.tolower()
                      .compare("endmusclegroup")) {
                    if (!property_tag.tolower().compare("originparent")) {
                        // Trouver dynamiquement le numéro du parent
                        file.read(origin_parent_str);
                        unsigned int idx = model->GetBodyId(
                                    origin_parent_str.c_str());
                        // Si parent_int est encore égal à zéro
                        // c'est qu'aucun nom a concordé
                        biorbd::utils::Error::check(
                                    model->IsBodyId(idx),
                             ...
                             .compare("insertionparent")) {
                        // Trouver dynamiquement le numéro du parent
                        file.read(insert_parent_str);
                        unsigned int idx = model->GetBodyId(
                                    insert_parent_str.c_str());
                        // Si parent_int est encore égal à zéro
                        // c'est qu'aucun nom a concordé
                        biorbd::utils::Error::check(
                                    model->IsBodyId(idx),
                ...
                model->addMuscleGroup(name, origin_parent_str,
                                      insert_parent_str);
    #else  // MODULE_MUSCLES
            biorbd::utils::Error::raise(
                        "Biorbd was build without the module Muscles "
                        "but the model defines a muscle group");
    #endif  // MODULE_MUSCLES
            }
                ...
                biorbd::muscles::STATE_FATIGUE_TYPE dynamicFatigueType(
                            biorbd::muscles::STATE_FATIGUE_TYPE
                ...
                while (file.read(property_tag) && property_tag.tolower()
                    ...
                    if (!property_tag.tolower().compare("musclegroup")) {
                        ...
                        else if (!tp_type.tolower()
                                 .compare("hillthelenfatigable")
                                 || !tp_type.tolower()
                                 .compare("thelenfatigable")) {
                            type = biorbd::muscles::MUSCLE_TYPE
                        ...
                        if (!tp_state.tolower().compare("buchanan")) {
                            ...
                            biorbd::utils::Error::raise(
                                        property_tag +
                                        " is not a valid muscle state type");
                        }
                    }
                    else if (!property_tag.tolower()
                             .compare("originposition")) {
                        for (unsigned int i = 0; i < 3; ++i) {
                            file.read(origin_pos(i), variable);
                        }
                    }
                    else if (!property_tag.tolower()
                             .compare("insertionposition")) {
                        for (unsigned int i = 0; i < 3; ++i) {
                            file.read(insert_pos(i), variable);
                        }
                    }
                    else if (!property_tag.tolower()
                             .compare("optimallength")) {
                        file.read(optimalLength, variable);
                    }
                    else if (!property_tag.tolower()
                             .compare("tendonslacklength")) {
                        file.read(tendonSlackLength, variable);
                    }
                    else if (!property_tag.tolower()
                             .compare("pennationangle")) {
                        file.read(pennAngle, variable);
                    }
                    else if (!property_tag.tolower()
                             .compare("maximalforce")) {
                        file.read(maxForce, variable);
                    }
                    else if (!property_tag.tolower()
                             .compare("maximalexcitation")) {
                        ...
                        file.read(PCSA, variable);
                             ...
                             .compare("fatigueparameters")) {
                        while (file.read(subproperty_tag)
                              && subproperty_tag.tolower()
                              .compare("endfatigueparameters")) {
                            if (!subproperty_tag.tolower()
                                    .compare("type")) {
                                biorbd::utils::String tp_fatigue_type;
                                file.read(tp_fatigue_type);
                                if (!tp_fatigue_type.tolower()
                                        .compare("simple")) {
                                    dynamicFatigueType =
                                            biorbd::muscles::STATE_FATIGUE_TYPE
                                            ::SIMPLE_STATE_FATIGUE;
                                }
                                else if (!tp_fatigue_type.tolower()
                                         .compare("xia")) {
                                    dynamicFatigueType =
                                            biorbd::muscles::STATE_FATIGUE_TYPE
                                            ::DYNAMIC_XIA;
                                ...
                                else if (!subproperty_tag.tolower()
                                         .compare("recoveryrate")) {
                                    fatigueParameters.recoveryRate(param);
                                }
                                else if (!subproperty_tag.tolower()
                                         .compare("developfactor")) {
                ...
                biorbd::utils::Error::check(
                            idxGroup != -1, "No muscle group was provided!");
                biorbd::muscles::Geometry geo(
                            biorbd::utils::Node3d(
                                origin_pos, name + "_origin",
                                model->muscleGroup(
                                    static_cast<unsigned int>(idxGroup))
                                .origin()),
                            biorbd::utils::Node3d(
                                insert_pos, name + "_insertion",
                                model->muscleGroup(
                                    static_cast<unsigned int>(idxGroup))
                                .insertion()));
                biorbd::muscles::State stateMax(
                            maxExcitation, maxActivation);
                biorbd::muscles::Caracteristics caract(
                            optimalLength, maxForce, PCSA, tendonSlackLength,
                            pennAngle, stateMax, fatigueParameters);
                model->muscleGroup(
                            static_cast<unsigned int>(idxGroup))
                        .addMuscle(name, type, geo, caract,
                                   biorbd::muscles::PathChangers(), stateType,
                                   dynamicFatigueType);
    #else  // MODULE_MUSCLES
            biorbd::utils::Error::raise(
                        "Biorbd was build without the module Muscles "
                        "but the model defines a muscle");
    #endif  // MODULE_MUSCLES
            }
            else if (!main_tag.tolower().compare("viapoint")) {
    #ifdef MODULE_MUSCLES
                biorbd::utils::String name;
                // Nom du muscle... Éventuellement ajouter groupe musculaire
                file.read(name);
                ...
                while (file.read(property_tag)
                    ...
                    if (!property_tag.tolower().compare("parent")) {
                        ...
                        // c'est qu'aucun nom a concordé
                        biorbd::utils::Error::check(
                                    model->IsBodyId(idx), "Wrong origin parent "
                                                          "name for a muscle");
                    }
                ...
                biorbd::utils::Error::check(
                            iMuscleGroup != -1,
                            "No muscle group was provided!");
                iMuscle = model->muscleGroup(
                            static_cast<unsigned int>(iMuscleGroup))
                        .muscleID(muscle);
                biorbd::utils::Error::check(
                            iMuscle != -1,
                            "No muscle was provided!");
                position.setName(name);
                position.setParent(parent);
                model->muscleGroup(static_cast<unsigned int>(iMuscleGroup))
                        .muscle(static_cast<unsigned int>(iMuscle))
                        .addPathObject(position);
    #else  // MODULE_MUSCLES
            biorbd::utils::Error::raise(
                        "Biorbd was build without the module Muscles "
                        "but the model defines a viapoint");
    #endif  // MODULE_MUSCLES
            }
                ...
                biorbd::utils::Error::check
                        (dia != 0.0, "Diameter was not defined");
                biorbd::utils::Error::check(
                            length != 0.0, "Length was not defined");
                biorbd::utils::Error::check(
                            length < 0.0, "Side was not properly defined");
                biorbd::utils::Error::check(
                            parent != "", "Parent was not defined");
                iMuscleGroup = model->getGroupId(
                            musclegroup);
                biorbd::utils::Error::check(
                            iMuscleGroup != -1,
                            "No muscle group was provided!");
                iMuscle = model->muscleGroup(
                        ...
                        .muscleID(muscle);
                biorbd::utils::Error::check(
                            iMuscle != -1, "No muscle was provided!");
                biorbd::muscles::WrappingCylinder cylinder(
                            RT, dia, length, side, name, parent);
                model->muscleGroup(static_cast<unsigned int>(iMuscleGroup))
                        .muscle(static_cast<unsigned int>(iMuscle))
                        .addPathObject(cylinder);
    #else  // MODULE_MUSCLES
            biorbd::utils::Error::raise(
                        "Biorbd was build without the module Muscles but "
                        "the model defines a wrapping object");
    #endif  // MODULE_MUSCLES
            }
        }
    } catch (std::runtime_error message) {
        biorbd::utils::String error_message(
                    "Reading of file \"" + path.filename() + "."
                    + path.extension() + "\" failed with the following error:");
        error_message += "\n" + biorbd::utils::String(message.what()) + "\n";
        if (name.compare("")) {
            error_message += "Element: " + main_tag + ", named: " + name + "\n";
        }
        if (property_tag.compare("")
                && property_tag.find_first_of("end") != 0) {
...
biorbd::Reader::readMarkerDataFile(
    ...
    for (unsigned int j = 0; j < nbMark; j++) {
        while (tp.compare("Marker")) {
            ...
            biorbd::utils::Error::check(
                        check, "Marker file error, wrong size "
                               "of marker or intervals?");
        }

        unsigned int noMarker;
        file.read(noMarker);
        for (unsigned int i = 0; i <= nbIntervals; i++) {
            // <= parce qu'il y a nbIntervals+1 valeurs
...
biorbd::Reader::readQDataFile(
    ...
    for (unsigned int j = 0; j < nbIntervals+1; j++) {
        while (tp.compare("T")) {
            bool check = file.read(tp);
            biorbd::utils::Error::check(
                        check, "Kin file error, wrong "
                               "size of NDDL or intervals?");
        }
...
std::vector<biorbd::utils::Vector> biorbd::Reader::readActivationDataFile(
        const biorbd::utils::String &path) {
    // Ouverture du fichier
    // std::cout << "Loading kin file: " << path << std::endl;
    biorbd::utils::IfStream file(path.c_str(), std::ios::in);
    ...
    for (unsigned int j = 0; j < nbIntervals+1; j++) {
        while (tp.compare("T")) {
            bool check = file.read(tp);
            biorbd::utils::Error::check(
                        check, "Kin file error, wrong size of number "
                               "of muscles or intervals?");
        }
...
std::vector<biorbd::utils::Vector> biorbd::Reader::readTorqueDataFile(
        const biorbd::utils::String &path) {
    // Ouverture du fichier
    // std::cout << "Loading kin file: " << path << std::endl;
    biorbd::utils::IfStream file(path.c_str(), std::ios::in);
    ...
    for (unsigned int j = 0; j < nbIntervals+1; j++) {
        while (tp.compare("T")) {
            bool check = file.read(tp);
            biorbd::utils::Error::check(
                        check, "Kin file error, wrong size of "
                               "NGeneralizedTorque or intervals?");
        }
...
std::vector<biorbd::utils::Vector> biorbd::Reader::readGrfDataFile(
        const biorbd::utils::String &path) {
    // Ouverture du fichier
...
void biorbd::Reader::readViconForceFile(
        ...
        std::vector<std::vector<unsigned int>> &frame,  // Time * number of pf
        std::vector<unsigned int> &frequency,  // frequency * n_pf
        std::vector<std::vector<biorbd::utils::Node3d>> &force,  // F * n_pf
        std::vector<std::vector<biorbd::utils::Node3d>> &moment,  // M * n_pf
        std::vector<std::vector<biorbd::utils::Node3d>> &cop) {  // CoP * n_pf
    // Ouverture du fichier
    ...
    while (!file.eof()) {
        ...
        while (!file.eof()) {  // return false if it fails
            ...
            // Si la ligne est vide, c'est qu'on a fini avec les forces pour
            // cette plate-forme
            if (!tp.compare("")) {
                break;
            }
            ...
            // now we'll use a stringstream to separate the fields out of the
            ...
            while (getline(ss, field, ',' )) {
                ...
                // (since we require that the CSV contains nothing but
            ...
            // S'assurer que la ligne faisait le bon nombre d'éléments
            // (2 temps, 3 cop, 3 forces, 3 moments)
            biorbd::utils::Error::check(data.size() == 11,
                        "Wrong number of element in a line in the force file");

            // Remplir les champs
            frame1pf.push_back(static_cast<unsigned int>(data[0]));  // Frames
...
biorbd::Reader::readViconForceFile(
    ...
    std::vector<std::vector<biorbd::utils::Node3d>> force;  // Linear forces
    std::vector<std::vector<biorbd::utils::Node3d>> moment;  // Moments
    std::vector<std::vector<biorbd::utils::Node3d>> cop;  // Center of pressure
    readViconForceFile(path, frame, frequency, force, moment, cop);

    // Redispatch des valeurs dans un vecteur de SpatialTransform
    std::vector<std::vector<RigidBodyDynamics::Math::SpatialVector>> st;
    for (unsigned int j = 0; j < force.size(); ++j) {
        std::vector<RigidBodyDynamics::Math::SpatialVector> tp;
        for (unsigned int i = 0; i < force[j].size(); ++i) {
            const biorbd::utils::Node3d& f = force[j][i];
            const biorbd::utils::Node3d& m = moment[j][i];
            tp.push_back(RigidBodyDynamics::Math::SpatialVector(
...
biorbd::Reader::readViconMarkerFile(
    ...
    for (unsigned int i = 0; i < idx_init.size()-1; ++i) {
        biorbd::utils::String tp;
        for (unsigned int j = *(idx_init.begin()+i)+1;
             j < *(idx_end.begin()+i); ++j)
            tp.push_back(t.at(j));
    ...
    if (nNodes != -1) {  // Si c'est tous, les jumps sont de 1
        ...
        for (unsigned int i = 0; i < 7; ++i)
            file.read(t);
        biorbd::utils::Error::check(nNodes != 0
                && nNodes != 1
                && static_cast < unsigned int>(nNodes) <= nbFrames,
                    "nNode should not be 0, 1 or greater than number of frame");
    ...
    while (!file.eof()) {
        biorbd::utils::Vector data_tp = biorbd::utils::Vector(
                    static_cast<unsigned int>(3*markOrder.size())).setZero();
        ...
        while (getline(ss, field, ',' )) {
            ...
            double f = 0.0;  // (default value is 0.0)
            fs >> f;
            if (cmp > 1 && cmp < 3*MarkersInFile.size()+2) {
                // Retirer les timespans et ne pas dépasser
                int idx = ordre[cmp-2];
                if (idx >= 0) {
        ...
        for (unsigned int i = 0;
             i < static_cast<unsigned int>(data_tp.size())/3; ++i) {
...
biorbd::rigidbody::BoneMesh biorbd::Reader::readBoneMeshFileBiorbdBones(
        const biorbd::utils::Path &path) {
