...
utils::Rotation::Rotation(const utils::Matrix3d& matrix)
...
utils::Rotation::Rotation(const RBDLCasadiMath::MX_Xd_static<3, 3>& other)
    : utils::Matrix3d(other) {}

utils::Rotation::Rotation(const RBDLCasadiMath::MX_Xd_SubMatrix& other)
    : utils::Matrix3d(other) {}
#endif

utils::Rotation::Rotation(
    const utils::Scalar& v00,
    const utils::Scalar& v01,
    const utils::Scalar& v02,
    const utils::Scalar& v10,
    const utils::Scalar& v11,
    const utils::Scalar& v12,
    const utils::Scalar& v20,
    const utils::Scalar& v21,
...
utils::Rotation::Rotation(const utils::SpatialTransform& st)
    : utils::Matrix3d(st.E) {}
...
utils::Rotation utils::Rotation::fromEulerAngles(
  ...
  for (size_t i = 0; i < seq.length(); ++i) {
    ...
    if (seq.tolower()[i] == 'x')
      tp = utils::Matrix3d(1, 0, 0, 0, cosVi, -sinVi, 0, sinVi, cosVi);

    else if (seq.tolower()[i] == 'y')
      tp = utils::Matrix3d(cosVi, 0, sinVi, 0, 1, 0, -sinVi, 0, cosVi);

    else if (seq.tolower()[i] == 'z')
      tp = utils::Matrix3d(cosVi, -sinVi, 0, sinVi, cosVi, 0, 0, 0, 1);
    else {
...
utils::Rotation utils::Rotation::fromMarkersNonNormalized(
  ...
  if (!axesNames.first.compare("") || !axesNames.second.compare("")) {
    utils::Error::raise(
        "axesNames must be defined with a pair of \"x\", \"y\" or \"z\"");
  }
...
utils::Rotation utils::Rotation::fromMarkers(
    ...
    const std::pair<utils::String, utils::String>& axesNames,
    const utils::String& axisToRecalculate) {
  utils::Rotation r_out(fromMarkersNonNormalized(
      axis1markers, axis2markers, axesNames, axisToRecalculate));
...
utils::Vector utils::Rotation::toEulerAngles(
  ...
  // https://pdfslide.net/documents/euler-angles-58fcebd3620f7.html
  if (!seq.compare("x")) {
...
void utils::Rotation::checkUnitary() {
  ...
  if (!checkUnit) {
    ...
    utils::Error::raise(
        ...
        sqrtNorm +
        ", but should be equal to 3. Consider replacing the RT matrix by this "
        "closest orthonormal matrix \n[" +
        R(0, 0) + ", " + R(0, 1) + ", " + R(0, 2) + "\n" + R(1, 0) + ", " +
        R(1, 1) + ", " + R(1, 2) + "\n" + R(2, 0) + ", " + R(2, 1) + ", " +
        R(2, 2) + "]\n" +
        "Alternatively, you can compile with SKIP_ASSERT set to ON to turn off "
        "this error message");
  }
...
void utils::Rotation::operator=(const RBDLCasadiMath::MX_Xd_SubMatrix& other) {
  this->Matrix3d::operator=(other);
}
#endif
