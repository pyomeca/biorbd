...
std::vector<utils::Equation> utils::Equation::prepareMathSymbols() {
...
utils::Equation::Equation() : utils::String("") {}

utils::Equation::Equation(const char* c) : utils::String(c) {}

utils::Equation::Equation(const utils::String& s) : utils::String(s) {}
...
std::vector<utils::Equation> utils::Equation::splitIntoEquation(
  ...
  while (1) {
    int firstIdx(
        static_cast<int>(wholeEq.size()) + 1);  // Assign an index too big
    ...
    for (size_t i = 0; i < symbols.size(); ++i) {
      ...
      if (idx < firstIdx &&
          idx != -1) {  // If yes, look if it's before another symbol
        firstIdx = idx;
      }
    }
    ...
    if (firstIdx + 1 == static_cast<int>(wholeEq.size())) {
      ...
      if (!wholeEq(0).compare("-")) {
        ...
        std::vector<utils::Equation> tp(
            splitIntoEquation(wholeEq.substr(1), variables));
        if (eq.size() != 0 && !eq[eq.size() - 1].compare("e")) {
          ...
          double res(
              -1 * evaluateEquation(splitIntoEquation(newWholeEq, variables)));
        ...
        std::vector<utils::Equation> tp(
            splitIntoEquation(wholeEq.substr(1), variables));
        if (eq.size() !=
            0) {  // If there's nothing before, it's false in all cases 1ex
          eq.push_back("+");
        }
        eq.insert(eq.end(), tp.begin(), tp.end());
        return eq;
      ...
      eq.push_back(wholeEq.substr(
          0, static_cast<size_t>(firstIdx)));  // Take everything before symbol
      eq.push_back(wholeEq(static_cast<size_t>(firstIdx)));  // Get the symbol
      wholeEq = wholeEq.substr(
          static_cast<size_t>(firstIdx) +
...
void utils::Equation::replaceCste(std::vector<utils::Equation>& eq) {
...
void utils::Equation::replaceVar(
  ...
  for (auto var : variables)
    while (eq.find(var.first) != std::string::npos) {
      ...
      eq = eq.substr(0, pos) + "(" + to_string(var.second) + ")" +
           eq.substr(pos + length);
    }
}

double utils::Equation::evaluateEquation(std::vector<utils::Equation> wholeEq) {
...
double utils::Equation::evaluateEquation(
  ...
  for (size_t j = 0; j < eq.size(); ++j) {
    if (!eq[j].compare(symbols[math]) && continuer) {
      if (j == 0 &&
          (!symbols[math].compare("+") || !symbols[math].compare("-"))) {
        ...
        if (!symbols[math].compare("(")) {
          ...
          if (!eq[j + 1].compare("-")) {
            eq2[j - 1] =
                to_string(stod(eq[j - 1]) * pow(10, -1 * stod(eq[j + 2])));
            ++j;
...
double utils::Equation::evaluateEquation(utils::Equation wholeEq) {
