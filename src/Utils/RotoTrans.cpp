...
utils::RotoTrans::RotoTrans(const RigidBodyDynamics::Math::Matrix4d& matrix)
...
utils::RotoTrans::RotoTrans(
    ...
    const utils::Scalar& v30,
    const utils::Scalar& v31,
...
utils::RotoTrans::RotoTrans(const utils::Rotation& rot)
    : RigidBodyDynamics::Math::Matrix4d(
          combineRotAndTrans(rot, utils::Vector3d::Zero())) {}
...
utils::RotoTrans::RotoTrans(const utils::SpatialTransform& st)
...
utils::RotoTrans utils::RotoTrans::fromMarkers(
  ...
  RotoTrans rt_out;
  rt_out.block(0, 0, 3, 3) = Rotation::fromMarkers(
...
utils::RotoTrans utils::RotoTrans::transpose() const {
  ...
  tp.block(3, 0, 1, 4) =
      RigidBodyDynamics::Math::Vector4d(0, 0, 0, 1).transpose();
  return tp;
}
...
utils::RotoTrans utils::RotoTrans::mean(const std::vector<RotoTrans>& mToMean) {
  ...
  utils::RotoTrans m_out(utils::Rotation::mean(rotations), v_tp);
  return m_out;
}
#endif
...
void utils::RotoTrans::checkUnitary() {
  ...
  utils::Error::check(
      this->block(3, 0, 1, 4).sum() == 1.,
      "Last row of the RotoTrans should be (0,0,0,1");
  utils::Error::check(
      (*this)(3, 3) == 1., "Last row of the RotoTrans should be (0,0,0,1");
#endif
#endif
}
