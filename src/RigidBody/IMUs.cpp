...
rigidbody::IMUs::IMUs(const rigidbody::IMUs &other) { m_IMUs = other.m_IMUs; }
...
void rigidbody::IMUs::addIMU(bool technical, bool anatomical) {
...
size_t rigidbody::IMUs::nbIMUs() const { return m_IMUs->size(); }
...
const rigidbody::IMU &rigidbody::IMUs::IMU(size_t idx) {
  return (*m_IMUs)[idx];
}
...
std::vector<utils::Matrix> rigidbody::IMUs::TechnicalIMUJacobian(
    const rigidbody::GeneralizedCoordinates &Q,
    bool updateKin) {
...
std::vector<utils::String> rigidbody::IMUs::anatomicalIMUsNames() {
