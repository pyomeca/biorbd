...
rigidbody::RotoTransNodes rigidbody::RotoTransNodes::DeepCopy() const {
...
void rigidbody::RotoTransNodes::addRT(const utils::RotoTransNode &RotoTrans) {
  m_RTs->push_back(utils::RotoTransNode(RotoTrans));
}

size_t rigidbody::RotoTransNodes::nbRTs() const { return m_RTs->size(); }

size_t rigidbody::RotoTransNodes::size() const { return m_RTs->size(); }

// Get the markers in the global reference
const std::vector<utils::RotoTransNode> &rigidbody::RotoTransNodes::RTs()
    const {
  return *m_RTs;
}

std::vector<utils::RotoTransNode> rigidbody::RotoTransNodes::RTs(
...
const utils::RotoTransNode &rigidbody::RotoTransNodes::RT(size_t idx) {
  return (*m_RTs)[idx];
}
...
std::vector<utils::RotoTransNode> rigidbody::RotoTransNodes::segmentRTs(
    const rigidbody::GeneralizedCoordinates &Q,
    size_t idx,
    bool updateKin) {
  // Assuming that this is also a Joints type (via BiorbdModel)
...
std::vector<utils::Matrix> rigidbody::RotoTransNodes::RTsJacobian(
    const rigidbody::GeneralizedCoordinates &Q,
    bool updateKin) {
  // Assuming that this is also a Joints type (via BiorbdModel)
  rigidbody::Joints &model = dynamic_cast<rigidbody::Joints &>(*this);
...
void rigidbody::RotoTransNodes::clear() { m_RTs->clear(); }