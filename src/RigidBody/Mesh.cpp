...
void rigidbody::Mesh::setColor(const utils::Vector3d &color) {
  *m_patchColor = color;
}

utils::Vector3d &rigidbody::Mesh::color() const { return *m_patchColor; }

bool rigidbody::Mesh::hasMesh() const { return m_vertex->size() > 0; }
void rigidbody::Mesh::addPoint(const utils::Vector3d &node) {
  m_vertex->push_back(node);
}
const utils::Vector3d &rigidbody::Mesh::point(size_t idx) const {
  return (*m_vertex)[idx];
}
size_t rigidbody::Mesh::nbVertex() const { return m_vertex->size(); }

void rigidbody::Mesh::rotate(const utils::RotoTrans &rt) {
  *m_rotation = rt;
  for (auto &v : *m_vertex) {
    v.applyRT(rt);
  }
...
utils::RotoTrans &rigidbody::Mesh::getRotation() const { return *m_rotation; }

void rigidbody::Mesh::scale(const utils::Vector3d &scaler) {
  *m_scale = scaler;
  for (auto &v : *m_vertex) {
    v(0) *= scaler(0);
    v(1) *= scaler(1);
...
utils::Vector3d &rigidbody::Mesh::getScale() const { return *m_scale; }

size_t rigidbody::Mesh::nbFaces() { return m_faces->size(); }
...
const std::vector<rigidbody::MeshFace> &rigidbody::Mesh::faces() const {
  return *m_faces;
}
const rigidbody::MeshFace &rigidbody::Mesh::face(size_t idx) const {
  return (*m_faces)[idx];
}

void rigidbody::Mesh::setPath(const utils::Path &path) { *m_pathFile = path; }

utils::Path &rigidbody::Mesh::path() { return *m_pathFile; }

const utils::Path &rigidbody::Mesh::path() const { return *m_pathFile; }
