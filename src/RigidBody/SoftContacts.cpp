...
rigidbody::SoftContacts::SoftContacts()
    : m_softContacts(
          std::make_shared<std::vector<std::shared_ptr<SoftContactNode>>>()) {}
...
void rigidbody::SoftContacts::DeepCopy(const rigidbody::SoftContacts &other) {
  m_softContacts->resize(other.m_softContacts->size());
  for (size_t i = 0; i < other.m_softContacts->size(); ++i) {
    if ((*other.m_softContacts)[i]->typeOfNode() ==
        utils::NODE_TYPE::SOFT_CONTACT_SPHERE) {
...
utils::String rigidbody::SoftContacts::softContactName(size_t i) {
...
void rigidbody::SoftContacts::addSoftContact(
  ...
  if (contact.typeOfNode() == utils::NODE_TYPE::SOFT_CONTACT_SPHERE) {
    m_softContacts->push_back(
        std::make_shared<rigidbody::SoftContactSphere>(contact));
  } else {
    utils::Error::raise(
        utils::String("The ") + contact.typeOfNode() + " does not exist");
  }
}

rigidbody::SoftContactNode &rigidbody::SoftContacts::softContact(size_t idx) {
...
rigidbody::NodeSegment rigidbody::SoftContacts::softContact(
  ...
  return rigidbody::NodeSegment(
      model.CalcBodyToBaseCoordinates(Q, sc.parent(), sc, updateKin));
}
...
rigidbody::NodeSegment rigidbody::SoftContacts::softContactAngularVelocity(
  ...
  const rigidbody::SoftContactNode &sc(softContact(idx));
  return rigidbody::NodeSegment(
      model.CalcPointVelocity6D(Q, Qdot, sc.parent(), sc, updateKin)
          .block(0, 0, 3, 1));
}

std::vector<rigidbody::NodeSegment>
rigidbody::SoftContacts::softContactsVelocity(
    const rigidbody::GeneralizedCoordinates &Q,
    const rigidbody::GeneralizedVelocity &Qdot,
    bool updateKin) {
...
std::vector<rigidbody::NodeSegment>
rigidbody::SoftContacts::softContactsAngularVelocity(
    const rigidbody::GeneralizedCoordinates &Q,
    const rigidbody::GeneralizedVelocity &Qdot,
    bool updateKin) {
  std::vector<rigidbody::NodeSegment> pos;
...
std::vector<size_t> rigidbody::SoftContacts::segmentSoftContactIdx(
  ...
  for (size_t i = 0; i < nbSoftContacts();
