...
rigidbody::SoftContactNode::SoftContactNode()
    : rigidbody::NodeSegment(),
      m_contactPlane(
          std::make_shared<std::pair<utils::Vector3d, utils::Vector3d>>(
...
rigidbody::SoftContactNode::SoftContactNode(const utils::Vector3d &other)
...
utils::SpatialVector rigidbody::SoftContactNode::computeForceAtOrigin(
  ...
  Joints &
#endif
      updatedModel = model.UpdateKinematicsCustom(
  ...
  utils::Vector3d dx(
      rigidbody::NodeSegment(
          updatedModel.CalcPointVelocity(Q, Qdot, id, *this, updateKin)));

  utils::Vector3d x(
      updatedModel.CalcBodyToBaseCoordinates(Q, id, *this, updateKin));
  utils::Vector3d angularVelocity(
      updatedModel
          .CalcPointVelocity6D(Q, Qdot, id, utils::Vector3d(0, 0, 0), updateKin)
          .block(0, 0, 3, 1));
  ...
  const utils::Vector3d &CoM(
      updatedModel.segment(parent()).characteristics().CoM());
  utils::Vector3d CoMinGlobal(
      updatedModel.CalcBodyToBaseCoordinates(Q, id, CoM, updateKin));
