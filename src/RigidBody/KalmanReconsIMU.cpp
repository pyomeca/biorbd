...
rigidbody::KalmanReconsIMU rigidbody::KalmanReconsIMU::DeepCopy() const {
  rigidbody::KalmanReconsIMU copy;
  copy.DeepCopy(*this);
  return copy;
}

void rigidbody::KalmanReconsIMU::DeepCopy(
    const rigidbody::KalmanReconsIMU &other) {
...
bool rigidbody::KalmanReconsIMU::first() { return *m_firstIteration; }
...
void rigidbody::KalmanReconsIMU::reconstructFrame(
  ...
  utils::Vector T(
      static_cast<size_t>(3 * 3 * IMUobs.size()));  // Matrix 3*3 * nbIMU
...
void rigidbody::KalmanReconsIMU::reconstructFrame(
  ...
  if (*m_firstIteration) {
    *m_firstIteration = false;
    for (size_t i = 0; i < 300; ++i) {
      // The first time, call in a recursive manner to have a decent
      // initial position
      reconstructFrame(model, IMUobs, nullptr, nullptr, nullptr);

      // We can't use the same trick for reinitiliazing speed in pP as IMU
      // are based on velocity
      m_xp->block(*m_nbDof, 0, *m_nbDof * 2, 1) =
          utils::Vector::Zero(*m_nbDof * 2);
    }
  }
  ...
  // Projected markers
  const std::vector<rigidbody::IMU> &zest_tp =
      updatedModel.technicalIMU(Q_tp, false);
  // Jacobian
  std::vector<utils::Matrix> J_tp =
      updatedModel.TechnicalIMUJacobian(Q_tp, false);
