...
rigidbody::ExternalForceSet::ExternalForceSet(
    ...
    : m_model(model),
      ...
      m_externalForcesInLocal(
          rigidbody::ExternalForceSet::LocalForcesInternal()),
      m_translationalForces(
...
void rigidbody::ExternalForceSet::add(
  ...
  size_t dofIndex = m_model.segment(segmentName)
                        .getLastDofIndexInGeneralizedCoordinates(m_model) +
                    1;
...
void rigidbody::ExternalForceSet::add(
  ...
  utils::SpatialVector atOrigin = transportAtOrigin(
      ...
      rigidbody::NodeSegment(
          utils::Vector3d(pointOfApplication, segmentName, segmentName)));
...
void rigidbody::ExternalForceSet::addInSegmentReferenceFrame(
  ...
  m_externalForcesInLocal.addNode(
      vector,
      utils::RotoTransNode(
          utils::RotoTrans(utils::Matrix3d::Identity(), pointOfApplication),
          segmentName,
          segmentName));
}
...
void rigidbody::ExternalForceSet::addTranslationalForce(
  ...
  if (!m_useTranslationalForces)
    throw std::runtime_error(
        "It is not possible to add translational forces if "
        "useTranslationalForces was set to false");
  m_translationalForces.push_back(std::make_pair(force, pointOfApplication));
}

std::vector<RigidBodyDynamics::Math::SpatialVector>
rigidbody::ExternalForceSet::computeRbdlSpatialVectors(
    rigidbody::Joints& updatedModel) {
  if (hasExternalForceInLocalReferenceFrame())
    throw std::runtime_error(
        "local reference frame requires Q when computing the Spatial "
        "Vectors");
  if (m_useTranslationalForces)
    throw std::runtime_error(
        "useTranslationalForce requires Q when computing the Spatial "
        "Vectors");
  if (m_useSoftContacts)
    throw std::runtime_error(
        "useSoftContacts requires Q and Qdot when computing the Spatial "
        "Vectors");
...
rigidbody::ExternalForceSet::computeRbdlSpatialVectors(
  ...
  if (m_useSoftContacts)
    throw std::runtime_error(
        "useSoftContacts requires Qdot when computing the Spatial Vectors");
  return computeRbdlSpatialVectors(
      updatedModel, Q, rigidbody::GeneralizedVelocity(updatedModel));
}
...
rigidbody::ExternalForceSet::computeRbdlSpatialVectors(
  ...
  std::vector<utils::SpatialVector> tp(
      computeSpatialVectors(updatedModel, Q, Qdot));
...
rigidbody::ExternalForceSet::computeSpatialVectors(
    rigidbody::Joints& updatedModel) {
  if (hasExternalForceInLocalReferenceFrame())
    throw std::runtime_error(
        "local reference frame requires Q when computing the Spatial "
...
rigidbody::ExternalForceSet::computeSpatialVectors(
  ...
  if (m_useSoftContacts)
    throw std::runtime_error(
        "useSoftContacts requires Qdot when computing the Spatial Vectors");
  return computeSpatialVectors(
      updatedModel, Q, rigidbody::GeneralizedVelocity());
}
...
rigidbody::ExternalForceSet::computeSpatialVectors(
  ...
  if (hasExternalForceInLocalReferenceFrame())
    combineLocalReferenceFrameForces(updatedModel, Q, out);
  if (m_useTranslationalForces)
    combineTranslationalForces(updatedModel, Q, out);
  if (m_useSoftContacts) combineSoftContactForces(updatedModel, Q, Qdot, out);

  return out;
}

bool rigidbody::ExternalForceSet::hasExternalForceInLocalReferenceFrame()
    const {
...
void rigidbody::ExternalForceSet::setZero() {
  ...
  m_externalForces.push_back(
      sv_zero);  // The first one is associated with the universe
...
void rigidbody::ExternalForceSet::combineLocalReferenceFrameForces(
  ...
  for (size_t i = 0; i < m_externalForcesInLocal.size(); i++) {
    ...
    // Traverse the segment hierarchy from root to child to root until we
    // get to a segment with at least one dof as it is not possible to add
    // forces on segment without degree of freedom
    ...
    do {
      segmentRotoTrans *=
          allGlobalJcs[updatedModel.getBodyBiorbdId(segment->name())];

      if (segment->nbDof() > 0) break;
      utils::Error::check(
          segment->parent().compare("root"),
          node.parent() +
              " should be attached to at least one segment "
    ...
    // Transport the force to the global reference frame (Add 1 to account
    // for the undeclared root)
    size_t dofIndex =
        updatedModel.segment(node.parent())
            .getLastDofIndexInGeneralizedCoordinates(updatedModel) +
        1;
    out[dofIndex] += transportAtOrigin(
        utils::SpatialVector(momentInGrf, forceInGrf), pointOfApplication);
  }
  return;
}
...
void rigidbody::ExternalForceSet::combineTranslationalForces(
  ...
  for (auto& e : m_translationalForces) {
    ...
    const rigidbody::Segment& segment(
        updatedModel.segment(pointOfApplication.parent()));
    size_t dofIndex =
        segment.getLastDofIndexInGeneralizedCoordinates(updatedModel) + 1;

    const utils::Vector3d& force = e.first;
    rigidbody::NodeSegment pointOfApplicationInGlobal(
        updatedModel.CalcBodyToBaseCoordinates(
            Q,
            static_cast<unsigned int>(segment.id()),
            pointOfApplication,
            false),
    ...
    // Add the force to the force vector (do not subtract 1 because 0 is the
    // base)
...
utils::SpatialVector rigidbody::ExternalForceSet::transportAtOrigin(
    ...
    const rigidbody::NodeSegment& pointOfApplication) const {
  // Transport to Origin (Bour's formula)
  utils::SpatialVector out(
      transportForceAtOrigin(v.force(), pointOfApplication));
