...
rigidbody::NodeSegment::NodeSegment(const utils::Vector3d& other)
...
void rigidbody::NodeSegment::DeepCopy(const rigidbody::NodeSegment& other) {
...
void rigidbody::NodeSegment::setValues(const rigidbody::NodeSegment& other) {
  (*this)[0] = other[0];
  (*this)[1] = other[1];
  (*this)[2] = other[2];
}

bool rigidbody::NodeSegment::isAnatomical() const { return *m_anatomical; }

bool rigidbody::NodeSegment::isTechnical() const { return *m_technical; }

int rigidbody::NodeSegment::parentId() const { return *m_id; }
...
int rigidbody::NodeSegment::nbAxesToRemove() const { return *m_nbAxesToRemove; }
...
void rigidbody::NodeSegment::addAxesToRemove(const utils::String& s) {
...
void rigidbody::NodeSegment::addAxesToRemove(const std::vector<size_t>& axes) {
...
void rigidbody::NodeSegment::addAxesToRemove(
    const std::vector<utils::String>& axes) {
  for (size_t i = 0; i < axes.size(); ++i) {
    addAxesToRemove(axes[i]);
  }
}
