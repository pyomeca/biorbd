...
size_t rigidbody::Joints::nbGeneralizedTorque() const { return nbQddot(); }
size_t rigidbody::Joints::nbDof() const { return *m_nbDof; }
...
size_t rigidbody::Joints::nbQ() const { return *m_nbQ; }
size_t rigidbody::Joints::nbQdot() const { return *m_nbQdot; }
size_t rigidbody::Joints::nbQddot() const { return *m_nbQddot; }
size_t rigidbody::Joints::nbRoot() const { return *m_nbRoot; }

utils::Scalar rigidbody::Joints::mass() const { return *m_totalMass; }
...
size_t rigidbody::Joints::AddSegment(
  ...
  if (this->GetBodyId(parentName.c_str()) ==
    ...
    *m_nbRoot += tp.nbDof();  // If the segment name is "Root", add the
                              // number of DoF of root
  }
  *m_nbDof += tp.nbDof();
  *m_nbQ += tp.nbQ();
  *m_nbQdot += tp.nbQdot();
...
size_t rigidbody::Joints::AddSegment(
  ...
  if (this->GetBodyId(parentName.c_str()) ==
      std::numeric_limits<unsigned int>::max()) {
    *m_nbRoot += tp.nbDof();  //  If the name of the segment is "Root", add
                              //  the number of DoF of root
  }
  *m_nbDof += tp.nbDof();
...
utils::Vector3d rigidbody::Joints::getGravity() const { return gravity; }

void rigidbody::Joints::setGravity(const utils::Vector3d &newGravity) {
  gravity = newGravity;
}
...
void rigidbody::Joints::updateSegmentCharacteristics(
    ...
    const rigidbody::SegmentCharacteristics &characteristics) {
  utils::Error::check(
      idx < m_segments->size(), "Asked for a wrong segment (out of range)");
  (*m_segments)[idx].updateCharacteristics(*this, characteristics);
}

rigidbody::Segment &rigidbody::Joints::segment(size_t idx) {
  utils::Error::check(
      idx < m_segments->size(), "Asked for a wrong segment (out of range)");
  return (*m_segments)[idx];
}

rigidbody::Segment &rigidbody::Joints::segment(const utils::String &name) {
  return segment(static_cast<size_t>(getBodyBiorbdId(name.c_str())));
}

const rigidbody::Segment &rigidbody::Joints::segment(size_t idx) const {
  utils::Error::check(
      idx < m_segments->size(), "Asked for a wrong segment (out of range)");
  return (*m_segments)[idx];
}
...
size_t rigidbody::Joints::nbSegment() const { return m_segments->size(); }

int rigidbody::Joints::getBodyBiorbdId(const utils::String &segmentName) const {
...
int rigidbody::Joints::getBodyRbdlId(const utils::String &segmentName) const {
  return GetBodyId(segmentName.c_str());
}

int rigidbody::Joints::getBodyRbdlIdToBiorbdId(const int idx) const {
...
size_t rigidbody::Joints::getBodyBiorbdIdToRbdlId(const int idx) const {
...
std::vector<std::vector<size_t>> rigidbody::Joints::getDofSubTrees() {
  ...
  for (size_t i = 1; i < this->mu.size();
       ++i) {  // begin at 1 because 0 is its own parent in rbdl.
    if (this->lambda[i] == 0) {
  ...
  for (size_t i = 0; i < dof_with_no_parent_id.size(); ++i) {
    ...
    std::vector<std::vector<size_t>> subTrees_temp_filled =
        recursiveDofSubTrees(subTrees_temp, dof_id);
...
std::vector<std::vector<size_t>> rigidbody::Joints::recursiveDofSubTrees(
  ...
  if (child_idx.size() > 0) {
    for (size_t i = 0; i < child_idx.size(); ++i) {
      ...
      subTrees_filled[idx].insert(
          subTrees_filled[idx].end(),
          subTree_child.begin(),
          subTree_child.end());
    }
  }
...
utils::RotoTrans rigidbody::Joints::globalJCS(
    ...
    size_t idx,
    bool updateKin) {
  return this->CalcBodyWorldTransformation(
      Q, (*m_segments)[idx].id(), updateKin);
}

std::vector<utils::RotoTrans> rigidbody::Joints::localJCS() const {
  std::vector<utils::RotoTrans> out;
  for (size_t i = 0; i < m_segments->size(); ++i) {
...
utils::RotoTrans rigidbody::Joints::localJCS(const utils::String &name) const {
  return localJCS(static_cast<size_t>(getBodyBiorbdId(name.c_str())));
}

utils::RotoTrans rigidbody::Joints::localJCS(const size_t idx) const {
...
utils::Vector3d rigidbody::Joints::CalcBodyToBaseCoordinates(
  ...
  return this->CalcBodyToBaseCoordinates(
      Q, this->GetBodyId(segmentName.c_str()), pointInLocal, updateKin);
}
...
utils::Vector3d rigidbody::Joints::CalcBodyToBaseCoordinates(
  ...
  return RigidBodyDynamics::CalcBodyToBaseCoordinates(
      model, Q, bodyId, pointInLocal, false);
}
...
utils::Vector3d rigidbody::Joints::CalcPointVelocity(
  ...
  return this->CalcPointVelocity(
      Q, Qdot, this->GetBodyId(segmentName.c_str()), pointInLocal, updateKin);
}
...
utils::Vector3d rigidbody::Joints::CalcPointVelocity(
  ...
  rigidbody::Joints &
      ...
      model = this->UpdateKinematicsCustom(
          updateKin ? &Q : nullptr, updateKin ? &Qdot : nullptr);

  return RigidBodyDynamics::CalcPointVelocity(
      model, Q, Qdot, bodyId, pointInLocal, false);
}
...
utils::SpatialVector rigidbody::Joints::CalcPointVelocity6D(
    ...
    const utils::Vector3d &pointInLocal,
    bool updateKin) {
  return this->CalcPointVelocity6D(
      Q, Qdot, this->GetBodyId(segmentName.c_str()), pointInLocal, updateKin);
}
...
utils::Vector3d rigidbody::Joints::CalcPointAcceleration(
  ...
  return this->CalcPointAcceleration(
      ...
      Qdot,
      Qddot,
      this->GetBodyId(segmentName.c_str()),
      pointInLocal,
      updateKin);
}
...
utils::Vector3d rigidbody::Joints::CalcPointAcceleration(
  ...
  return RigidBodyDynamics::CalcPointAcceleration(
      model, Q, Qdot, Qddot, bodyId, pointInLocal, false);
}
...
utils::Matrix rigidbody::Joints::CalcPointJacobian(
  ...
  RigidBodyDynamics::CalcPointJacobian(
      model, Q, bodyId, pointInLocal, out, false);
  return out;
}
...
std::vector<rigidbody::NodeSegment> rigidbody::Joints::projectPoint(
  ...
  for (size_t i = 0; i < marks.nbMarkers(); ++i) {
    ...
    if (tp.nbAxesToRemove() != 0) {
      // Project the marker by removing an axis by using the current pose
      // of the model
      utils::Vector3d pos = v[i].applyRT(
          updatedModel.globalJCS(Q, tp.parent(), false).transpose());

      out.push_back(updatedModel.projectPoint(Q, pos, false));
    } else {
      // If there isn't any projection to perform, just use the actual
      // marker
      out.push_back(v[i]);
    }
  }
...
rigidbody::NodeSegment rigidbody::Joints::projectPoint(
  ...
  // Create a marker
  const utils::String &segmentName(
      segment(static_cast<size_t>(segmentIdx)).name());
  rigidbody::NodeSegment node(
      v.applyRT(
          updatedModel.globalJCS(Q, static_cast<size_t>(segmentIdx), false)
              .transpose()),
      "tp",
      segmentName,
      true,
...
rigidbody::NodeSegment rigidbody::Joints::projectPoint(
    ...
    bool updateKin) {
  // Assuming that this is also a Marker type (via BiorbdModel)
  return dynamic_cast<rigidbody::Markers &>(*this).marker(
      Q, n, updateKin, true);
}
...
utils::Matrix rigidbody::Joints::projectPointJacobian(
  ...
  utils::Matrix G_tp(
      marks.markersJacobian(Q, node.parent(), utils::Vector3d(0, 0, 0), false));
  utils::Matrix JCor(utils::Matrix::Zero(9, static_cast<unsigned int>(nbQ())));
  updatedModel.CalcMatRotJacobian(
      Q,
      GetBodyId(node.parent().c_str()),
  ...
  for (size_t n = 0; n < 3; ++n) {
    if (node.isAxisKept(n)) {
      G_tp += JCor.block(
                  ...
                  0,
                  3,
                  static_cast<unsigned int>(nbQ())) *
              node(static_cast<unsigned int>(n));
    }
  }
...
std::vector<utils::Matrix> rigidbody::Joints::projectPointJacobian(
  ...
  // Gather the points
  const std::vector<rigidbody::NodeSegment> &tp(
      updatedModel.projectPoint(Q, v, false));
  updateKin = false;
  ...
  for (size_t i = 0; i < tp.size(); ++i) {
    // Actual marker
    G.push_back(updatedModel.projectPointJacobian(
        Q, rigidbody::NodeSegment(v[i]), false));
  }
  return G;
}
...
utils::SpatialTransform rigidbody::Joints::CalcBodyWorldTransformation(
  ...
  if (segmentIdx >= updatedModel.fixed_body_discriminator) {
    size_t fbody_id =
        segmentIdx - static_cast<size_t>(updatedModel.fixed_body_discriminator);
    size_t parent_id =
        static_cast<size_t>(updatedModel.mFixedBodies[fbody_id].mMovableParent);
  ...
  return utils::SpatialTransform(
      updatedModel.X_base[segmentIdx].E.transpose(),
      updatedModel.X_base[segmentIdx].r);
}
...
utils::Vector3d rigidbody::Joints::segmentAngularVelocity(
  ...
  return this
      ->CalcPointVelocity6D(
          Q, Qdot, segment(idx).name(), utils::Vector3d(0, 0, 0), updateKin)
      .block(0, 0, 3, 1);
}
...
utils::Vector3d rigidbody::Joints::CoM(
    const rigidbody::GeneralizedCoordinates &Q,
    bool updateKin) {
  // For each segment, find the CoM (CoM = sum(segment_mass * pos_com_seg) /
  // total mass)
  const std::vector<rigidbody::NodeSegment> &com_segment(
      CoMbySegment(Q, updateKin));
...
utils::Matrix rigidbody::Joints::massMatrix(
  ...
  utils::Matrix massMatrix(
      static_cast<unsigned int>(nbQ()), static_cast<unsigned int>(nbQ()));
  massMatrix.setZero();
  RigidBodyDynamics::CompositeRigidBodyAlgorithm(
      updatedModel, Q, massMatrix, false);
  return massMatrix;
}
...
utils::Matrix rigidbody::Joints::massMatrixInverse(
  ...
  for (i = static_cast<int>(updatedModel.mBodies.size() - 1); i > 0; i--) {
    unsigned int q_index_i =
        static_cast<size_t>(updatedModel.mJoints[i].q_index);
    ...
    for (j = 0; j < sub_tree.size(); j++) {
      utils::SpatialVector Ftemp(
          F[q_index_i].block(0, static_cast<unsigned int>(sub_tree[j]), 6, 1));
      Minv(q_index_i, static_cast<unsigned int>(sub_tree[j])) -=
          (1.0 / updatedModel.d[i]) * updatedModel.S[i].transpose() * Ftemp;
    }

    size_t lambda = static_cast<size_t>(updatedModel.lambda[i]);
    size_t lambda_q_i =
        static_cast<size_t>(updatedModel.mJoints[lambda].q_index);
    if (lambda != 0) {
      for (j = 0; j < sub_tree.size(); j++) {
        F[q_index_i].block(0, static_cast<unsigned int>(sub_tree[j]), 6, 1) +=
            updatedModel.U[i] *
            Minv.block(q_index_i, static_cast<unsigned int>(sub_tree[j]), 1, 1);

        F[lambda_q_i].block(0, static_cast<unsigned int>(sub_tree[j]), 6, 1) +=
            updatedModel.X_lambda[i].toMatrixTranspose() *
            F[q_index_i].block(0, static_cast<unsigned int>(sub_tree[j]), 6, 1);
      }

      RigidBodyDynamics::Math::SpatialMatrix Ia =
          updatedModel.IA[i] -
          updatedModel.U[i] *
              (updatedModel.U[i] / updatedModel.d[i]).transpose();

#ifdef BIORBD_USE_CASADI_MATH
      updatedModel.IA[lambda] += updatedModel.X_lambda[i].toMatrixTranspose() *
                                 Ia * updatedModel.X_lambda[i].toMatrix();

#else
      updatedModel.IA[lambda].noalias() +=
          updatedModel.X_lambda[i].toMatrixTranspose() * Ia *
          updatedModel.X_lambda[i].toMatrix();
#endif
    }
  }
  // End Backward Pass
  ...
  for (i = 1; i < updatedModel.mBodies.size(); i++) {
    ...
    if (lambda != 0) {
      for (j = q_index_i; j < static_cast<int>(updatedModel.dof_count); j++) {
        ...
        Minv(q_index_i, j) -=
            (1.0 / updatedModel.d[i]) *
            (updatedModel.U[i].transpose() * X_lambda.toMatrix()) * Ftemp;
      }
    }
    // F[i,:,i:] = np.outer(S,Minv[i,i:]) // could be simplified (S *
    // M[q_index_i,q_index_i:]^T)
    for (j = q_index_i; j < static_cast<int>(updatedModel.dof_count); j++) {
      F[q_index_i].block(0, j, 6, 1) =
          updatedModel.S[i] * Minv.block(q_index_i, j, 1, 1);  // outer product
    }
...
utils::Vector3d rigidbody::Joints::CoMdot(
  ...
  for (const auto &segment : *m_segments) {
    utils::Matrix Jac = updatedModel.CalcPointJacobian(
        Q, segment.name(), segment.characteristics().mCenterOfMass, false);
...
utils::Vector3d rigidbody::Joints::CoMbySegment(
    ...
    bool updateKin) {
  utils::Error::check(
      idx < m_segments->size(), "Choosen segment doesn't exist");
...
std::vector<utils::Matrix> rigidbody::Joints::meshPointsInMatrix(
    const rigidbody::GeneralizedCoordinates &Q,
    bool updateKin) {
  const std::vector<utils::RotoTrans> &RT(allGlobalJCS(Q, updateKin));
...
std::vector<std::vector<rigidbody::MeshFace>> rigidbody::Joints::meshFaces()
    const {
...
const std::vector<rigidbody::MeshFace> &rigidbody::Joints::meshFaces(
    size_t idx) const {
...
const rigidbody::Mesh &rigidbody::Joints::mesh(size_t idx) const {
...
std::vector<utils::Vector3d> rigidbody::Joints::CalcSegmentsAngularMomentum(
    const rigidbody::GeneralizedCoordinates &Q,
    const rigidbody::GeneralizedVelocity &Qdot,
    bool updateKin) {
#ifdef BIORBD_USE_CASADI_MATH
...
#endif
      updatedModel = this->UpdateKinematicsCustom(
          updateKin ? &Q : nullptr, updateKin ? &Qdot : nullptr, nullptr);
  ...
  for (size_t i = 1; i < updatedModel.mBodies.size(); i++) {
    updatedModel.Ic[i] = updatedModel.I[i];
    updatedModel.hc[i] = updatedModel.Ic[i].toMatrix() * updatedModel.v[i];

    utils::SpatialVector h =
        updatedModel.X_lambda[i].applyTranspose(updatedModel.hc[i]);
    if (updatedModel.lambda[i] != 0) {
      size_t j(i);
      do {
        j = updatedModel.lambda[j];
        h = updatedModel.X_lambda[j].applyTranspose(h);
      } while (updatedModel.lambda[j] != 0);
    }
...
size_t rigidbody::Joints::nbQuat() const { return *m_nRotAQuat; }
  ...
  for (size_t i = 0; i < nbSegment(); ++i) {
    ...
    if (segment_i.isRotationAQuaternion()) {
      ...
      utils::Quaternion quat_tp(
          ...
          Q.block(
              cmpDof + static_cast<unsigned int>(segment_i.nbDofTrans()),
              0,
              3,
              1),
          k_stab);
      ...
      QdotOut.block(
          cmpDof, 0, static_cast<unsigned int>(segment_i.nbDofTrans()), 1) =
          Qdot.block(
              cmpDof, 0, static_cast<unsigned int>(segment_i.nbDofTrans()), 1);

      // Get the 4d derivative for the quaternion part
      quat_tp.derivate(Qdot.block(
          cmpDof + static_cast<unsigned int>(segment_i.nbDofTrans()), 0, 3, 1));
      QdotOut.block(
          cmpDof + static_cast<unsigned int>(segment_i.nbDofTrans()), 0, 3, 1) =
          quat_tp.block(1, 0, 3, 1);
      QdotOut(Q.size() - static_cast<unsigned int>(*m_nRotAQuat + cmpQuat)) =
          quat_tp(0);  // Placer dans le vecteur de sortie
      ...
      QdotOut.block(
          cmpDof, 0, static_cast<unsigned int>(segment_i.nbDof()), 1) =
          Qdot.block(
              cmpDof, 0, static_cast<unsigned int>(segment_i.nbDof()), 1);
    }
...
utils::Scalar rigidbody::Joints::KineticEnergy(
  ...
  return RigidBodyDynamics::Utils::CalcKineticEnergy(
      updatedModel, Q, Qdot, false);
}
...
utils::Scalar rigidbody::Joints::Lagrangian(
  ...
  utils::Scalar kinetic(
      RigidBodyDynamics::Utils::CalcKineticEnergy(
          updatedModel, Q, Qdot, false));
  utils::Scalar potential(
      RigidBodyDynamics::Utils::CalcPotentialEnergy(updatedModel, Q, false));
...
rigidbody::GeneralizedTorque rigidbody::Joints::InverseDynamics(
    ...
    const rigidbody::GeneralizedVelocity &Qdot,
    const rigidbody::GeneralizedAcceleration &Qddot) {
  rigidbody::ExternalForceSet forceSet(
      static_cast<BIORBD_NAMESPACE::Model &>(*this));
...
rigidbody::GeneralizedAcceleration rigidbody::Joints::ForwardDynamics(
  ...
  RigidBodyDynamics::ForwardDynamics(
      updatedModel, Q, Qdot, dampedTau, Qddot, &fExt);
  return Qddot;
}
...
rigidbody::Joints::ForwardDynamicsFreeFloatingBase(
  ...
  utils::Error::check(
      QddotJoints.size() == this->nbQddot() - this->nbRoot(),
      "Size of QddotJ must be equal to number of Qddot - "
      "number of root coordinates.");

  utils::Error::check(
      this->nbRoot() > 0, "Must have a least one degree of freedom on root.");
  ...
  utils::Matrix massMatrixRoot = this->massMatrix(Q).block(
      0,
      0,
      static_cast<unsigned int>(this->nbRoot()),
      static_cast<unsigned int>(this->nbRoot()));

  Qddot.block(0, 0, static_cast<unsigned int>(this->nbRoot()), 1) =
      utils::Vector(this->nbRoot()).setZero();
  Qddot.block(
      static_cast<unsigned int>(this->nbRoot()),
      0,
      static_cast<unsigned int>(this->nbQddot() - this->nbRoot()),
      1) = QddotJoints;
...
#ifdef BIORBD_USE_CASADI_MATH
  auto linsol =
      casadi::Linsol("linsol", "symbolicqr", massMatrixRoot.sparsity());
  QRootDDot = linsol.solve(
      massMatrixRoot,
      -MassMatrixNlEffects.block(
          0, 0, static_cast<unsigned int>(this->nbRoot()), 1));
#else
  QRootDDot = massMatrixRoot.llt().solve(-MassMatrixNlEffects.block(
      0, 0, static_cast<unsigned int>(this->nbRoot()), 1));
#endif
...
rigidbody::Joints::ForwardDynamicsConstraintsDirect(
  ...
  rigidbody::Contacts CS =
      dynamic_cast<rigidbody::Contacts *>(this)->getConstraints();
...
rigidbody::Joints::ForwardDynamicsConstraintsDirect(
  ...
  rigidbody::Contacts CS =
      dynamic_cast<rigidbody::Contacts *>(this)->getConstraints();
  return this->ForwardDynamicsConstraintsDirect(
      Q, Qdot, Tau, CS, externalForces, updateKin);
}
...
rigidbody::Joints::ForwardDynamicsConstraintsDirect(
  ...
  return ForwardDynamicsConstraintsDirect(
      Q, Qdot, Tau, CS, forceSet, updateKin);
}
...
rigidbody::Joints::ForwardDynamicsConstraintsDirect(
  ...
  RigidBodyDynamics::ForwardDynamicsConstraintsDirect(
      updatedModel, Q, Qdot, dampedTau, CS, Qddot, updateKin, &fExt);
  return Qddot;
}
...
utils::Matrix3d rigidbody::Joints::bodyInertia(
  ...
  return RigidBodyDynamics::Math::Xtrans(-com)
      .applyTranspose(Itot)
      .toMatrix()
      .block(0, 0, 3, 3);
}
...
#ifdef BIORBD_USE_CASADI_MATH
  auto linsol =
      casadi::Linsol("linear_solver", "symbolicqr", body_inertia.sparsity());
  RigidBodyDynamics::Math::Vector3d out =
      linsol.solve(body_inertia, angularMomentum);
#else
  RigidBodyDynamics::Math::Vector3d out =
      body_inertia.colPivHouseholderQr().solve(angularMomentum);
#endif
  ...
  for (unsigned int iAxes = 0; iAxes < 3; ++iAxes) {
    utils::Matrix3d bodyMatRot(
        RigidBodyDynamics::CalcBodyWorldOrientation(
            model, Q, static_cast<unsigned int>(segmentIdx), updateKin)
            .transpose());
    ...
    while (j != 0) {
...
#ifdef BIORBD_USE_CASADI_MATH
      if (model.S[j].is_zero() && model.S[j](4).is_zero() &&
          model.S[j](5).is_zero())
#else
...
#endif
      {
        ...
        X_base.r =
            utils::Vector3d(0, 0, 0);  // Remove all concept of translation
                                       // (only keep the rotation matrix)

        if (model.mJoints[j].mDoFCount == 3) {
          G.block(iAxes * 3, q_index, 3, 3) =
              ((point_trans * X_base.inverse()).toMatrix() *
               model.multdof3_S[j])
                  .block(3, 0, 3, 3);
        } else {
          G.block(iAxes * 3, q_index, 3, 1) =
              point_trans.apply(X_base.inverse().apply(model.S[j]))
                  .block(3, 0, 3, 1);
        }
      }
...
utils::Matrix rigidbody::Joints::JacobianSegmentRotMat(
    ...
    bool updateKin) {
  size_t segmentIdx =
      getBodyBiorbdIdToRbdlId(static_cast<int>(biorbdSegmentIdx));

  utils::Matrix jacobianMat(
      utils::Matrix::Zero(9, static_cast<unsigned int>(nbQ())));
  this->CalcMatRotJacobian(
      Q, segmentIdx, utils::Matrix3d::Identity(), jacobianMat, updateKin);
  return jacobianMat;
}
  ...
  if (Q) {
    utils::Error::check(
        ...
        "Wrong size for the Generalized Coordiates, " +
            utils::String("expected ") + std::to_string(nbQ()) + " got " +
  ...
  if (Qdot) {
    utils::Error::check(
        ...
        "Wrong size for the Generalized Velocities, " +
            utils::String("expected ") + std::to_string(nbQdot()) + " got " +
  ...
  if (Qddot) {
    utils::Error::check(
        ...
        "Wrong size for the Generalized Accelerations, " +
            utils::String("expected ") + std::to_string(nbQddot()) + " got " +
  ...
  if (torque) {
    utils::Error::check(
        ...
        "Wrong size for the Generalized Torques, " +
            utils::String("expected ") + std::to_string(nbGeneralizedTorque()) +
            " got " + std::to_string(torque->size()));
  }
#endif
}
