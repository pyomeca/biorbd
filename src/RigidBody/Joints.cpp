...
rigidbody::Joints::Joints() : RigidBodyDynamics::Model(),
...
rigidbody::Joints::Joints(const rigidbody::Joints &other) : RigidBodyDynamics::Model(other),
...
std::vector<std::vector<size_t>> rigidbody::Joints::getDofSubTrees()
{
    ...
    for (size_t i = 1; i < this->mu.size(); ++i)
    { // begin at 1 because 0 is its own parent in rbdl.
        if (this->lambda[i] == 0)
        {
    ...
    bool updateKin)
{
    ...
    for (i = static_cast<int>(updatedModel.mBodies.size() - 1); i > 0; i--)
    {
        ...
        if (lambda != 0)
        {
            ...
            RigidBodyDynamics::Math::SpatialMatrix Ia = updatedModel.IA[i] - updatedModel.U[i] * (updatedModel.U[i] / updatedModel.d[i]).transpose();

#ifdef BIORBD_USE_CASADI_MATH
            updatedModel.IA[lambda] += updatedModel.X_lambda[i].toMatrixTranspose() * Ia * updatedModel.X_lambda[i].toMatrix();

#else
            updatedModel.IA[lambda].noalias() += updatedModel.X_lambda[i].toMatrixTranspose() * Ia * updatedModel.X_lambda[i].toMatrix();
#endif
        }
    }
    // End Backward Pass
...
const std::vector<rigidbody::MeshFace> &rigidbody::Joints::meshFaces(size_t idx) const
{
    ...
    bool updateKin)
{
    ...
    rigidbody::Joints &
#endif
        updatedModel = this->UpdateKinematicsCustom(updateKin ? &Q : nullptr, updateKin ? &Qdot : nullptr, nullptr);
    ...
    for (size_t i = 1; i < updatedModel.mBodies.size(); i++)
    {
        ...
        utils::SpatialVector h = updatedModel.X_lambda[i].applyTranspose(
            updatedModel.hc[i]);
    ...
    const utils::Scalar &k_stab)
{
    ...
    for (size_t i = 0; i < nbSegment(); ++i)
    {
        ...
        if (segment_i.isRotationAQuaternion())
        {
            ...
            QdotOut.block(cmpDof, 0, static_cast<unsigned int>(segment_i.nbDofTrans()), 1) = Qdot.block(cmpDof, 0, static_cast<unsigned int>(segment_i.nbDofTrans()), 1);
