...
rigidbody::Segment::Segment() : utils::Node(),
...
rigidbody::Segment::Segment(
    ...
    const utils::SpatialTransform &cor) : utils::Node(name, parentName),
                                          m_idxInModel(std::make_shared<int>(-1)),
...
rigidbody::Segment::characteristics()
{
    ...
    const rigidbody::Joints &model) const
{
    ...
    for (int i = 0; i < static_cast<int>(model.nbSegment()); ++i)
    {
        ...
        if (!segment.name().compare(toCompare))
            return dofCount;
