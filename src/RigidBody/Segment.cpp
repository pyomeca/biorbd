...
rigidbody::Segment::Segment(
    ...
    : utils::Node(name, parentName),
      ...
      m_dofPosition(std::make_shared<std::vector<size_t>>()),
      m_characteristics(
          std::make_shared<rigidbody::SegmentCharacteristics>(characteristics)),
      m_dofCharacteristics(
          std::make_shared<std::vector<rigidbody::SegmentCharacteristics>>()) {
  setType();
...
rigidbody::Segment::Segment(
    ...
    const utils::String &seqR,  // Cardan sequence to classify the rotation DoF
    const std::vector<utils::Range> &QRanges,
    const std::vector<utils::Range> &QdotRanges,
    const std::vector<utils::Range> &QddotRanges,
    const std::vector<utils::Scalar> &jointDampings,
    const rigidbody::SegmentCharacteristics
        &characteristics,  // Mass, Center of mass of segment, Inertia of
                           // segment, etc.
    const utils::SpatialTransform &cor  //  Transformation from parent to child
    )
    : utils::Node(name, parentName),
      m_idxInModel(std::make_shared<int>(-1)),
      m_cor(std::make_shared<utils::SpatialTransform>(cor)),
...
void rigidbody::Segment::DeepCopy(const rigidbody::Segment &other) {
...
size_t rigidbody::Segment::nbGeneralizedTorque() const { return nbQddot(); }
size_t rigidbody::Segment::nbDof() const { return *m_nbDofTrueOutside; }
size_t rigidbody::Segment::nbDofTrans() const { return *m_nbDofTrans; }
size_t rigidbody::Segment::nbDofRot() const { return *m_nbDofRot; }
size_t rigidbody::Segment::nbQ() const { return *m_nbDofTrue; }
size_t rigidbody::Segment::nbQdot() const { return *m_nbQdot; }
size_t rigidbody::Segment::nbQddot() const { return *m_nbQddot; }

const utils::String &rigidbody::Segment::nameDof(const size_t i) const {
  // Return the number of DoF of the segment
  utils::Error::check(i < *m_nbDofTrue, "Dof ouside N dof for this segment");
  return (*m_nameDof)[i];
}

const utils::String &rigidbody::Segment::seqT() const { return *m_seqT; }

const utils::String &rigidbody::Segment::seqR() const { return *m_seqR; }

const std::vector<utils::Range> &rigidbody::Segment::QRanges() const {
  return *m_QRanges;
}

void rigidbody::Segment::setQRanges(const std::vector<utils::Range> &QRanges) {
  utils::Error::check(
      isVectorHasDofDimension(QRanges, true),
      "QRanges must be empty or of the same size as the number of dof");
  *m_QRanges = QRanges;
}
...
void rigidbody::Segment::setQdotRanges(
    const std::vector<utils::Range> &QdotRanges) {
  utils::Error::check(
      isVectorHasDofDimension(QdotRanges, false),
      "QdotRanges must be empty or of the same size as the number of dof");
...
void rigidbody::Segment::setQddotRanges(
    const std::vector<utils::Range> &QddotRanges) {
  utils::Error::check(
      isVectorHasDofDimension(QddotRanges, false),
      "QddotRanges must be empty or of the same size as the number of dof");
...
void rigidbody::Segment::setJointDampings(
    const std::vector<utils::Scalar> &jointDampings) {
  utils::Error::check(
      isVectorHasDofDimension(jointDampings, false),
      "jointDampings must be empty or of the same size as the number of dof");
...
utils::RotoTrans rigidbody::Segment::localJCS() const {
  return utils::SpatialTransform(
      m_cor->rotation().transpose(), m_cor->translation());
}
...
void rigidbody::Segment::setLocalJCS(
  ...
  *m_cor =
      utils::SpatialTransform(rototrans.rot().transpose(), rototrans.trans());
...
void rigidbody::Segment::updateCharacteristics(
  ...
  *m_characteristics = characteristics.DeepCopy();
  RigidBodyDynamics::Math::SpatialRigidBodyInertia rbi = RigidBodyDynamics::
...
rigidbody::SegmentCharacteristics &rigidbody::Segment::characteristics() {
  return *m_characteristics;
}

const rigidbody::SegmentCharacteristics &rigidbody::Segment::characteristics()
    const {
  return *m_characteristics;
}
...
void rigidbody::Segment::determineIfRotIsQuaternion(const utils::String &seqR) {
...
void rigidbody::Segment::str2numSequence(
  ...
  for (size_t i = 0; i < *m_nbDofQuat; ++i) {
    (*m_nameDof)[*m_nbDofTrans + *m_nbDofRot + i] =
        utils::String("Quat") + xyz.substr(i, 1);
  }
}
void rigidbody::Segment::str2numSequence(
...
void rigidbody::Segment::setNumberOfDof(size_t nTrans, size_t nRot) {
  *m_nbDofTrans = nTrans;
  if (*m_isQuaternion) {
    *m_nbDofRot = 0;
    *m_nbDofQuat = 4;
...
void rigidbody::Segment::setSequence(
    const utils::String &seqT,
    const utils::String
...
void rigidbody::Segment::fillSequence() {
  ...
  for (size_t i = 0; i < *m_nbDofTrans; i++) {
    (*m_dofPosition)[i] = (*m_sequenceTrans)[i];  // Place the translation first
  ...
  if (*m_isQuaternion) {
    ...
    for (size_t i = 0; i < *m_nbDofRot; i++) {
      (*m_dofPosition)[i + *m_nbDofTrans] =
          (*m_sequenceRot)[i];  // Place the rotation following the
                                // translations in the requested order
    }
}
  ...
  if (*m_nbDof != 0) {
    ...
    for (size_t i = 0; i < *m_nbDofTrans; i++) {
      (*m_dof)[i] = RigidBodyDynamics::Joint(
          RigidBodyDynamics::JointTypePrismatic, axis[(*m_dofPosition)[i]]);
    }

    // Declaration of the DoFs in rotation
    if (*m_isQuaternion) {
      (*m_dof)[*m_nbDofTrans] =
          RigidBodyDynamics::Joint(RigidBodyDynamics::JointTypeSpherical);
    } else {
      for (size_t i = *m_nbDofTrans; i < *m_nbDofRot + *m_nbDofTrans; i++) {
        (*m_dof)[i] = RigidBodyDynamics::Joint(
            RigidBodyDynamics::JointTypeRevolute, axis[(*m_dofPosition)[i]]);
      }
    }
  } else {
    m_dof->resize(1);
...
void rigidbody::Segment::setJoints(rigidbody::Joints &model) {
  setDofCharacteristicsOnLastBody();  // Apply the segment caracteristics only
                                      // to the last segment
  setJointAxis();  // Choose the axis order in relation to the selected
                   // sequence
  ...
  if (*m_nbDof == 0)
    (*m_idxDof)[0] = model.AddBody(
        parent_id, *m_cor, (*m_dof)[0], (*m_dofCharacteristics)[0], name());
  else if (*m_nbDof == 1)
    (*m_idxDof)[0] = model.AddBody(
        parent_id, *m_cor, (*m_dof)[0], (*m_dofCharacteristics)[0], name());
  else {
    (*m_idxDof)[0] = model.AddBody(
        parent_id, *m_cor, (*m_dof)[0], (*m_dofCharacteristics)[0]);
    for (size_t i = 1; i < *m_nbDof; i++)
      if (i != *m_nbDof - 1)
        (*m_idxDof)[i] = model.AddBody(
            static_cast<unsigned int>((*m_idxDof)[i - 1]),
            zero,
            (*m_dof)[i],
            (*m_dofCharacteristics)[i]);
      else
        (*m_idxDof)[i] = model.AddBody(
            static_cast<unsigned int>((*m_idxDof)[i - 1]),
            zero,
...
size_t rigidbody::Segment::getDofIdx(const utils::String &dofName) const {
  size_t idx(INT_MAX);
  bool found = false;
  ...
  utils::Error::check(
      found,
      "Type should be \"Rot\" or \"Trans\" and axis should be \"X\", \"Y\" "
      "or "
      "\"Z\", e.g. "
...
size_t rigidbody::Segment::getFirstDofIndexInGeneralizedCoordinates(
  ...
  // Start at root and descend the Generalized Coordinate vector until we get
...
size_t rigidbody::Segment::getLastDofIndexInGeneralizedCoordinates(
    const rigidbody::Joints &model) const {
  return getFirstDofIndexInGeneralizedCoordinates(model) +
         findFirstSegmentWithDof(model).nbDof() - 1;
}
