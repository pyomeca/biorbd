...
rigidbody::KalmanReconsMarkers rigidbody::KalmanReconsMarkers::DeepCopy()
    const {
...
void rigidbody::KalmanReconsMarkers::DeepCopy(
    const rigidbody::KalmanReconsMarkers &other) {
...
void rigidbody::KalmanReconsMarkers::manageOcclusionDuringIteration(
  ...
  for (size_t i = 0; i < occlusion.size(); ++i)
    for (size_t j = occlusion[i] * 3; j < occlusion[i] * 3 + 3; ++j) {
      InvTp(j, j) = 0;  // Artefact due to the fact that m_R has a value
                        // at (j:j+2,j:j+2)
      measure[j] = 0;
    }
}

bool rigidbody::KalmanReconsMarkers::first() { return *m_firstIteration; }
...
void rigidbody::KalmanReconsMarkers::reconstructFrame(
  ...
  if (*m_firstIteration) {
    *m_firstIteration = false;
    ...
    TobsTP.block(
        3 * model.nbTechnicalMarkers(0),
        0,
        3 * model.nbTechnicalMarkers() - 3 * model.nbTechnicalMarkers(0),
        1) =
        utils::Vector::Zero(
    ...
    for (size_t j = 0; j < 2;
      ...
      for (size_t i = 0; i < 50; ++i) {
        // The first time, call in a recursive manner to get a descent
        // initial position
        reconstructFrame(model, TobsTP, nullptr, nullptr, nullptr);

        // Reset Pp to initial (we are not interested in the velocity to
        // get to the initial position)
        *m_Pp = *m_PpInitial;
        m_xp->block(*m_nbDof, 0, *m_nbDof * 2, 1) = utils::Vector::Zero(
  ...
  Model &updatedModel =
      static_cast<Model &>(model.UpdateKinematicsCustom(&Q_tp));

  // Projected markers
  const std::vector<rigidbody::NodeSegment> &zest_tp(
      updatedModel.technicalMarkers(Q_tp, removeAxes, false));
  // Jacobian
  const std::vector<utils::Matrix> &J_tp(
      updatedModel.technicalMarkersJacobian(Q_tp, removeAxes, false));
  ...
  // If there is a marker
        ...
        !Tobs(i * 3).is_zero() && !Tobs(i * 3 + 1).is_zero() &&
        !Tobs(i * 3 + 2).is_zero()
#else
        Tobs(i * 3) * Tobs(i * 3) + Tobs(i * 3 + 1) * Tobs(i * 3 + 1) +
                Tobs(i * 3 + 2) * Tobs(i * 3 + 2) !=
            0.0 &&
        !isnan(
            Tobs(i * 3) * Tobs(i * 3) + Tobs(i * 3 + 1) * Tobs(i * 3 + 1) +
            Tobs(i * 3 + 2) * Tobs(i * 3 + 2))
