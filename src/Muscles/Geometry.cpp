...
muscles::Geometry muscles::Geometry::DeepCopy() const
{
    muscles::Geometry copy;
    copy.DeepCopy(*this);
    return copy;
}

void muscles::Geometry::DeepCopy(const muscles::Geometry &other)
{
...
void muscles::Geometry::setOrigin(
...
const utils::Vector3d& muscles::Geometry::originInLocal() const
{
    return *m_origin;
}

void muscles::Geometry::setInsertionInLocal(
    const utils::Vector3d &position)
{
    if (dynamic_cast<const rigidbody::NodeSegment*>(&position)) {
...
const utils::Vector3d &muscles::Geometry::insertionInLocal()
const
{
    return *m_insertion;
}
...
const utils::Vector3d &muscles::Geometry::originInGlobal() const
{
...
&muscles::Geometry::musclesPointsInGlobal() const
{
    utils::Error::check(*m_isGeometryComputed,
...
const utils::Scalar& muscles::Geometry::length() const
{
    utils::Error::check(*m_isGeometryComputed,
...
const utils::Scalar& muscles::Geometry::velocity() const
{
...
const utils::Matrix &muscles::Geometry::jacobianLength() const
{
    utils::Error::check(*m_isGeometryComputed,
...
void muscles::Geometry::_updateKinematics(
    const rigidbody::GeneralizedVelocity* Qdot,
    const muscles::Characteristics* characteristics,
    muscles::PathModifiers *pathModifiers)
{
    ...
    muscles::PathModifiers *pathModifiers)
{
    ...
    if (pathModifiers->nbWraps()!=0) {
        ...
        muscles::WrappingObject& w =
            static_cast<muscles::WrappingObject&>(pathModifiers->object(0));
        ...
        for (unsigned int i=0; i<pathModifiers->nbObjects(); ++i) {
            const muscles::ViaPoint& node(static_cast<muscles::ViaPoint&>
...
const utils::Scalar& muscles::Geometry::length(
    const muscles::Characteristics *characteristics,
    muscles::PathModifiers *pathModifiers)
{
    *m_muscleTendonLength = 0;
    ...
    if (pathModifiers != nullptr && pathModifiers->nbWraps()!=0) {
        ...
        static_cast<muscles::WrappingObject&>(
...
const utils::Scalar& muscles::Geometry::velocity(
    const rigidbody::GeneralizedVelocity &Qdot)
{
...
void muscles::Geometry::jacobian(const utils::Matrix &jaco)
{
...
void muscles::Geometry::computeJacobianLength()
{
