...
void Writer::writeModel(Model& model, const utils::Path& pathToWrite) {
  ...
  for (size_t i = 0; i < model.nbSegment(); ++i) {
    biorbdModelFile << comment << " Informations about "
                    << model.segment(i).name() << " segment" << std::endl;
    biorbdModelFile << sep << comment << " Segment" << std::endl;
    biorbdModelFile << sep << "segment" << sep << model.segment(i).name()
                    << std::endl;
    biorbdModelFile << sep << sep << "parent" << sep
                    << model.segment(i).parent() << std::endl;
    biorbdModelFile << sep << sep << "RTinMatrix" << sep << true << std::endl;
    biorbdModelFile << sep << sep << "RT" << std::endl;
    for (size_t j = 0; j < 4; ++j) {
      biorbdModelFile << sep << sep << sep << localJCS[i].block(j, 0, 1, 4)
                      << std::endl;
    }
    if (model.segment(i).nbDofTrans() > 0) {
      biorbdModelFile << sep << sep << "translations" << sep
                      << model.segment(i).seqT() << std::endl;
    }
    ...
    biorbdModelFile << std::endl;
    biorbdModelFile << sep << sep << "mass" << sep
                    << model.segment(i).characteristics().mass() << std::endl;
    biorbdModelFile << sep << sep << "inertia" << std::endl
                    << sep << sep << sep
                    << model.segment(i).characteristics().inertia()
                    << std::endl;
    biorbdModelFile
        << sep << sep << "com" << sep
        << model.segment(i).characteristics().mCenterOfMass.transpose()
        << std::endl;
    if (model.segment(i).characteristics().mesh().path().filename().compare(
            "")) {
      biorbdModelFile
          << sep << sep << "meshfile" << sep
          << model.segment(i).characteristics().mesh().path().originalPath()
          << std::endl;
      biorbdModelFile
          << sep << sep << "meshcolor" << sep
          << model.segment(i).characteristics().mesh().color().transpose()
          << std::endl;
      biorbdModelFile
          << sep << sep << "meshscale" << sep
          << model.segment(i).characteristics().mesh().getScale().transpose()
          << std::endl;
      biorbdModelFile
          << sep << sep << "meshrt" << sep
          << utils::RotoTrans::toEulerAngles(
                 model.segment(i).characteristics().mesh().getRotation(),
                 utils::String("xyz"))
                 .transpose()
          << " xyz "
          << model.segment(i)
    ...
    std::vector<rigidbody::NodeSegment> markers(
        model.markers(model.segment(i).name()));
    if (markers.size() > 0) {
      ...
      for (size_t j = 0; j < markers.size(); ++j) {
        ...
        biorbdModelFile << sep << sep << "parent" << sep << markers[j].parent()
                        << std::endl;
        biorbdModelFile << sep << sep << "position" << sep
                        << markers[j].transpose() << std::endl;
        biorbdModelFile << sep << sep << "technical" << sep
                        << markers[j].isTechnical() << std::endl;
        ...
        if (markers[j].nbAxesToRemove() != 0) {
          biorbdModelFile << sep << sep << "axestoremove" << sep
                          << markers[j].axesToRemoveAsString() << std::endl;
        }
    ...
    if (imus.size() > 0) {
      biorbdModelFile << sep << comment << " Inertial Magnetic Unit"
                      << std::endl;
      for (size_t j = 0; j < imus.size(); ++j) {
        ...
        biorbdModelFile << sep << sep << "parent" << sep << imus[j].parent()
                        << std::endl;
        biorbdModelFile << sep << sep << "RTinMatrix" << sep << true
                        << std::endl;
        biorbdModelFile << sep << sep << "RT" << std::endl;
        for (size_t k = 0; k < 4; ++k) {
          biorbdModelFile << sep << sep << sep << imus[j].block(k, 0, 1, 4)
                          << std::endl;
        }
        biorbdModelFile << sep << sep << "technical" << sep
                        << imus[j].isTechnical() << std::endl;
        biorbdModelFile << sep << sep << "anatomical" << sep
                        << imus[j].isAnatomical() << std::endl;
    ...
    if (rts.size() > 0) {
      ...
      for (size_t j = 0; j < rts.size(); ++j) {
        ...
        biorbdModelFile << sep << sep << "RT" << std::endl;
        for (size_t k = 0; k < 4; ++k) {
          biorbdModelFile << sep << sep << sep << rts[j].block(k, 0, 1, 4)
                          << std::endl;
        }
