...
void biorbd::Writer::writeModel(biorbd::Model & model,
    ...
    biorbdModelFile << "root_actuated" << sep
                    << model.isRootActuated() << std::endl;
    biorbdModelFile << "external_forces" << sep
                    << model.hasExternalForces() << std::endl;
    biorbdModelFile << std::endl;
    ...
    for (unsigned int i = 0; i < model.nbBone(); ++i) {
        biorbdModelFile << com << " Informations about " << model.bone(i).name()
                        << " segment" << std::endl;
        biorbdModelFile << sep << com << " Segment" << std::endl;
        biorbdModelFile << sep << "segment" << sep << model.bone(i).name()
                        << std::endl;
        biorbdModelFile << sep << sep << "parent" << sep
                        << model.bone(i).parent() << std::endl;
        biorbdModelFile << sep << sep << "RTinMatrix" << sep
                        << true << std::endl;
        biorbdModelFile << sep << sep << "RT" << std::endl;
        for (unsigned int j = 0; j < 4; ++j) {
            biorbdModelFile << sep << sep << sep
                            << localJCS[i].block(j, 0, 1, 4) << std::endl;
        }
        if (model.bone(i).nDofTrans() > 0) {
            biorbdModelFile << sep << sep << "translations" << sep
                            << model.bone(i).seqT() << std::endl;
        }
        if (model.bone(i).nDofRot() > 0) {
            biorbdModelFile << sep << sep << "rotations" << sep
                            << model.bone(i).seqR() << std::endl;
        }
        if (model.bone(i).caract().mesh().path().filename().compare("")) {
            biorbdModelFile << sep << sep << "meshfile" << sep
                            << model.bone(i).caract().mesh().path()
        ...
        std::vector<biorbd::rigidbody::NodeBone> markers(
                    model.marker(model.bone(i).name()));
        if (markers.size() > 0) {
            biorbdModelFile << sep << com << " Markers" << std::endl;
            for (size_t j = 0; j< markers.size(); ++j) {
                biorbdModelFile << sep << "marker" << sep
                                << markers[j].name() << std::endl;
                biorbdModelFile << sep << sep << "parent"
                                << sep << markers[j].parent() << std::endl;
                biorbdModelFile << sep << sep << "position"
                                << sep << markers[j].transpose() << std::endl;
                biorbdModelFile << sep << sep << "technical"
                                << sep << markers[j].isTechnical() << std::endl;
                biorbdModelFile << sep << sep << "anatomical"
                                << sep << markers[j].isAnatomical()
                                << std::endl;
        ...
        std::vector<biorbd::rigidbody::IMU> imus(
                    model.IMU(model.bone(i).name()));
        if (imus.size() > 0) {
            biorbdModelFile << sep << com
                            << " Inertial Magnetic Unit" << std::endl;
            for (size_t j = 0; j< imus.size(); ++j) {
                biorbdModelFile << sep << "imu" << sep
                                << imus[j].name() << std::endl;
                biorbdModelFile << sep << sep << "parent"
                                << sep << imus[j].parent() << std::endl;
                biorbdModelFile << sep << sep << "RTinMatrix"
                                << sep << true << std::endl;
                biorbdModelFile << sep << sep << "RT" << std::endl;
                for (unsigned int k = 0; k < 4; ++k) {
                    biorbdModelFile << sep << sep << sep
                                    << imus[j].block(k, 0, 1, 4) << std::endl;
                }
