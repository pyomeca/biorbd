...
void S2M_segmentInertia_global( int nlhs, mxArray *plhs[],
    ...
    double *Inertia_global = mxGetPr(plhs[0]);


    /* Préparer la sortie conditionnelle (tous si idx==-1, seulement celui demandé sinon) */
    unsigned int cmp(0);
    for (unsigned int i=0; i<nFrame; ++i){
        std::vector<RBDM::Vector3d> am_all(model->CalcSegmentsInertia_global (*model, *(Q.begin()+i), *(QDot.begin()+i), true));
        if (idx==-1){
            // Remplir le output
            std::vector<RBDM::Vector3d>::iterator it=am_all.begin();
            for (unsigned int i=0; (it+i)!=am_all.end(); ++i){
                Inertia_global[cmp*3  ] = (*(it+i))(0);
                Inertia_global[cmp*3+1] = (*(it+i))(1);
                Inertia_global[cmp*3+2] = (*(it+i))(2);
                cmp++;
            }
        }
        else{
            // Remplir le output
            std::vector<RBDM::Vector3d>::iterator it=am_all.begin();
            Inertia_global[i*3+0] = (*(it+idx))(0);
            Inertia_global[i*3+1] = (*(it+idx))(1);
            Inertia_global[i*3+2] = (*(it+idx))(2);
