...
def complex_model_from_scratch(remove_temporary: bool = True):
    """
    We define a new model by feeding in the actual dimension and position of the model.
    ...
    mesh_path = "../test/models meshFiles/stl/pendulum.STL"
    ...
    bio_model.segments["PENDULUM"] = Segment(
        ...
        mesh_file=MeshFile(mesh_file_name=mesh_path,
                            ...
                            mesh_scale=lambda m: np.array([1, 1, 10]),
                            mesh_rotation=lambda m: np.array([np.pi/2, 0, 0]),
                            mesh_translation=lambda m: np.array([0.1, 0, 0])),
    )
    # The pendulum segment contact point
    bio_model.segments["PENDULUM_CONTACT"].add_contact(Contact(name="PENDULUM_CONTACT",
    ...
    bio_model.muscles["PENDULUM_MUSCLE"] = Muscle("PENDULUM_MUSCLE",
                                                ...
                                                optimal_length_function=lambda m: 0.1,
                                                maximal_force_function=lambda m: 100,
                                                tendon_slack_length_function=lambda m: 0.05,
                                                pennation_angle_function=lambda m: 0.05,
    ...
    bio_model.via_points["PENDULUM_MUSCLE"] = ViaPoint("PENDULUM_MUSCLE",
                                                        ...
                                                        muscle_group_name="PENDULUM_MUSCLE_GROUP",
                                                        )


    # Put the model together, print it and print it to a bioMod file
    bio_model.write(kinematic_model_file_path, {"fake_maker_set_unused":, np.array([0, 0, 0])})
