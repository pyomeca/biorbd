...
void Matlab_IMU(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  // Verifier les arguments d'entrée
  checkNombreInputParametres(
      nrhs,
      3,
      4,
      "3 arguments are required [+1 optional] where the 2nd is the handler on "
      "the model, 3rd is the Q and 4th is the wanted IMUType to be return "
      "(['all'], 'technical' or anatomical')");
  // Recevoir le model
  ...
  // Récupérer les IMU selon que l'on veut tous ou seulement anatomiques ou
  // techniques
  unsigned int nIMUs(0);  // Nombre de IMU
  std::vector<std::vector<BIORBD_NAMESPACE::rigidbody::IMU>>
      IMU_tp;  // récupérer les IMU
  if (nrhs == 4) {
    BIORBD_NAMESPACE::utils::String type(getString(prhs, 3));
    if (!type.tolower().compare("all")) {
      nIMUs = model->nbIMUs();
      for (std::vector<BIORBD_NAMESPACE::rigidbody::GeneralizedCoordinates>::
               iterator Q_it = Q.begin();
           Q_it != Q.end();
           ++Q_it) {
  ...
  for (std::vector<std::vector<BIORBD_NAMESPACE::rigidbody::IMU>>::iterator
           AllIMU_it = IMU_tp.begin();
       AllIMU_it != IMU_tp.end();
       ++AllIMU_it)
    for (std::vector<BIORBD_NAMESPACE::rigidbody::IMU>::iterator IMU_it =
             (*AllIMU_it).begin();
         IMU_it != (*AllIMU_it).end();
         ++IMU_it)
