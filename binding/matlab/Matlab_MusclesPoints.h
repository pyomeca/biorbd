...
void Matlab_MusclesPoints(
  ...
  checkNombreInputParametres(
      ...
      3,
      3,
      "3 arguments are required where the 2nd is the handler on the model and "
      "3rd is the Q");
  // Recevoir le model
  ...
  std::vector<BIORBD_NAMESPACE::utils::NODE_TYPE>
      wrap_type;  // forme du wrapping
  std::vector<BIORBD_NAMESPACE::utils::RotoTrans>
      wrap_RT;                    // orientation du wrapping
  ...
  for (unsigned int iQ = 0; iQ < Q.size(); ++iQ)
    for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i)
      for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
        ...
        std::vector<BIORBD_NAMESPACE::utils::Vector3d> via(
            model->muscleGroup(i).muscle(j).musclesPointsInGlobal());

        // Si le nombre de wrap est > 0, c'est qu'il n'y a pas de viapoints et
        // il n'y a qu'UN wrap
        if (model->muscleGroup(i).muscle(j).pathModifier().nbWraps() > 0) {
          const BIORBD_NAMESPACE::internal_forces::WrappingObject
              &wrappingObject(
                  dynamic_cast<const BIORBD_NAMESPACE::internal_forces::
                                   WrappingObject &>(
                      model->muscleGroup(0).muscle(0).pathModifier().object(
                          0)));

          // De quel type
          BIORBD_NAMESPACE::utils::NODE_TYPE type(wrappingObject.typeOfNode());
          ...
          if (type ==
              BIORBD_NAMESPACE::utils::NODE_TYPE::WRAPPING_HALF_CYLINDER) {
            const BIORBD_NAMESPACE::internal_forces::WrappingHalfCylinder
                &cylinder(
                    dynamic_cast<const BIORBD_NAMESPACE::internal_forces::
                                     WrappingHalfCylinder &>(wrappingObject));
  ...
  if (nlhs >= 2) {  // Sortir la déclaration des wrapping s'il y a lieu
    ...
    for (unsigned int i = 0; i < wrap_RT.size(); ++i) {
      ...
      mxArray *tp_forme = mxCreateString(
          BIORBD_NAMESPACE::utils::NODE_TYPE_toStr(wrap_type[i]));
      ...
      mxSetCell(
          plhs[1], (nWrap * 4) * cmpTime + (cmpWrap + 0 * nWrap), tp_forme);
      mxSetCell(plhs[1], (nWrap * 4) * cmpTime + (cmpWrap + 1 * nWrap), tp_RT);
      mxSetCell(
          plhs[1], (nWrap * 4) * cmpTime + (cmpWrap + 2 * nWrap), tp_dim1);
      mxSetCell(
          plhs[1], (nWrap * 4) * cmpTime + (cmpWrap + 3 * nWrap), tp_dim2);
