                           ...
                           int nrhs, const mxArray*prhs[] )
{
    ...
    std::vector<BIORBD_NAMESPACE::utils::NODE_TYPE> wrap_type; // forme du wrapping
    std::vector<BIORBD_NAMESPACE::utils::RotoTrans> wrap_RT; // orientation du wrapping
    ...
    for (unsigned int iQ=0; iQ<Q.size(); ++iQ)
        for (unsigned int i=0; i<model->nbMuscleGroups(); ++i)
            for (unsigned int j=0; j<model->muscleGroup(i).nbMuscles(); ++j) {
                ...
                std::vector<BIORBD_NAMESPACE::utils::Vector3d> via(model->muscleGroup(i).muscle(
    ...
    if (nlhs >= 2) { // Sortir la déclaration des wrapping s'il y a lieu
        ...
        for (unsigned int i=0; i<wrap_RT.size(); ++i) {
            ...
            mxArray *tp_forme = mxCreateString( BIORBD_NAMESPACE::utils::NODE_TYPE_toStr(
                                                    wrap_type[i]) );
