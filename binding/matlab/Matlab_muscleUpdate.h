...
void Matlab_muscleUpdate(int, mxArray *[], int nrhs, const mxArray *prhs[]) {
  // Verifier les arguments d'entrée
  // checkNombreInputParametres(nrhs, 6, 7, "6 arguments are required [+1
  // optional] where the 2nd is the handler on the model, 3rd is the Q, 4th is
  // QDot, 5th is all muscles points (origin, via points, insertion), 6th is the
  // muscle point Jacobian and optional 7th is a 1xN matrix depicting the number
  // of point for each muscle (default is the one given in the model)");
  checkNombreInputParametres(
      nrhs,
      6,
      6,
      "6 arguments are required where the 2nd is the handler on the model, 3rd "
      "is the Q, 4th is QDot, 5th is all muscles points (origin, via points, "
      "insertion), 6th is the muscle point Jacobian");
  ...
  std::vector<BIORBD_NAMESPACE::rigidbody::GeneralizedCoordinates> Q =
      getParameterQ(prhs, 2, nQ);
  // Recevoir Qdot
  std::vector<BIORBD_NAMESPACE::rigidbody::GeneralizedVelocity> QDot =
      getParameterQdot(prhs, 3, nQdot);

  // S'assurer qu'il n'y a qu'un seul frame
  unsigned int nFrame(static_cast<unsigned int>(Q.size()));
  if (nFrame != 1) {
    mexErrMsgIdAndTxt(
        "MATLAB:dim:WrongDimension", "Q must be exactly composed of 1 frame");
  }
  if (QDot.size() != nFrame) {
    mexErrMsgIdAndTxt(
        "MATLAB:dim:WrongDimension",
        "QDot must have the same number of frames than Q");
  }

  // Si on n'envoie pas le nombre de via par muscles, assumer que c'est eux du
  // modèle
  ...
  int cmpMus(0);
  for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i) {
    BIORBD_NAMESPACE::internal_forces::muscles::MuscleGroup grMus(
        model->muscleGroup(i));
  ...
  std::vector<std::vector<BIORBD_NAMESPACE::utils::Vector3d>> musclePosition(
      getMusclePosition(prhs, 4, nPoints));

  // Recueillir la matrice jacobienne
  std::vector<BIORBD_NAMESPACE::utils::Matrix> musclePointsJaco(
      getMusclePointsJaco(prhs, 5, nPoints, nQ));
