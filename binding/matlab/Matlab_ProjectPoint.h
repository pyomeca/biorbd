                          ...
                          int nrhs, const mxArray*prhs[] )
{
    ...
    // Recevoir Q
    std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::GeneralizedCoordinates> Qall = getParameterQ(
                prhs, 2, nQ);
    ...
    for (unsigned int i=0; i<nFrames; ++i) {
        biorbd::BIORBD_MATH_NAMESPACE::rigidbody::GeneralizedCoordinates Q(*(Qall.begin()+i));
        std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment> projectedPoint(model->projectPoint(
                    Q, *(markersOverTime.begin()+i), true));
        for (unsigned int j=0; j<static_cast<unsigned int>(nMarker); ++j) {
            biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment tp(*(projectedPoint.begin()+j));
            Markers[cmp+0] = tp(0);
            Markers[cmp+1] = tp(1);
            Markers[cmp+2] = tp(2);
            cmp += 3;
