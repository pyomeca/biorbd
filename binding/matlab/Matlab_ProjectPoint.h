...
void Matlab_ProjectPoint(
  ...
  checkNombreInputParametres(
      ...
      "is the Q, 4th are the 3xNxT points in global reference frame where N = "
      "nMarkers of the model");
  // Recevoir le model
  ...
  for (unsigned int i = 0; i < nFrames; ++i) {
    ...
    std::vector<BIORBD_NAMESPACE::rigidbody::NodeSegment> projectedPoint(
        model->projectPoint(Q, *(markersOverTime.begin() + i), true));
    for (unsigned int j = 0; j < static_cast<unsigned int>(nMarker); ++j) {
      BIORBD_NAMESPACE::rigidbody::NodeSegment tp(
          *(projectedPoint.begin() + j));
      Markers[cmp + 0] = tp(0);
      Markers[cmp + 1] = tp(1);
      Markers[cmp + 2] = tp(2);
      cmp += 3;
