...
void Matlab_segmentCOMddot( int, mxArray *plhs[],
    ...
    if (i==-1){
        std::vector<biorbd::utils::Vector3d> COMddot = model->CoMddotBySegment(Q,QDot,QDDot,true);
        ...
        biorbd::utils::Vector3d COMddot = model->CoMddotBySegment(Q,QDot,QDDot,static_cast<unsigned int>(i),true);
