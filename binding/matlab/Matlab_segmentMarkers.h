                            ...
                            int nrhs, const mxArray*prhs[] )
{
    ...
    if ( idx==-1) { // Si on a demande tous les segments
        ...
        std::vector<std::vector<BIORBD_NAMESPACE::rigidbody::NodeSegment>> allMarkers;
        for (unsigned int i=0; i<model->nbSegment(); ++i)    {
            std::vector<BIORBD_NAMESPACE::rigidbody::NodeSegment> markers_tp = model->segmentMarkers(
                        Q, i, removeAxes);
