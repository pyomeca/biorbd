...
void Matlab_segmentMarkers(
  ...
  checkNombreInputParametres(
      ...
      "the model, 3rd is the Q, 4th if you want to remove axes as specified in "
      "the model file [default = true] and 5th optional is a specific segment "
      "index");
  // Recevoir le model
  ...
  if (idx == -1) {  // Si on a demande tous les segments
    ...
    for (unsigned int i = 0; i < model->nbSegment(); ++i) {
      std::vector<BIORBD_NAMESPACE::rigidbody::NodeSegment> markers_tp =
          model->segmentMarkers(Q, i, removeAxes);
    ...
    for (unsigned int i_bone = 0; i_bone < allMarkers.size(); ++i_bone) {
      mxArray *markers_out_tp = mxCreateDoubleMatrix(
          3, (*(allMarkers.begin() + i_bone)).size(), mxREAL);
      ...
      for (unsigned int i = 0;
           (it + i) != (*(allMarkers.begin() + i_bone)).end();
           ++i) {
        markers[i * 3] = (*(it + i))(0);
