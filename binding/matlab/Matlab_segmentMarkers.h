                            ...
                            int nrhs, const mxArray*prhs[] )
{
    ...
    if ( idx==-1) { // Si on a demande tous les segments
        ...
        std::vector<std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment>> allMarkers;
        for (unsigned int i=0; i<model->nbSegment(); ++i)    {
            std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment> markers_tp = model->segmentMarkers(
                        Q, i, removeAxes);
        ...
        for (unsigned int i_bone=0; i_bone<allMarkers.size(); ++i_bone) {
            ...
            // Remplir le output
            std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment>::iterator it=(*
        ...
        std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment> markers_tp =
