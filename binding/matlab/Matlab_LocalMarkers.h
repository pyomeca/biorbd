                          ...
                          int nrhs, const mxArray*prhs[] )
{
    ...
    // Gestion du type
    biorbd::BIORBD_MATH_NAMESPACE::utils::String type("all");
    if (nrhs >= 3) {
    ...
    std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment>
    markers_tp; // récupérer les marqueurs
    if (!type.tolower().compare("all")) {
        nMarkers = model->nbMarkers();
        markers_tp = model->biorbd::BIORBD_MATH_NAMESPACE::rigidbody::Markers::markers(removeAxes);
    } else if (!type.tolower().compare("anatomical")) {
        nMarkers = model->nbAnatomicalMarkers();
        markers_tp = model->biorbd::BIORBD_MATH_NAMESPACE::rigidbody::Markers::anatomicalMarkers(removeAxes);
    } else if (!type.tolower().compare("technical")) {
        nMarkers = model->nbTechnicalMarkers();
        markers_tp = model->biorbd::BIORBD_MATH_NAMESPACE::rigidbody::Markers::technicalMarkers(removeAxes);
    } else {
    ...
    // Remplir le output
    unsigned int cmp(0);
    std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::NodeSegment>::iterator it=markers_tp.begin();
