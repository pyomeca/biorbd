...
void Matlab_segmentsInertia(
  ...
  const mwSize dims[3] = {
      6, 6, static_cast<mwSize>(nQ + 1)};  // model->nbBone()};
...
void Matlab_segmentsInertiaLocal(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      2,
      3,
      "2 arguments are required (+1 optional) where the 2nd is the handler on "
      "the model and optional third is the segment index");
  ...
  if (nrhs > 2) {
    ...
    Eigen::Matrix3d Im =
        model->segment(idx - 1)
            .characteristics()
            .inertia();  // Mettre les masses dans la variable de sortie
    unsigned int cmp(0);
