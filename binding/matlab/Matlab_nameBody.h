...
void Matlab_nameBody(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs > 2) {
    ...
    plhs[0] = mxCreateString(
        model->segment(idx - 1).name().c_str());  // Recueillir le nom
  } else {
    // Sortie des noms
    ...
    for (unsigned int i = 0; i < model->nbSegment(); ++i) {
      mxArray *nomBone = mxCreateString(
          model->segment(i).name().c_str());  // Recueillir le nom
      mxSetCell(
          plhs[0], i, nomBone);  // Mettre les noms dans la variable de sortie
    }
  }
  return;
}
