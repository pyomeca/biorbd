...
void Matlab_nameDof(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  for (unsigned int i = 0; i < model->nbSegment(); ++i) {
    for (unsigned int j = 0; j < model->segment(i).nbDof(); ++j) {
      mxArray *nomDof = mxCreateString(
          (model->segment(i).name() + "_" + model->segment(i).nameDof(j))
              .c_str());  // Recueillir le nom
      mxSetCell(
          plhs[0], cmp, nomDof);  // Mettre les noms dans la variable de sortie
      ++cmp;
    }
  }
