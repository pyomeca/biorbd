...
void Matlab_Mesh(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      "the model, 3rd is the Q and 4th optional is a specific segment index");
  // Recevoir le model
  ...
  if (idx == -1) {  // Si on a demande tous les segments
    ...
    std::vector<std::vector<BIORBD_NAMESPACE::utils::Vector3d>> allMesh(
        model->meshPoints(Q));
    ...
    for (unsigned int i_bone = 0; i_bone < allMesh.size(); ++i_bone) {
      mxArray *mesh_out_tp =
          mxCreateDoubleMatrix(3, (*(allMesh.begin() + i_bone)).size(), mxREAL);
    ...
    std::vector<BIORBD_NAMESPACE::utils::Vector3d> Mesh_tp(
        model->meshPoints(Q, static_cast<size_t>(idx)));
