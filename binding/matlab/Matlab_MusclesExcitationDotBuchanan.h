...
void Matlab_MusclesExcitationDotBuchanan(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      "the model, 3rd is the excitation, 4th is the activation and 5th is a "
      "bool to express if excitation is already normalized");
  // Recevoir le model
  ...
  std::vector<std::vector<
      ...
      state =
          getParameterMuscleStateBuchanan(prhs, 2, 3, model->nbMuscleTotal());
  ...
  for (unsigned int iTime = 0; iTime < state.size(); ++iTime) {
    ...
    for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i)
      for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
        ...
        double shapeFactor =
            dynamic_cast<const BIORBD_NAMESPACE::internal_forces::muscles::
                             StateDynamicsBuchanan &>(
                model->muscleGroup(i).muscle(j).state())
                .shapeFactor();
        (*((*(state.begin() + iTime)).begin() + cmpState))
            .shapeFactor(shapeFactor);
        edot[cmp] =
            (*((*(state.begin() + iTime)).begin() + cmpState))
                .timeDerivativeExcitation(
                    model->muscleGroup(i).muscle(j).characteristics(), true);
        updateKin = false;
        ++cmp;
        ++cmpState;
      }
