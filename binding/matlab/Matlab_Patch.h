...
void Matlab_Patch(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      "the model and the optional 4th is a specific segment index");
  // Recevoir le model
  ...
  if (idx == -1) {  // Si on a demande tous les segments
    ...
    std::vector<BIORBD_NAMESPACE::rigidbody::MeshFace> Mesh_tp(
        model->meshFaces(static_cast<unsigned int>(idx)));
    ...
    // Remplir le output
    std::vector<BIORBD_NAMESPACE::rigidbody::MeshFace>::iterator it =
        Mesh_tp.begin();
