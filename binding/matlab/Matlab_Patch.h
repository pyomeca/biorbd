                   ...
                   int nrhs, const mxArray*prhs[] )
{
    ...
    if ( idx==-1) { // Si on a demande tous les segments
        ...
        std::vector<std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::MeshFace>> allMesh(
                    model->meshFaces());
        ...
        for (unsigned int i_bone=0; i_bone<allMesh.size(); ++i_bone) {
            ...
            for (unsigned int i=0; allMesh[i_bone].size(); ++i) {
                Mesh[i*3] = allMesh[i_bone][i](0)
                            +1; // +1 Car l'indice dans biorbd::BIORBD_MATH_NAMESPACE::rigidbody::s est par rapport à 0
        ...
        std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::MeshFace> Mesh_tp(model->meshFaces(
        ...
        // Remplir le output
        std::vector<biorbd::BIORBD_MATH_NAMESPACE::rigidbody::MeshFace>::iterator it=Mesh_tp.begin();
