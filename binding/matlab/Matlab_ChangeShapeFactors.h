...
void Matlab_ChangeShapeFactors(
    int,
    mxArray *[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      3,
      3,
      "3 arguments are required where the 2nd is the handler on the model, 3rd "
      "is the shape factor for each muscle");
  // Recevoir le model
  ...
  if (shapeFactors.size() != model->nbMuscleTotal()) {
    ...
    msg << "Length of vector of shape factors should match the number of "
           "muscles ("
        << model->nbMuscleTotal() << ")";
