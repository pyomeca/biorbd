...
void Matlab_CoMddot( int nlhs, mxArray *plhs[],
    ...
    checkNombreInputParametres(nrhs, 5, 5, "4 arguments are required where the 2nd is the handler on the model, 3rd is the Q and 4th is QDot");
    ...
    // Recevoir Qddot
    std::vector<biorbd::rigidbody::GeneralizedAcceleration> QDDot = getParameterQddot(prhs, 4, nQddot);
    ...
    for (unsigned int i=0; i<Q.size(); ++i){
        RigidBodyDynamics::Math::Vector3d COMDDot = model->CoMddot(Q[i], QDot[i], QDDot[i]);
