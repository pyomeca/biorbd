...
void Matlab_MusclesForceMax(
  ...
  checkNombreInputParametres(
      ...
      "the model. If 2 arguments were sent, the function returns values for "
      "each muscles, if a 3rd is sent, the max forces are replace by the sent "
      "values.");
  // Recevoir le model
  ...
  for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i)
    for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
      if (nrhs == 2) {  // Recueillir toutes les forces max
        muscleForceMax[cmp] =
            model->muscleGroup(i).muscle(j).characteristics().forceIsoMax();
