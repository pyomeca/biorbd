...
void Matlab_muscleJointTorqueFromActivation(
    int nlhs,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      3,
      5,
      "3 arguments are required (+2 optional) where the 2nd is the handler on "
      "the model, "
      "3rd is the muscles states and optional 4th and 5th are the Q and QDot, "
      "respectively."
      "WARNING: if the function is called without Q and Qdot, the user MUST "
      "update by himself "
  ...
  std::vector<std::vector<
      ...
      s = getParameterMuscleStateActivation(prhs, 2, nMuscleTotal);
  unsigned int nFrame(static_cast<unsigned int>(s.size()));

  bool updateKin(false);  // Par défaut c'est false, mais ce comportement est
                          // celui le moins attendu
  if (nrhs >= 4) {        // Si on doit récupérer la cinématique
    if (nrhs != 5) {
      mexErrMsgIdAndTxt(
          "MATLAB:dim:WrongArguments",
  ...
  if (updateKin) {  // Si on update pas la cinématique Q et Qdot ne sont pas
                    // nécessaire
    // Recevoir Q
