...
void Matlab_Markers(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      "the model, 3rd is the Q and 4th is the wanted markerType to be return "
      "('all', 'technical' or anatomical') and 5th if you want to remove axes "
  ...
  // Remplir le output
  unsigned int cmp(0);
  for (std::vector<std::vector<BIORBD_NAMESPACE::rigidbody::NodeSegment>>::
           iterator markers_it = markers_tp.begin();
       markers_it != markers_tp.end();
       ++markers_it) {
