...
void Matlab_nameMarkers(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  for (unsigned int i_bone = 0; i_bone < allMarkers.size(); ++i_bone) {
    mxArray *markers_out_tp =
        mxCreateString((*(allMarkers.begin() + i_bone)).c_str());
...
void Matlab_nameTechnicalMarkers(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  // Trouver ou sont les marqueurs
  std::vector<BIORBD_NAMESPACE::utils::String> allMarkers(
      model->technicalMarkerNames());
...
void Matlab_nameAnatomicalMarkers(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
