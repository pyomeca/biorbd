...
void Matlab_segmentMass(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs > 2) {
    ...
    mass[0] = model->segment(idx - 1)
                  .characteristics()
                  .mass();  // Mettre les masses dans la variable de sortie
  } else {
    // Sortie des noms
