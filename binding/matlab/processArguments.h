...
std::vector<std::vector<std::shared_ptr<biorbd::muscles::State>>> getParameterMuscleState(
        const mxArray*prhs[],
        int idxExcitation,
        int idxActivation,
        unsigned int nMus){
    ...
    for (unsigned int j=0; j<nFramesTotal; ++j){
        ...
        for (unsigned int i=0; i<nMus; i++)
            if (isThereExcitation && isThereActivation){
                States_tp.push_back( std::make_shared<biorbd::muscles::State>(stateExcitation[j*nMus+i], stateActivation[j*nMus+i]));
            }
            else if (!isThereExcitation && isThereActivation){
                States_tp.push_back( std::make_shared<biorbd::muscles::State>(0, stateActivation[j*nMus+i]));
            }
            else if (isThereExcitation && !isThereActivation){
                States_tp.push_back( std::make_shared<biorbd::muscles::State>(stateExcitation[j*nMus+i], 0));
            }
...
std::vector<std::vector<std::shared_ptr<biorbd::muscles::State>>> getParameterMuscleStateActivation(const mxArray*prhs[], int idxActivation, unsigned int nMus){
    return getParameterMuscleState(prhs, -1, idxActivation, nMus);
}
std::vector<std::vector<std::shared_ptr<biorbd::muscles::State>>> getParameterMuscleStateExcitation(const mxArray*prhs[], int idxExcitation, unsigned int nMus){
