...
void checkNombreInputParametres(
    int nrhs,
    int min,
    int max,
    std::string message = "") {
  if (nrhs < min) {
    mexErrMsgIdAndTxt("MATLAB:yprime:invalidNumInputs", message.c_str());
  } else if (nrhs > max) {
    mexErrMsgIdAndTxt("MATLAB:yprime:invalidNumInputs", message.c_str());
  }
}
...
getParameterAllMarkers(
  ...
  if (nsubs < 2 || nsubs > 3) {
    std::ostringstream msg;
    msg << "Wrong size! Input markers matrix should be a 3-dimensions or "
           "2-dimensions matrix";
    mexErrMsgTxt(msg.str().c_str());
  }
  ...
  if (static_cast<int>(n / nFrames) !=
      nMark) {  // puisque les dimensions supplémentaires sont ajoutées a la
                // fin, il faut / par nFrames
    std::ostringstream msg;
    msg << "Wrong size! Input markers matrix should should have the same "
           "number of technical markers ("
        << nMark << " markers.";
    mexErrMsgTxt(msg.str().c_str());
  }
  ...
  for (unsigned int i = 0; i < nFrames; ++i) {
    std::vector<BIORBD_NAMESPACE::rigidbody::NodeSegment>
        markers_tp;  // Markers a un temps i

    for (int j = 0; j < nMark; ++j) {
      BIORBD_NAMESPACE::rigidbody::NodeSegment tp(
          markers[m * cmp + 0],  // m est 3 ou 4
          markers[m * cmp + 1],
          markers[m * cmp + 2]);
...
std::vector<std::vector<BIORBD_NAMESPACE::rigidbody::IMU>> getParameterAllIMUs(
  ...
  if (nsubs < 2 || nsubs > 4) {
    std::ostringstream msg;
    msg << "Wrong size! Input IMUs matrix should be a 3-dimensions or "
           "2-dimensions matrix";
    mexErrMsgTxt(msg.str().c_str());
  }
  // Get number of IMUs
  ...
  for (unsigned int i = 0; i < nFrames; ++i) {
    ...
    for (unsigned int j = 0; j < nIMUs; ++j) {
      ...
      if (n == 4) {
        rot << imus[i * 16 * nIMUs + j * 16 + 0],
            imus[i * 16 * nIMUs + j * 16 + 4],
            imus[i * 16 * nIMUs + j * 16 + 8],
            imus[i * 16 * nIMUs + j * 16 + 1],
            imus[i * 16 * nIMUs + j * 16 + 5],
            imus[i * 16 * nIMUs + j * 16 + 9],
            imus[i * 16 * nIMUs + j * 16 + 2],
            imus[i * 16 * nIMUs + j * 16 + 6],
            imus[i * 16 * nIMUs + j * 16 + 10];
        trans << imus[i * 16 * nIMUs + j * 16 + 12],
            imus[i * 16 * nIMUs + j * 16 + 13],
            imus[i * 16 * nIMUs + j * 16 + 14];
      } else {
        rot << imus[i * 9 * nIMUs + j * 9 + 0], imus[i * 9 * nIMUs + j * 9 + 3],
            imus[i * 9 * nIMUs + j * 9 + 6], imus[i * 9 * nIMUs + j * 9 + 1],
            imus[i * 9 * nIMUs + j * 9 + 4], imus[i * 9 * nIMUs + j * 9 + 7],
            imus[i * 9 * nIMUs + j * 9 + 2], imus[i * 9 * nIMUs + j * 9 + 5],
            imus[i * 9 * nIMUs + j * 9 + 8];
        trans.setZero();
      }
...
getVector(const mxArray *prhs[], unsigned int idx, std::string type = "") {
  // Check data type of input argument
  if (!(mxIsDouble(prhs[idx]))) {
    std::ostringstream msg;
    msg << "Argument " << idx + 1 << " must be of type double.";
  ...
  mwSize length(0);
  if (m != 1 && n == 1) {  // on permet le nombre juste ou juste une valeur qui
                           // sera dupliquée
    length = m;
  } else if (n != 1 && m == 1) {
    length = n;
  ...
  BIORBD_NAMESPACE::rigidbody::GeneralizedCoordinates vect(
...
BIORBD_NAMESPACE::rigidbody::GeneralizedCoordinates getVector(
    const mxArray *prhs[],
    unsigned int idx,
    unsigned int length,
    std::string type = "") {
  mwSize m = mxGetM(prhs[idx]);  // line
  // Get the number of elements in the input argument
  if (!(m == length || m == 1)) {  // on permet le nombre juste ou juste une
                                   // valeur qui sera dupliquée
    std::ostringstream msg;
...
std::vector<BIORBD_NAMESPACE::rigidbody::GeneralizedAcceleration>
getParameterQddot(
    const mxArray *prhs[],
    unsigned int idx,
    unsigned int nbQddot,
...
std::vector<BIORBD_NAMESPACE::rigidbody::GeneralizedTorque>
getParameterGeneralizedTorque(
    const mxArray *prhs[],
    unsigned int idx,
    unsigned int nControl) {
  std::vector<BIORBD_NAMESPACE::rigidbody::GeneralizedCoordinates>
      AllGeneralizedTorque_tp =
          getParameterQ(prhs, idx, nControl, "GeneralizedTorque");
  std::vector<BIORBD_NAMESPACE::rigidbody::GeneralizedTorque>
      AllGeneralizedTorque;

  for (unsigned int j = 0; j < AllGeneralizedTorque_tp.size(); ++j) {
    BIORBD_NAMESPACE::rigidbody::GeneralizedTorque GeneralizedTorque_tp(
        nControl);
...
std::vector<std::vector<BIORBD_NAMESPACE::utils::SpatialVector>> getForcePlate(
  ...
  if (!(mxIsDouble(prhs[idx]))) {
    mexErrMsgIdAndTxt(
        "MATLAB:findnz:invalidInputType", "Argument 6 must be of type double.");
  }
  ...
  if (mPF * nPF ==
      mxGetNumberOfElements(prhs[idx])) {  // s'il n'y a pas de dimension temps
    timeStamp = 1;
  } else {
  ...
  for (unsigned int j = 0; j < timeStamp; ++j) {
    ...
    for (unsigned int i = 0; i < nPF; ++i) {  // pour chaque plateforme
      BIORBD_NAMESPACE::utils::SpatialVector tp(
          pf[0 + cmp * 6],
          pf[1 + cmp * 6],
          pf[2 + cmp * 6],
          pf[3 + cmp * 6],
...
double
getDouble(const mxArray *prhs[], unsigned int idx, std::string type = "d") {
  // Check data type of input argument
  if (!(mxIsDouble(prhs[idx]))) {
    std::ostringstream msg;
    msg << "Argument " << idx + 1 << " must be of type double.";
...
BIORBD_NAMESPACE::utils::String getString(
    const mxArray *prhs[],
    unsigned int idx) {
  // Check data type of input argument
...
std::vector<std::vector<
    std::shared_ptr<BIORBD_NAMESPACE::internal_forces::muscles::State>>>
...
std::vector<std::vector<
    BIORBD_NAMESPACE::internal_forces::muscles::StateDynamicsBuchanan>>
getParameterMuscleStateBuchanan(
    const mxArray *prhs[],
    int idxExcitation,
    int idxActivation,
    unsigned int nMus) {
...
std::vector<Eigen::VectorXd> getParameterMuscleForceNorm(
    const mxArray *prhs[],
    unsigned int idx,
    unsigned int nMus,
...
std::vector<std::vector<BIORBD_NAMESPACE::utils::Vector3d>> getMusclePosition(
  ...
  if (nRows < 3 || nRows > 4) {
    std::ostringstream msg;
    msg << "Muscle position should be a column-wise matrix with 3 or 4 rows "
           "(where the 4th is a line of 1)";
    mexErrMsgTxt(msg.str().c_str());
  }
  ...
  if (nCol != nVia) {  // on permet le nombre juste ou juste une valeur qui sera
                       // dupliquée
    std::ostringstream msg;
    msg << "muscle position should be a column-wise matrix with columns = "
        << nVia;
    mexErrMsgTxt(msg.str().c_str());
  }
...
std::vector<BIORBD_NAMESPACE::utils::Matrix> getMusclePointsJaco(
    const mxArray *prhs[],
    unsigned int idx,
    Eigen::VectorXd nPointsByMuscles,
    unsigned int nQ) {
  ...
  for (unsigned int i = 0; i < nPointsByMuscles.rows(); ++i) {
    // Préparer les matrices intermédiaires (chaque muscle)
    BIORBD_NAMESPACE::utils::Matrix mus(
        static_cast<unsigned int>(nPointsByMuscles(i)) * 3, nQ);
    for (unsigned int j = 0; j < static_cast<unsigned int>(nPointsByMuscles(i));
         ++j) {
      // Stocker
