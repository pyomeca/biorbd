...
void Matlab_globalJCS(int, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  for (std::vector<
           BIORBD_NAMESPACE::rigidbody::GeneralizedCoordinates>::iterator Q_it =
           Q.begin();
       Q_it != Q.end();
       ++Q_it) {
  ...
  const mwSize dims[4] = {
      4, 4, mwSize(model->nbSegment()), mwSize(JSC_vec.size())};
  plhs[0] = mxCreateNumericArray(4, dims, mxDOUBLE_CLASS, mxREAL);
  double *JCS = mxGetPr(plhs[0]);

  // Remplir l'output
  unsigned int cmpJCS = 0;
  for (std::vector<std::vector<BIORBD_NAMESPACE::utils::RotoTrans>>::iterator
           AllJCS_it = JSC_vec.begin();
       AllJCS_it != JSC_vec.end();
       ++AllJCS_it)
    for (std::vector<BIORBD_NAMESPACE::utils::RotoTrans>::iterator JSC_it =
             (*AllJCS_it).begin();
         JSC_it != (*AllJCS_it).end();
         ++JSC_it)
