...
void Matlab_setEKF_IMU(
  ...
  checkNombreInputParametres(
      ...
      2,
      5,
      "2 arguments are required (+3 optional) where the 2nd is the handler on "
      "the model, the optional 3th is acquisition frequency [default 100Hz], "
      "4th is noise factor [default 5e-3] and 5th is error factor [default "
      "1e-10]");
...
void Matlab_delEKF_IMU(int nlhs, mxArray *[], int nrhs, const mxArray *prhs[]) {
  // Verifier les arguments d'entrée
  checkNombreInputParametres(
      nrhs,
      2,
...
void Matlab_inverseKinematicsEKF_IMUstep(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      4,
      5,
      "4 arguments are required (+1 optional) where the 2nd is the handler on "
      "the model, 3rd is the handler on kalman filter info, 4th is the 3x3xNxT "
      "or 4x4xNxT IMU hypermatrix, the optional 5th is the initial guess for Q "
      "[default 0], 5th is acquisition frequency [default 100Hz]");
...
void Matlab_inverseKinematicsEKF_IMUallInOneCall(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  checkNombreInputParametres(
      ...
      "the model, 3th is the 3x3xNxT or 4x4xNxT IMU hypermatrix, the optional "
      "4th is the initial guess for Q [default 0], 5th is acquisition "
      "frequency [default 100Hz], 6th is noise factor [default 5e-3] and 7th "
      "is error factor [default 1e-10]");
  ...
  for (unsigned int i = 0; i < nFrames; ++i) {
    ...
    kalman.reconstructFrame(
        *model, *(imusOverTime.begin() + i), &Q, &QDot, &QDDot);
