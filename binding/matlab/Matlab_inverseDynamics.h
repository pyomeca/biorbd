...
void Matlab_inverseDynamics(
  ...
  checkNombreInputParametres(
      ...
      "model, 3rd is the Q, 4th is QDot and 5th is QDDot and the optional 6th "
      "is the forces in global frame");
  ...
  std::vector<std::vector<BIORBD_NAMESPACE::utils::SpatialVector>> f_tp;
  if (externalForces) {
  ...
  BIORBD_NAMESPACE::rigidbody::GeneralizedTorque Tau(nTau);
  for (unsigned int j = 0; j < Q.size(); ++j) {
    Tau.setZero();
    if (externalForces) {
      mexErrMsgTxt(
          "External forces was was not implemented in Matlab after the "
          "introduction of ExternalForceSet");
      ...
      //     *model, Q[j], QDot[j], QDDot[j], Tau, f_ext_rbdl);// Inverse
      //     Dynamics
      // delete f_ext_rbdl;
    } else {
      RigidBodyDynamics::InverseDynamics(
