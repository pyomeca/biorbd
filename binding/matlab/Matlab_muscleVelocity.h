...
void Matlab_muscleVelocity(
  ...
  if (Q.size() != Qdot.size()) {
    mexErrMsgIdAndTxt(
        "MATLAB:findnz:time", "Time stamps for Q and Qdot don't correspond");
  }
  ...
  for (unsigned int iQ = 0; iQ < Q.size(); ++iQ)
    for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i)
      for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
        ...
        length[cmp] = model->muscleGroup(i).muscle(j).velocity(
            *model, Q[iQ], Qdot[iQ], true);
        ++cmp;
      }
