...
void Matlab_MusclesParentNames(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i) {
    ...
    for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
      ...
      mxArray *viaNames = mxCreateCellMatrix(
          model->muscleGroup(i).muscle(j).pathModifier().nbObjects() + 2,
          1);  // +2 pour origine et insertion

      mxArray *nameOrigin = mxCreateString(model->muscleGroup(i)
                                               .muscle(j)
                                               .position()
                                               .originInLocal()
                                               .parent()
      ...
      mxArray *nameInsertion = mxCreateString(model->muscleGroup(i)
                                                  .muscle(j)
                                                  .position()
                                                  .insertionInLocal()
                                                  .parent()
