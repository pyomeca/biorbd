    ...
    for (unsigned int i=0; i<model->nbMuscleGroups(); ++i) {
        ...
        for (unsigned int j=0; j<model->muscleGroup(i).nbMuscles(); ++j) {
            ...
            mxArray *viaNames = mxCreateCellMatrix(
                model->muscleGroup(i).muscle(j).pathModifier().nbObjects()+2, 1);// +2 pour origine et insertion
            ...
            for (unsigned int k=0; k<model->muscleGroup(i).muscle(j).pathModifier().nbObjects(); ++k) {
                mxArray * viaName = mxCreateString(
                    model->muscleGroup(i).muscle(j).pathModifier().object(k).parent().c_str()); // Recueillir le nom du muscle
                mxSetCell(viaNames, k+1, viaName); // Les mettres une cellule (+1 pour skipper origin)
            }
            mxArray * nameInsertion = mxCreateString(
                model->muscleGroup(i).muscle(j).position().insertionInLocal().parent().c_str()); // Insertion
            mxSetCell(viaNames, model->muscleGroup(i).muscle(j).pathModifier().nbObjects() +1, nameInsertion);
