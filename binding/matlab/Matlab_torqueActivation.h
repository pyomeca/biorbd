...
void Matlab_torqueActivation( int, mxArray *plhs[],
    ...
    checkNombreInputParametres(nrhs, 5, 5, "5 or 5 arguments are required where the 2nd is the handler on the model, 3rd is the Q, 4th is QDot and 5th is GeneralizedTorque");

    // Recevoir le model
    biorbd::Model * model = convertMat2Ptr<biorbd::Model>(prhs[1]);
    unsigned int nQ = model->nbQ(); // Get the number of DoF
    unsigned int nQdot = model->nbQdot(); // Get the number of DoF
    unsigned int nGeneralizedTorque = model->nbGeneralizedTorque(); // Nombre de GeneralizedTorque
    ...
    // Recevoir Qddot
    std::vector<biorbd::rigidbody::GeneralizedTorque> act = getParameterGeneralizedTorque(prhs, 4, model->nbGeneralizedTorque());
