  ...
  for (unsigned int iQ = 0; iQ < Q.size(); ++iQ) {
    ...
    for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i)
      for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
        ...
        length[cmp] =
            model->muscleGroup(i).muscle(j).length(*model, Q[iQ], updateKin);
        updateKin = 1;
        ++cmp;
      }
  }
...
void Matlab_MusclesTendonLength(
    int,
    mxArray *plhs[],
    int nrhs,
    const mxArray *prhs[]) {
  ...
  for (unsigned int iQ = 0; iQ < Q.size(); ++iQ) {
    ...
    for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i)
      for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
        // Recueillir toutes les longueurs musculaire
        length[cmp] = model->muscleGroup(i).muscle(j).musculoTendonLength(
