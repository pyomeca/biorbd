#ifndef BIORBD_MATLAB_MUSCLES_NAMES_H
#define BIORBD_MATLAB_MUSCLES_NAMES_H
...
void Matlab_MusclesNames(
  ...
  // Sortie des noms
  plhs[0] = mxCreateCellMatrix(
      model->nbMuscleGroups(), 1);  // Stockage des noms de groupe
  plhs[1] = mxCreateCellMatrix(
      model->nbMuscleGroups(), 1);  // Stockage des noms de muscles
  plhs[2] = mxCreateCellMatrix(
      model->nbMuscleGroups(), 1);  // Stockage des noms de viapoints

  // Stocker chaque valeur
  // Pour chaque groupe musculaire
  for (unsigned int i = 0; i < model->nbMuscleGroups(); ++i) {
    // Créer une matrice de cellule pour recueillir les noms (a stocker dans la
    // plus grosse matrice)
    mxArray *muscleNames =
        mxCreateCellMatrix(model->muscleGroup(i).nbMuscles(), 1);
    // Créer une matrice de cellule pour recueillir les noms (a stocker dans la
    // plus grosse matrice)
    mxArray *viaNamesByMuscles =
        mxCreateCellMatrix(model->muscleGroup(i).nbMuscles(), 1);

    // Pour chaque muscle
    for (unsigned int j = 0; j < model->muscleGroup(i).nbMuscles(); ++j) {
      mxArray *name = mxCreateString(
          model->muscleGroup(i).muscle(j).name().c_str());  // Recueillir le nom
                                                            // du muscle
      ...
      for (unsigned int k = 0;
           k < model->muscleGroup(i).muscle(j).pathModifier().nbObjects();
           ++k) {
        mxArray *viaName =
            mxCreateString(model->muscleGroup(i)
                               .muscle(j)
                               .pathModifier()
                               .object(k)
                               .name()
                               .c_str());  // Recueillir le nom du muscle
    ...
    mxArray *groupNames = mxCreateString(
        model->muscleGroup(i).name().c_str());  // Recueillir le nom du groupe
...
#endif  // BIORBD_MATLAB_MUSCLES_NAMES_H
