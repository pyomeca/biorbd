...
///
/// \brief Class A state of fatigue that dynamically evolves over the usage of
/// the muscle
///
class BIORBD_API FatigueDynamicState : public FatigueState {
 public:
  ...
  ///
  FatigueDynamicState(const std::shared_ptr<FatigueState> other);

  ///
  /// \brief Deep copy of a fatigue dynamic state
  /// \param other The other fatigue state
  ///
  void DeepCopy(const FatigueDynamicState& other);
  ...
  virtual void timeDerivativeState(
      ...
      const Characteristics& characteristics) = 0;
