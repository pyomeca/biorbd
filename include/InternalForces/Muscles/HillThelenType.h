...
/// giving a = 3 / 11;
/// giving b = 3 * this->characteristics().maxShorteningSpeed() / 11; with
/// maxShorteningSpeed being normalized (i.e. 1) And finally rearranged so F is
...
class BIORBD_API HillThelenType : public HillType {
 public:
  ...
  ///
  HillThelenType(const Muscle& other);

  ///
  /// \brief Construct a Hill-Thelen-type muscle from another muscle
  /// \param other The other muscle (pointer)
  ///
  HillThelenType(const std::shared_ptr<Muscle> other);
