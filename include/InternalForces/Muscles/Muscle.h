...
class BIORBD_API Muscle : public Compound {
 ...
 public:
  ...
  /// \param other The other muscle
  ///
  Muscle(const std::shared_ptr<Muscle> other);
  ...
  ///
  void DeepCopy(const Muscle& other);
  ...
  ///
  /// \brief Set the position of all the points attached to the muscle (0 being
  /// the origin)
  /// \param positions New value of the position
  ///
  void setPosition(const internal_forces::muscles::MuscleGeometry& positions);

  ///
  /// \brief Return the position of all the points attached to the muscle (0
  /// being the origin)
  ...
  ///
  void setCharacteristics(const Characteristics& characteristics);
  ...
  /// \param updatedModel The updated joint model (The user must ensure that the
  /// model is updated to the proper level)
  /// \param Q The generalized coordinates
  /// \param updateMuscleParameters Update the kinematic related parameters of
  /// the muscles
  ...
  ///
  /// \brief Return the previously computed muscle points in global reference
  /// frame
  /// \return The muscle points in global reference frame
  ///
  ...
  ///
  void setForceIsoMax(const utils::Scalar& forceMax);

  ///
  /// \brief Set the dynamic state
  /// \param emg The dynamic state value
  ///
  void setState(const State& emg);
  ...
  ///
  virtual const utils::Scalar& force(const State& emg) = 0;
 ...
 protected:
  ...
  /// \param emg EMG data
  ///
  virtual void computeForce(const State& emg);
  ...
  ///
  virtual utils::Scalar getForceFromActivation(const State& emg) = 0;
