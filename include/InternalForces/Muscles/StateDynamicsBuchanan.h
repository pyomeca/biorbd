...
namespace muscles {
///
/// \brief Time derivative of activation as described by Buchanan
/// (https://www.sciencedirect.com/science/article/pii/S0021929003001520)
///
class BIORBD_API StateDynamicsBuchanan : public StateDynamics {
 public:
  ...
  ///
  /// \brief Construct a state dynamics Buchanan from another state dynamics
  /// Buchanan
  /// \param other The other state dynamics Buchanan
  ///
  StateDynamicsBuchanan(const State& other);
  ...
  ///
  /// \brief Deep copy of state dynamics Buchanan into another state dynamics
  /// Buchanan
  /// \param other The state dynamics Buchanan to copy
  ///
  void DeepCopy(const StateDynamicsBuchanan& other);
  ...
  ///
  virtual void setNeuralCommand(const utils::Scalar& val);

  ///
  /// \brief Set the shape factor
  /// \param shape_factor Value of the shape factor
  ///
  void shapeFactor(const utils::Scalar& shape_factor);
  ...
  /// \param notUsed the activation is computed from the neuralCommand and
  /// excitation
  ///
 ...
 protected:
  ...
  std::shared_ptr<utils::Scalar> m_excitationDot;  ///< The excitation velocity
};
