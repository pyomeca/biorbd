...
class BIORBD_API Muscles {
 public:
  ...
  ///
  void DeepCopy(const Muscles& other);
  ...
  ///
  int getMuscleGroupId(const utils::String& name) const;
  ...
  /// \return The muscle
  ///
  Muscle& muscle(size_t idx) const;
  ...
  ///
  MuscleGroup& muscleGroup(size_t idx);
  ...
  /// \return A muscle group
  ///
  const MuscleGroup& muscleGroup(size_t idx) const;
  ...
  /// \return A muscle group
  ///
  const MuscleGroup& muscleGroup(const utils::String& name) const;
  ...
  ///
  /// \brief Update by hand all the muscles (positions, jacobian, velocity,
  /// etc.)
  ...
  /// i.e. \f$-J \times F\f$
  ///
  /// where \f$J\f$ is the muscle lengths jacobian and \f$F\f$ is the force
  /// vector of all the muscles
  ///
  /// Warning: This function assumes that muscles are already updated (via
  /// `updateMuscles`)
  ///
  rigidbody::GeneralizedTorque muscularJointTorque(const utils::Vector& F);
  ...
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  /// \param updateKin Update kinematics (0: don't update, 1:only muscles, [2:
  /// both kinematics and muscles])
  ///
  /// This function updates the muscles and then performs the computation for
  /// the muscular joint torque is done from virtual power:
  ///
  ...
  ///
  /// This functions converts muscle activations into muscle forces and then
  /// performs the computation for the muscular joint torque is done from
  /// virtual power:
  ///
  /// i.e. \f$-J \times F\f$
  ///
  ...
  /// `updateMuscles`)
  ///
  utils::Vector muscleForces(const std::vector<std::shared_ptr<State>>& emg);
