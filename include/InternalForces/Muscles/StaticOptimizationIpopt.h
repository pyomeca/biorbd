...
class BIORBD_API StaticOptimizationIpopt : public Ipopt::TNLP {
 public:
  ...
  /// \param m Number of constraints
  /// \param init_lambda If lambda parameters are initialized. That variable
  /// must be false
 ...
 protected:
  ...
  std::shared_ptr<unsigned int>
      m_nbQ;  ///< The number of generalized coordinates
  std::shared_ptr<unsigned int>
  ...
  std::shared_ptr<utils::Vector> m_torqueResidual;  ///< The torque residual
