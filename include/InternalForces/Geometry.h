...
class BIORBD_API Geometry {
 public:
  ...
  ///
  Geometry(const utils::Vector3d& origin, const utils::Vector3d& insertion);
  ...
  ///
  void DeepCopy(const Geometry& other);
  ...
  /// \param Q The generalized coordinates of the joints (not needed if
  /// updateKin is less than 2)
  /// \param Qdot The generalized velocities of the joints (not needed if
  /// updateKin is less than 2)
  ///
  /// updateKinematics MUST be called before retreiving data that are dependent
  /// on Q and/or Qdot
  ///
  /// WARNING If one used updateKinematics providing the jacobian, using
  /// updateKinematics overrides this previously sent position and jacobian
  ///
  ...
  ///
  void setOrigin(const utils::Vector3d& position);
  ...
  ///
  void setInsertionInLocal(const utils::Vector3d& position);
  ...
  ///
  /// Geometry (updateKin) must be computed at least once before calling. Return
  /// already computed via points
  ///
  ...
  ///
  utils::Matrix jacobian(size_t idxViaPoint) const;
 ...
 protected:
  ...
  ///
  /// \brief Updates the kinematics and return the position of the insertion
  /// node
  /// \param model The joint model
  /// \param Q The generalized coordinates of the model
  /// \return The origin position in the global reference
  ...
  ///
  void setPointsInGlobal(std::vector<utils::Vector3d>& ptsInGlobal);
  ...
  ///
  /// \brief Update the kinematics, compute and return the muscle velocity
  /// assuming not via points nor wrapping objects
  /// \param Qdot The generalized velocities
  /// \return The muscle velocity
  ///
  const utils::Scalar& velocity(const rigidbody::GeneralizedVelocity& Qdot);

  ///
  /// \brief Set the jacobian dimensions
  /// \param model The joint model
  ///
  void setJacobianDimension(rigidbody::Joints& model);

  ///
  /// \brief Force a jacobian computed from the user
  /// \param jaco The jacobian
  ///
  void jacobian(const utils::Matrix& jaco);
  ...
  std::shared_ptr<utils::Vector3d>
      m_insertionInGlobal;  ///< Position of the insertion node in the global
                            ///< reference
  ...
  std::shared_ptr<bool> m_isGeometryComputed;  ///< To know if the geometry was
                                               ///< computed at least once
  std::shared_ptr<bool> m_isVelocityComputed;  ///< To know if the velocity was
                                               ///< computed in the last update
  std::shared_ptr<bool>
      m_posAndJacoWereForced;  ///< To know if the override was used on the
                               ///< muscle position and the Jacobian
};
