...
///
/// \brief Class LigamentSpringLinear is a ligament with same behavior than
/// linear spring defined by: k(l-l0) with k is the stiffness
class BIORBD_API LigamentSpringLinear : public Ligament {
 public:
  ...
  ///
  LigamentSpringLinear(const Ligament& other);

  ///
  /// \brief Construct a linear spring like ligament from another ligament
  /// \param other THe other ligament
  ///
  LigamentSpringLinear(const std::shared_ptr<Ligament> other);
  ...
  ///
  /// \brief Deep copy of a linear spring like ligament in a new linear spring
  /// like ligament
  /// \param other The linear spring like ligament to copy
  ///
  void DeepCopy(const LigamentSpringLinear& other);
