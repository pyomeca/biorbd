...
///
/// \brief Class LigamentSpringSecondOrder is a ligament with same behavior than
/// second order spring defined by: (k/2) * ((l-l0) + sqrt((l-l0)*(l-l0) +
/// epsilon*epsilon)) with k is the stiffness and espilon a small parameter to
/// avoid negative values. https://ieeexplore.ieee.org/abstract/document/6755458
///
...
class BIORBD_API LigamentSpringSecondOrder : public Ligament {
 public:
  ...
  ///
  LigamentSpringSecondOrder(const Ligament& other);

  ///
  /// \brief Construct a second order spring like ligament from another ligament
  /// \param other THe other ligament
  ///
  LigamentSpringSecondOrder(const std::shared_ptr<Ligament> other);
  ...
  ///
  /// \brief Deep copy of a second order spring like ligament in a new second
  /// order spring like ligament
  /// \param other The second order spring like ligament to copy
  ///
  void DeepCopy(const LigamentSpringSecondOrder& other);
  ...
  ///
  void setEpsilon(const utils::Scalar& epsilon);
 ...
 protected:
  ...
  std::shared_ptr<utils::Scalar>
      m_epsilon;  ///< small parameter to avoid negative value
};
