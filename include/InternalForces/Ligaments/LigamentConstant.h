...
class BIORBD_API LigamentConstant : public Ligament {
 public:
  ...
  ///
  LigamentConstant(const Ligament& other);

  ///
  /// \brief Construct a ligament constant  from another ligament
  /// \param other THe other ligament
  ///
  LigamentConstant(const std::shared_ptr<Ligament> other);
  ...
  ///
  void DeepCopy(const LigamentConstant& other);
 ...
 protected:
  ...
  std::shared_ptr<utils::Scalar>
      m_force;  ///< Force of the ligament applied when l>l0
};
