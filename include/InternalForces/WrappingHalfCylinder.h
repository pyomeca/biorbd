...
///
/// \brief Half cylinder of infinite length (the length only affect the
/// graphical representation) object that makes the muscle to wrap around
///
/// Watch out, due to high computational cost, the CasADi backend always assume
/// contact between the muscle and cylinder. This will create discrepancies
/// between the Eigen and the CasADi backend
///
class BIORBD_API WrappingHalfCylinder : public WrappingObject {
 public:
  ...
  /// \brief Construct a wrapping half cylinder
  ///
  WrappingHalfCylinder(const utils::Vector3d& other);
  ...
  ///
  /// \brief Deep copy of the wrapping half cylinder in another wrapping half
  /// cylinder
  /// \param other The wrapping half cylinder to copy
  ///
  void DeepCopy(const WrappingHalfCylinder& other);

  ///
  /// \brief From the position of the half cylinder, return the 2 locations
  ...
  ///
  void setRadius(const utils::Scalar& val);
  ...
  ///
  void setLength(const utils::Scalar& val);
 ...
 protected:
  ...
  class NodeMusclePair {
   public:
    ...
    /// \param p2 Point 2
    ///
    NodeMusclePair(const utils::Vector3d& p1, const utils::Vector3d& p2)
  ...
  ///
  void findTangentToCircle(const utils::Vector3d& p, utils::Vector3d& p_tan)
      const;
  ...
  ///
  void selectTangents(const NodeMusclePair& p, utils::Vector3d& p_tan) const;

  ///
  /// \brief Find the height of both points
  /// \param pointsInGlobal The position of the muscle pair in global reference
  /// frame
  ...
  ///
  utils::Scalar computeLength(const NodeMusclePair& p) const;

  std::shared_ptr<utils::Scalar>
      m_radius;  ///< Diameter of the half cylinder diametre du cylindre
  std::shared_ptr<utils::Scalar> m_length;  ///< Length of the half cylinder
  ...
  std::shared_ptr<utils::Scalar>
      m_lengthAroundWrap;  ///< Length between p1 and p2
};
