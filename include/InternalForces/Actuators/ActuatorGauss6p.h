...
namespace actuator {
///
/// \brief Class ActuatorGauss6p is a joint actuator type which maximum is
/// bimodal 6 parameter gaussian (Gauss6p) Please note that all parameters are
/// given in degrees
///
class BIORBD_API ActuatorGauss6p : public Actuator {
 public:
  ...
  ///
  ActuatorGauss6p(const ActuatorGauss6p& other);
  ...
  /// \param wc Angluar velocity of the vertical asymptote of the concentric
  /// hyperbola
  ...
  ///
  void DeepCopy(const ActuatorGauss6p& other);
 ...
 protected:
  ...
  // For informations on these parameters, see Monique Iris Jackson's these from
  // page 54 Angular/velocity relationship
  ...
  std::shared_ptr<utils::Scalar> m_T0;    ///< Maximum torque isometric
  std::shared_ptr<utils::Scalar> m_wmax;  ///< Maximum angular velocity above
                                          ///< which torque cannot be produced
  std::shared_ptr<utils::Scalar>
      m_wc;  ///< Angular velocity of the vertical asymptote of the concentric
             ///< hyperbola
  ...
  std::shared_ptr<utils::Scalar> m_wr;    ///< 1/10 of the distance amax/amin
  std::shared_ptr<utils::Scalar> m_w1;    ///< Mid point plateau

  // Torque/angle relationship
  std::shared_ptr<utils::Scalar> m_r;     ///< width of the 1st gaussian curve
  std::shared_ptr<utils::Scalar> m_qopt;  ///< 1st Optimal position
  std::shared_ptr<utils::Scalar> m_facteur;  ///< Factor of the 6p
  std::shared_ptr<utils::Scalar> m_r2;     ///< width of the 2nd gaussian curve
