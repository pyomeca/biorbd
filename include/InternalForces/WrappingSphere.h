...
class BIORBD_API WrappingSphere : public WrappingObject {
 public:
  ...
  /// \param diameter Diameter of the sphere
  ///
  WrappingSphere(const utils::Vector3d& v, const utils::Scalar& diameter);
  ...
  ///
  void DeepCopy(const WrappingSphere& other);
  ...
  ///
  virtual void
  wrapPoints(utils::Vector3d&, utils::Vector3d&, utils::Scalar* = nullptr) {}
  ...
  ///
  void setDiameter(const utils::Scalar& val);
 ...
 protected:
  std::shared_ptr<utils::Scalar> m_dia;  ///< Diameter of the wrapping sphere
};
