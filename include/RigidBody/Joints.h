...
#endif
{
 public:
  ...
  ///
  void DeepCopy(const Joints& other);
  ...
  /// \param QRanges Ranges of the translations and rotations dof. The length of
  /// QRanges must be equal to length of translations and rotations
  /// \param QdotRanges Ranges of the translations and rotations dof velocity.
  /// The length of QdotRanges must be equal to length of translations and
  /// rotations
  /// \param QddotRanges Ranges of the translations and rotations dof
  /// acceleration. The length of QddotRanges must be equal to length of
  /// translations and rotations
  /// \param jointDampings The damping of the joints to apply to the dynamics
  /// \param characteristics The characteristics of the semgent (mass, center of
  /// mass, inertia of the segment, etc)
  ...
  ///
  void setGravity(const utils::Vector3d& newGravity);
  ...
  ///
  int getBodyBiorbdId(const utils::String& segmentName) const;
  ...
  ///
  int getBodyRbdlId(const utils::String& segmentName) const;
  ...
  /// \return The Biorbd body identification
  ///
  int getBodyRbdlIdToBiorbdId(const int idx) const;
  ...
  /// \return The Rbdl body identification
  ///
  size_t getBodyBiorbdIdToRbdlId(const int idx) const;
 ...
 protected:
  ...
  /// \return the rbdl idx of subtrees of each segments starting from the
  /// specified index
  ///
 ...
 public:
  ...
  ///
  /// \brief updateSegmentCharacteristics Change the inertia characteristics of
  /// the segment idx
  ...
  /// new characteristic values will replace everything which is attach in a
  /// fixed manner (that is no degrees-of-freedom). So if your model has 3
  /// segments, but only the first one has dof (and the rest is rigidly attached
  /// to the first), then it doesn't matter if idx is 0, 1 or 2, because RBDL
  /// considers that all these segment are 1 segment. It is therefore expected
  /// that characteristics is the combination of mass and inertia for these 3
  /// segments as well.
  ///
  ...
  /// \return The segment
  ///
  Segment& segment(const utils::String& name);
  ...
  /// \return The segment
  ///
  const Segment& segment(size_t idx) const;
  ...
  /// \return The segment
  ///
  const Segment& segment(const utils::String& name) const;
  ...
  ///
  GeneralizedTorque computeDampedTau(const GeneralizedVelocity& Qdot) const;

 public:
  ///
  /// \brief Update the kinematic variables such as body velocities and
  /// accelerations in the model to reflect the variables passed to this
  /// function
  ...
  ///
  /// \brief Return the joint coordinate system (JCS) in global reference frame
  /// at a given Q
  /// \param Q The generalized coordinates
  ...
  ///
  /// \brief Return the joint coordinate system (JCS) for the segment in global
  /// reference frame at a given Q
  /// \param Q The generalized coordinates
  ...
  ///
  /// \brief Return the joint coordinate system (JCS) for the segment idx in
  /// global reference frame at a given Q
  /// \param Q The generalized coordinates
  ...
  ///
  utils::RotoTrans
  globalJCS(const GeneralizedCoordinates& Q, size_t idx, bool updateKin = true);

  ///
  /// \brief Return all the joint coordinate system (JCS) in its parent
  /// reference frame
  ...
  ///
  /// \brief Return the joint coordinate system (JCS) of the segment its parent
  /// reference frame
  /// \param name The name of the segment
  /// \return The JCS of the segment in parent reference frame
  ///
  utils::RotoTrans localJCS(const utils::String& name) const;

  ///
  /// \brief Return the joint coordinate system (JCS) of the segment idx its
  /// parent reference frame
  /// \param idx The index of the segment
  /// \return The JCS of the segment idx in parent reference frame
  ///
  utils::RotoTrans localJCS(const size_t idx) const;

  ///
  /// \brief Interface to call CalcBodyToBaseCoordinates. It is pretty much
  /// useless in Eigen, but fixes an issue in Casadi where RBDL changes internal
  /// variables which creates a free variables issue
  /// \param Q The generalized coordinates
  /// \param segmentName The name of the segment
  /// \param pointInLocal The point in the body
  /// \param updateKin If the kinematics of the model should be computed (always
  /// true for casadi)
  ...
  ///
  /// \brief Interface to call CalcPointVelocity. It is pretty much useless in
  /// Eigen, but fixes an issue in Casadi where RBDL changes internal variables
  /// which creates a free variables issue
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  /// \param segmentName The name of the segment
  /// \param pointInLocal The point in the body
  ...
  ///
  /// \brief Interface to call CalcPointVelocity6D. It is pretty much useless in
  /// Eigen, but fixes an issue in Casadi where RBDL changes internal variables
  /// which creates a free variables issue
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  ...
  ///
  /// \brief Interface to call CalcPointAcceleration. It is pretty much useless
  /// in Eigen, but fixes an issue in Casadi where RBDL changes internal
  /// variables which creates a free variables issue
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  /// \param Qddot The generalized accelerations
  ...
  ///
  /// \brief Interface to call CalcPointJacobian. Instead of sending the
  /// jacobian as an parameter, it is returned. It is pretty much useless in
  /// Eigen, but fixes an issue in Casadi where RBDL changes internal variables
  /// which creates a free variables issue
  /// \param Q The generalized coordinates///
  /// \param segmentName The name of the segment
  ...
  /// \param Q The generalized coordinates
  /// \param v All the points to project. The number of points must match the
  /// number of marker in the joint model
  /// \param updateKin If the kinematics of the model should be computed
  /// \return The projected markers from points corresponding to markers from
  /// the model
  ///
  /// Return projected markers from points corresponding to markers from the
  /// model. The vector needs to be equal to the number of markers and in the
  /// order given by Markers and in global coordinates
  ///
  ...
  ///
  /// \brief Return the projected markers from a point corresponding to a marker
  /// from the model
  /// \param Q The generalized coordinates
  /// \param n A reference to a marker from the model
  /// \param updateKin If the kinematics of the model should be computed
  /// \return The projected markers from a point corresponding to a marker from
  /// the model
  ///
  NodeSegment projectPoint(
      const GeneralizedCoordinates& Q,
  ...
  ///
  /// \brief Return the jacobian matrix of the projected markers for a marker
  /// from the model
  /// \param Q The generalized coordinates
  ...
  ///
  /// \brief Return the Jacobian matrix of a projected marker on the segment
  /// segmentIdx
  /// \param Q The generalized coordinates
  ...
  /// \param updateKin If the kinematics of the model should be computed
  /// \return The Jacobian matrix of a projected marker on the segment
  /// segmentIdx
  ///
  utils::Matrix projectPointJacobian(
      const GeneralizedCoordinates& Q,
  ...
  /// \param Q The generalized coordinates
  /// \param v All the markers. The size of which must match the one in the
  /// joint model
  /// \param updateKin If the kinematics of the model should be computed
  /// \return The jacobian matrix of the projected markers
  ///
  /// Return the jacobian matrix of projected markers from points corresponding
  /// to markers from the model. The vector needs to be equal to the number of
  /// markers and in the order given by Markers and in global coordinates
  ///
  ...
  ///
  utils::Vector3d CoM(const GeneralizedCoordinates& Q, bool updateKin = true);

  ///
  /// \brief Interface to CalcCenterOfMass. It is pretty much useless in Eigen,
  /// but fixes an issue in Casadi where RBDL changes internal variables which
  ...
  /// coordinates
  /// \param comVelocity (optional output) linear velocity of the COM in base
  /// coordinates
  /// \param comAcceleration (optional output) linear acceleration of the COM in
  /// base coordinates
  /// \param angularMomentum (optional output) angular momentum of the model at
  /// the COM in base coordinates
  /// \param changeOfAngularMomentum (optional output) change of angular
  /// momentum of the model at the COM in base coordinates
  /// \param updateKin (optional input) whether the kinematics should be updated
  /// (defaults to true)
  ///
  ...
  ///
  /// \brief Return the position of the center of mass of each segment in a
  /// matrix
  /// \param Q The generalized coordinates
  /// \param updateKin If the kinematics of the model should be computed
  /// \return The position of the center of mass of each segment
  ...
  // -- MESH OF THE MODEL -- //
  ///
  /// \brief Return the vertices of the mesh for all segments in global
  /// reference frame
  /// \param Q The generalized coordinates
  /// \param updateKin If the kinematics of the model should be computed
  ...
  ///
  const std::vector<MeshFace>& meshFaces(size_t idx) const;
  ...
  ///
  const Mesh& mesh(size_t idx) const;
  ...
  ///
  /// \brief Calculate the segment's contribution to the whole body angular
  /// momentum with respect to the global center of mass / formulae used: H_i =
  /// m_i * (r_i - r_com) x (v_i - v_com) + I_i * w_i) / The matrix returned
  /// contains a contribution for each last dof of a segment, zeros on all other
  /// DoFs.
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  /// \param updateKin If the kinematics of the model should be computed
  ...
  ///
  /// \brief Calculate the angular velocity of the model around its center of
  /// mass.
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  /// \param updateKin If the kinematics of the model should be computed
  /// \return The the angular velocity around the center of mass in the global
  /// reference frame
  ///
  ...
  ///
  /// \brief Calculate the jacobian matrix of a rotation matrix for a given
  /// segment idx
  /// \param Q The generalized coordinates
  /// \param segmentIdx The index of the segment
  /// \param updateKin If the kinematics of the model should be computed
  ...
  ///
  /// \brief Return the derivate of Q in function of Qdot (if not Quaternion,
  /// Qdot is directly returned)
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  /// \param k_stab
  ...
  ///
  /// \brief Biorbd's implementation of forward dynamics with a free floating
  /// base
  /// \param Q The Generalized Coordinates
  /// \param Qdot The Generalized Velocities
 ...
 protected:
  std::shared_ptr<std::vector<Segment>> m_segments;  ///< All the articulations

  std::shared_ptr<size_t> m_nbRoot;  ///< The number of DoF on the root segment
  std::shared_ptr<size_t> m_nbDof;   ///< The total number of degrees of freedom
  ...
  ///
  /// \brief Calculate the joint coordinate system (JCS) in global reference
  /// frame of a specified segment
  /// \param Q The generalized coordinates
  /// \param segmentIdx The index of the segment
  /// \param updateKin If the kinematics of the model should be computed
 ...
 public:
  ///
  /// \brief Check for the Generalized coordinates, velocities, acceleration and
  /// torque dimensions
  /// \param Q The generalized coordinates
  /// \param Qdot The generalized velocities
  /// \param Qddot The generalized accelerations
