...
class BIORBD_API GeneralizedCoordinates : public utils::Vector {
 public:
  ...
  ///
  GeneralizedCoordinates(size_t nbQ);

  ///
  /// \brief Create generalized coordinates from a joint Model
  /// \param j The joint model
  ///
  GeneralizedCoordinates(const Joints& j);

  ///
  /// \brief Construct generalized coordinates
  /// \param Q State vector of the internal joints
  ///
  GeneralizedCoordinates(const GeneralizedCoordinates& Q);
  ...
  GeneralizedCoordinates(const Eigen::MatrixBase<OtherDerived>& other)
      : utils::Vector(other) {}
  ...
  /// \param v The vector to copy
  ///
  GeneralizedCoordinates(const casadi::MX& v);
