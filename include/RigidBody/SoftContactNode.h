...
class BIORBD_API SoftContactNode : public rigidbody::NodeSegment {
 public:
  ...
  ///
  SoftContactNode(const utils::Vector3d& other);
  ...
  ///
  /// \brief Get the force in a spatial vector at the origin of the world base
  /// coordinates
  ...
  ///
  virtual utils::Vector3d applicationPoint(const utils::Vector3d& x) const = 0;
 ...
 protected:
  ...
  std::shared_ptr<std::pair<utils::Vector3d, utils::Vector3d>>
      m_contactPlane;  ///< The contact plane that interface with the node in
