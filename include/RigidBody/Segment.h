...
class BIORBD_API Segment : public utils::Node {
 public:
  ...
  /// \param characteristics of the segment (mass, center of mass, inertia,
  /// etc.)
  ...
  ///
  void DeepCopy(const Segment &other);
  ...
  ///
  /// \brief Return the ranges for all the dof, translations and rotations
  /// respectively
  /// \return The ranges for all the dof, translations and rotations
  /// respectively
  ///
  const std::vector<utils::Range> &QRanges() const;

  ///
  /// \brief Set the ranges for all the dof, translations and rotations
  /// respectively
  /// \param QRanges The ranges for all the dof, translations and rotations
  /// respectively
  ///
  void setQRanges(const std::vector<utils::Range> &QRanges);

  ///
  /// \brief Return the ranges for all the dof velocity, translations and
  /// rotations respectively
  /// \return The ranges for all the dof velocity, translations and rotations
  /// respectively
  ///
  const std::vector<utils::Range> &QdotRanges() const;

  ///
  /// \brief Set the ranges for all the dof velocity, translations and rotations
  /// respectively
  /// \param QdotRanges The ranges for all the dof velocity, translations and
  /// rotations respectively
  ///
  void setQdotRanges(const std::vector<utils::Range> &QdotRanges);

  ///
  /// \brief Return the ranges for all the dof acceleration, translations and
  /// rotations respectively
  /// \return The ranges for all the dofa acceleration, translations and
  /// rotations respectively
  ///
  const std::vector<utils::Range> &QddotRanges() const;

  ///
  /// \brief Set the ranges for all the dof acceleration, translations and
  /// rotations respectively
  /// \param QddotRanges The ranges for all the dof acceleration, translations
  ...
  ///
  size_t getDofIdx(const utils::String &dofName) const;

  ///
  /// \brief Traverse the model hierarchy until it gets to a segment with a dof.
  /// If the current segment has at least one dof, the current segment is
  /// returned
  ...
  ///
  /// \brief Returns the first index of the segment in the Generalized
  /// Coordinates vector
  /// \return The index of the first dof in the Generalized Coordinates vector
  ///
  /// Warning: if the current segment does not have any dof, it returns the
  /// first index of the parent segment
  size_t getFirstDofIndexInGeneralizedCoordinates(
      const rigidbody::Joints &model) const;

  ///
  /// \brief Returns the last index of the segment in the Generalized
  /// Coordinates vector
  /// \return The index of the last dof in the Generalized Coordinates vector
  ///
  /// Warning: if the current segment does not have any dof, it returns the last
  ...
  ///
  const utils::String &nameDof(const size_t i) const;

  ///
  /// \brief Return the joint coordinate system (JCS) in the parent reference
  /// frame
  ...
  ///
  /// Warning: This function doesn't work for segments that have no degree of
  /// freedom w.r.t their parent segment.
  ///
  void setLocalJCS(rigidbody::Joints &model, utils::RotoTrans &rototrans);

  ///
  /// \brief updateCharacteristics Change the inertia characteristics of the
  /// segment
 ...
 protected:
  ...
  std::shared_ptr<utils::SpatialTransform>
  ...
  ///
  /// \brief Check if the vector has the same number of elements as the number
  /// of DoF
  /// \param vec The vector to check
  /// \param isQLevel If the vector is a Q level or a Qdot/Qddot level. This is
  /// important to compute number of rotations when using quaternions
  /// \return If the vector has the same number of elements as the number of DoF
  template <typename T>
  bool isVectorHasDofDimension(const std::vector<T> &vec, bool isQLevel) const;
  ...
  ///
  void setNumberOfDof(size_t nbTrans, size_t nbRot);
  ...
  std::shared_ptr<size_t>
      m_nbDofTrue;  ///< Number of degrees of freedom including the extra DoF
                    ///< when there is a quaternion
  std::shared_ptr<size_t>
      m_nbDofTrueOutside;  ///< Number of degree of freedom read from the
                           ///< outside (Same as nDof except if Quaternion)
  std::shared_ptr<size_t>
      m_nbDofTrans;  ///< Number of degrees of freedom in translation
  std::shared_ptr<size_t>
      m_nbDofRot;  ///< Number of degrees of freedom in rotation
  std::shared_ptr<size_t> m_nbDofQuat;  ///< Number of degrees of freedom in
                                        ///< rotation if expressed in quaternion

  std::shared_ptr<std::vector<utils::Range>>
      m_QRanges;  ///< Minimum and maximum coordinate values that each dof
                  ///< should hold. This is only prescriptive and can be ignored
                  ///< when setting the GeneralizedCoordinates
  std::shared_ptr<std::vector<utils::Range>>
      m_QdotRanges;  ///< Minimum and maximum velocity values that each dof
                     ///< should hold. This is only prescriptive and can be
                     ///< ignored when setting the GeneralizedVelocities
  std::shared_ptr<std::vector<utils::Range>>
      m_QddotRanges;  ///< Minimum and maximum acceleration values that each dof
                      ///< should hold. This is only prescriptive and can be
                      ///< ignored when setting the GeneralizedAccelerations

  std::shared_ptr<std::vector<utils::Scalar>>
      m_jointDampings;  ///< The joint dampings to apply to the dynamics
  ...
  std::shared_ptr<std::vector<RigidBodyDynamics::Joint>>
      m_dof;  ///< Actual DoF: t1, t2, t3, r1, r2, r3; where the order depends
              ///< on seqT and seqR
  std::shared_ptr<std::vector<size_t>>
      m_idxDof;  ///< Index of the parent segment

  ///
  /// \brief Set angle and translation sequences, adjust angle sequence and
  /// redeclare if is necessary
  /// \param seqT Sequence of the translations
  /// \param seqR Angle sequence of the Euler rotations
  ///
  void setSequence(const utils::String &seqT, const utils::String &seqR);

  ///
  /// \brief Fill the transation and rotation sequences
  ///
  /// Places the translations first, followed by the rotations in the asked
  /// order.
  ///
  void fillSequence();

  ///
  /// \brief Convert a text sequence to its number counterpart (x = 0, y = 1, z
  /// = 2, q = 3)
  /// \param sequenceInteger Initialized vector into which the results should be
  /// placed in
  ...
  /// \param seqR Angle sequence of the Euler rotations
  ///
  void str2numSequence(const utils::String &seqT, const utils::String &seqR);
  ...
  /// \param model The joint model
  ///
  virtual void setJoints(rigidbody::Joints &model);
  ...
  ///
  /// The idea is that since a segment is described by all of its DoF, the
  /// inertia and masses must be put on the last body
  ///
  void setDofCharacteristicsOnLastBody();

  std::shared_ptr<SegmentCharacteristics>
      m_characteristics;  ///< Non-used virtual segment; it allows to "save" the
                          ///< data and to avoid the use of multiple
                          ///< intermediate variables
  std::shared_ptr<std::vector<SegmentCharacteristics>>
      m_dofCharacteristics;  ///< Variable containing the inertial data and
                             ///< other from each segment (on a 6DoF segment, 0
                             ///< to 4 should be empty and 5 filled)
};
