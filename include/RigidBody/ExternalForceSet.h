...
///
/// \brief An External force set that can apply forces to the model while
...
class BIORBD_API ExternalForceSet {
...
#ifndef SWIG
  ///
  /// \brief This is an internal structure holding the information for the local
  /// forces. RotoTransNodes could have suffised, but we need to have a
  /// structure that was holding the spatial vector as well.
  class LocalForcesInternal {
   public:
    void addNode(
        const utils::SpatialVector& vector,
        utils::RotoTransNode node) {
      m_nodes.push_back(node);
      m_vectors.push_back(vector);
    }

    ///
    /// \brief Return the pair force vector/point of application in the local
    /// reference frame
    std::pair<utils::SpatialVector, utils::RotoTransNode> get(int index) {
      return std::pair<utils::SpatialVector, utils::RotoTransNode>(
          m_vectors[index], m_nodes[index]);
    }
    ...
    std::vector<utils::SpatialVector>
        m_vectors;  ///< All the force vectors, the size of which should always
                    ///< match the size of m_nodes
  };
#endif
 ...
 public:
  ...
  /// \param model The model onto which the external forces will be applied on.
  /// This is used in conjunction with segmentName (when using set) to determine
  /// at which index the set vector is positioned in the internal Set.
  ...
  ///
  /// \brief Apply a new value to the specified spatial vector of the Set.
  /// WARNING: This vector is expected to be acting on segmentName, applied at
  /// origin and expressed in the global reference frame. method offered by
  /// ExternalForceSet below.
  ...
  /// \brief Apply a new value to the specified spatial vector of the Set.
  /// WARNING: This vector is expected to be acting on segmentName, applied at
  /// pointOfApplication and expressed in the global reference frame.
  /// \param segmentName The name of the segment to apply the spatial vector on.
  /// \param vector The SpatialVector to add to the set.
  ...
  /// \brief Apply a new value to the specified spatial vector of the Set.
  /// WARNING: This vector is expected to be acting on segmentName, applied at
  /// pointOfApplication and expressed in the segment reference frame.
  /// \param segmentName The name of the segment to apply the spatial vector on.
  /// \param vector The SpatialVector to add to the set.
  /// \param pointOfApplication Where the v vector is currenlty applied in the
  ...
  /// \param pointOfApplicationInLocal The position in the reference frame of
  /// [segmentName]
  ///
  ...
  ///
  /// \brief The forces in a rbdl compatible format. This won't work if
  /// useTranslationalForces
  ...
  ///
  /// \brief The forces in a rbdl compatible format. This won't work if
  /// useTranslationalForces or useSoftContacts is set to true
  /// \param updatedModel The joint model that with its kinematics updated
  ///
  ...
  ///
  /// \brief Return if there is at least one external force expressed in the
  /// local reference frame. This is important since having this forces to send
  /// Q when computing the force in global reference frame
  ///
  bool hasExternalForceInLocalReferenceFrame() const;

 protected:
  ///
  /// \brief Add the forces expressed in the local reference to the internal
  /// Set.
  /// \param updatedModel The joint model that with its kinematics updated
  /// \param Q The Generalized coordinates.
  /// \param out The vector of SpatialVector to fill
  ///
  /// Note: as this is an internal method, even though Q is passed, it is
  /// assumed update kinematics was already done.
  ///
  ...
  void combineTranslationalForces(
      rigidbody::Joints& updatedModel,
      const rigidbody::GeneralizedCoordinates& Q,
      std::vector<utils::SpatialVector>& out) const;
  ...
  ///
  /// \brief Get the rigid contacts in a list of spatial vector of dimension
  /// 6xNdof
  /// \param force The force amplitude applied at the point
  /// \param pointOfApplication The position where the force is applied in base
  /// coordinate
 ...
 protected:
  ...
  bool
      m_useTranslationalForces;  ///< If translational forces should be included
  ...
  LocalForcesInternal
      m_externalForcesInLocal;  ///< The vector that holds all the external
                                ///< forces that are expressed in local
                                ///< reference frame (must call Q).

  std::vector<std::pair<utils::Vector3d, rigidbody::NodeSegment>>
      m_translationalForces;  ///< The translational forces. The first is the
                              ///< amplitude and the second is the application
                              ///< point (that include the name of the
                              ///< parentSegment it is applied on)
};
}  // namespace rigidbody
}  // namespace BIORBD_NAMESPACE
