...
class BIORBD_API Mesh {
 public:
  ...
  ///
  Mesh(const std::vector<utils::Vector3d> &other);
  ...
  ///
  void DeepCopy(const Mesh &other);

  ///
  /// \brief Set the patch color
  /// \param color The color
  ///
  void setColor(const utils::Vector3d &color);
  ...
  ///
  void addPoint(const utils::Vector3d &node);
  ...
  ///
  const utils::Vector3d &point(size_t idx) const;
  ...
  ///
  void rotate(const utils::RotoTrans &rt);
  ...
  ///
  void scale(const utils::Vector3d &scaler);
  ...
  ///
  const MeshFace &face(size_t idx) const;
  ...
  ///
  void setPath(const utils::Path &path);
 ...
 protected:
  ...
  std::shared_ptr<std::vector<MeshFace>> m_faces;          ///< The faces
