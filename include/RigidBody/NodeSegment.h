...
class BIORBD_API NodeSegment : public utils::Vector3d {
 public:
  ...
  ///
  NodeSegment(const utils::Vector3d& other);
  ...
  /// \return Projected position of the node when removing speficic axes
  /// speficied using addAxesToRemove
  ///
  ...
  NodeSegment& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
 ...
 protected:
  ...
  std::shared_ptr<int>
      m_nbAxesToRemove;  ///< Removed one of multiple axes (1 axis : project on
                         ///< a plan, 2 axes : project on the 3rd axis, 3 axes :
                         ///< return the position of the parent reference)
