...
class BIORBD_API RotoTransNodes {
 public:
  ...
  ///
  RotoTransNodes(const RotoTransNodes& other);
  ...
  ///
  void DeepCopy(const RotoTransNodes& other);
  ...
  ///
  virtual void addRT(const utils::RotoTransNode& RotoTrans);
  ...
  ///
  std::vector<utils::RotoTransNode> RTs(const utils::String& segmentName);
  ...
  /// \return RT of idx i
  ///
  const utils::RotoTransNode& RT(size_t idx);
  ...
  ///
  utils::RotoTransNode
  RT(const GeneralizedCoordinates& Q, size_t idx, bool updateKin = true);
 ...
 protected:
  std::shared_ptr<std::vector<utils::RotoTransNode>> m_RTs;  ///< All the RTs
};
