...
///
/// \brief Homogenous matrix to describe translations and rotations
/// simultaneously
///
#ifdef SWIG
...
#endif
{
 public:
  ...
  ///
  RotoTrans(const Rotation& rot, const Vector3d& trans);
  ...
  ///
  RotoTrans(const utils::SpatialTransform& st);
  ...
  RotoTrans(const Eigen::MatrixBase<OtherDerived>& other)
  ...
  ///
  /// \brief fromMarkers Creates a system of axes from two axes and an origin
  /// defined by markers
  /// \param origin The position of the origin
  /// \param axis1markers The beginning and ending of the vector of the first
  /// axis
  /// \param axis2markers The beginning and ending of the vector of the second
  /// axis
  /// \param axesNames The names ("x", "y" or "z") of the axes
  /// \param axisToRecalculate The axis to recalculate to ensure orthonormal
  /// system of axes
  ...
  /// \return The matrix of RotoTrans
  ///
  static RotoTrans fromSpatialTransform(const utils::SpatialTransform& st);
  ...
  ///
  static RotoTrans
  fromEulerAngles(const Vector& rot, const Vector3d& trans, const String& seq);

  ///
  /// \brief Return extracted angles from the rotation matrix into Euler angles
  /// using the provided sequence
  /// \param rt The RotoTrans matrix to extract angles from
  /// \param seq The angle sequence
  /// \return The angles (the length of the vector will match the length of
  /// sequence)
  ///
  /// The rotation sequence can be any combination of x, y and z
  ///
  static Vector toEulerAngles(const RotoTrans& rt, const String& seq);
  ...
  ///
  static utils::RotoTrans mean(const std::vector<utils::RotoTrans>& rt);
#endif
  ...
  ///
  template <typename OtherDerived>
  RotoTrans& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
 ...
 protected:
  ...
  ///
  RigidBodyDynamics::Math::Vector4d expand3dTo4d(const Vector3d& v1);

 public:
  ///
  /// \brief Check if the RotoTrans has a unitary matrix of rotation and the
  /// last row is (0, 0, 0, 1)
  ///
...
std::ostream& operator<<(
    std::ostream& os,
    const BIORBD_NAMESPACE::utils::RotoTrans& rt);
#endif
