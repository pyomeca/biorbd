...
class BIORBD_API IfStream {
 public:
  ...
  ///
  IfStream(const Path& path, std::ios_base::openmode mode);
  ...
  ///
  bool open(const Path& path, std::ios_base::openmode mode);
  ...
  /// \return True on success
  ///
  bool read(RBDLCasadiMath::MX_Xd_SubMatrix val);
#endif
  ...
  /// \return True on success
  ///
  bool read(double& result, const std::map<Equation, double>& variables);
#ifdef BIORBD_USE_CASADI_MATH
  ///
  /// \brief Read and evaluate an equation
  /// \param result The number read (output)
  ...
  /// \return True on success
  ///
  bool readFromBinary(char* output, int n_elements);
  ...
  /// \return True on success
  ///
  bool readFromBinary(float& result);

  ///
  /// \brief Read a float in binary file
  /// \return True on success
  ///
  bool readFromBinary(Vector3d& result);
  ...
  /// \return True on success
  ///
  bool readSpecificTag(const String& tag, String& text);
  ...
  /// \return True on success
  ///
  bool reachSpecificTag(const String& tag, size_t maxTag = -1);

  ///
  /// \brief Counts the number of consecutive lines starting with the same tag
  /// and then brings it back to the initial position
  /// \param tag The tag to count
  /// \return The number of consecutive lines starting with the same tag
  ///
  int countTagsInAConsecutiveLines(const String& tag);
