...
class BIORBD_API Benchmark {
 public:
  ...
  /// \param force If force is true, the timer starts even though it is already
  /// started
  ///
  void startTimer(const String& name, bool force = false);
  ...
  ///
  double getLap(const String& name);
  ...
  /// \return The lap time of a specified timer
  ///
  double stopTimer(const String& name);
  ...
  ///
  int getTimerIdx(const String& name);
