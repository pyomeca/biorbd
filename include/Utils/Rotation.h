...
#endif
{
 public:
  ...
  Rotation(const Eigen::MatrixBase<OtherDerived>& other) : Matrix3d(other) {
    checkUnitary();
  }
#endif
  ...
  ///
  Rotation(const RBDLCasadiMath::MX_Xd_static<3, 3>& other);

  ///
  /// \brief operator= For submatrices
  /// \param other The matrix to copy
  ///
  Rotation(const RBDLCasadiMath::MX_Xd_SubMatrix& other);
#endif
  ...
  /// sequence
  ///
  Rotation(const Vector& rotation, const String& rotationSequence);

  ///
  /// \brief Contruct Rototrans
  /// \param st Spatial Transform vector
  ///
  Rotation(const utils::SpatialTransform& st);
  ...
  ///
  static Rotation fromSpatialTransform(const utils::SpatialTransform& st);
  ...
  /// sequence
  ///
  static Rotation fromEulerAngles(const Vector& rot, const String& seq);

  ///
  /// \brief fromMarkers Creates a system of axes from two axes defined by
  /// markers
  ...
  ///
  static Vector toEulerAngles(const Rotation& r, const String& seq);
  ...
  ///
  static utils::Rotation mean(const std::vector<utils::Rotation>& mToMean);
#endif
  ...
  /// \param other The matrix to copy
  ///
  void operator=(const RBDLCasadiMath::MX_Xd_static<3, 3>& other);
...
std::ostream& operator<<(
    std::ostream& os,
    const BIORBD_NAMESPACE::utils::Rotation& rt);
#endif
