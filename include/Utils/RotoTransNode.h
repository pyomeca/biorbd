...
class BIORBD_API RotoTransNode : public RotoTrans, public Node {
 public:
  ...
  RotoTransNode(const Eigen::MatrixBase<OtherDerived>& other)
      : RotoTrans(other) {}
#endif
  ...
  ///
  void operator=(const RotoTrans& other);
  ...
  /// \return Rotated matrix
  ///
  RotoTrans operator*(const RotoTransNode& other) const;
  ...
  ///
  template <typename OtherDerived>
  RotoTransNode& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
...
/// \return Rotated matrix
///
RotoTransNode operator*(const RotoTrans& other, const RotoTransNode& me);
