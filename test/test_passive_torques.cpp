...
static std::string modelPathForGeneralTesting(
    "models/arm26_WithPassiveTorques.bioMod");
static std::string modelPathOnePassiveTorque(
    "models/arm26_WithOnePassiveTorques.bioMod");
...
TEST(PassiveTorqueConstant, passiveTorque) {
  internal_forces::passive_torques::PassiveTorqueConstant const_torque_act(
      150, 0);
...
TEST(PassiveTorqueLinear, passiveTorque) {
  ...
  internal_forces::passive_torques::PassiveTorqueLinear linear_torque_act(
      2, 1, 1);
  CALL_BIORBD_FUNCTION_1ARG(
      torqueLinearVal, linear_torque_act, passiveTorque, Q);
#ifdef BIORBD_USE_CASADI_MATH
  EXPECT_NEAR(
      static_cast<double>(torqueLinearVal(0, 0)),
      torqueLinearExpected,
      requiredPrecision);
#else
...
TEST(PassiveTorqueExponential, passiveTorque) {
  ...
  internal_forces::passive_torques::PassiveTorqueExponential
      exponential_torque_act(2, 2, 5, 5, 1, 8, 5, 2, 1, 2, 2);
  CALL_BIORBD_FUNCTION_2ARGS(
      torqueExponentialVal, exponential_torque_act, passiveTorque, Q, Qdot);
#ifdef BIORBD_USE_CASADI_MATH
  EXPECT_NEAR(
      static_cast<double>(torqueExponentialVal(0, 0)),
      torqueExponentialExpected,
      requiredPrecision);
#else
  EXPECT_NEAR(
      torqueExponentialVal, torqueExponentialExpected, requiredPrecision);
#endif
}
...
TEST(PassiveTorques, jointTorqueFromAllTypesOfPassiveTorque) {
  ...
  std::vector<double> torqueExpected = {
      3.100000000000000, -22.237006567213825, 5};
  ...
  for (size_t i = 0; i < model.nbGeneralizedTorque(); ++i) {
    EXPECT_NEAR(
        static_cast<double>(tau(i, 0)), torqueExpected[i], requiredPrecision);
  }
}
