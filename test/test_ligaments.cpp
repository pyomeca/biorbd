...
TEST(constant, unitTest) {
  ...
  }
  {
    ...
    // with damping
    internal_forces::ligaments::LigamentCharacteristics charac(
        ligamentConstant.characteristics());
  ...
  }
  {
    ...
    SCALAR_TO_DOUBLE(
        force, originalLigament.force(updatedModel, Q, Qdot, true));
...
TEST(constant, copy) {
  {
    ...
    internal_forces::ligaments::LigamentConstant deepCopyNow(
        ligamentConstant.DeepCopy());
  ...
  }
  {
    ...
    const_cast<internal_forces::Geometry&>(ligamentConstant.position())
    ...
    ligamentConstant.updateOrientations(updatedModel, Q, Qdot);
    {
      ...
      EXPECT_NEAR(deepCopyLaterLength, 0.14927574026595969, requiredPrecision);
      EXPECT_EQ(
          ligamentConstant.position().insertionInLocal().utils::Node::name(),
          newName);
      EXPECT_EQ(
...
TEST(springLinear, copy) {
  {
    ...
    internal_forces::ligaments::LigamentSpringLinear shallowCopy(
        ligamentSpringLinear);
    internal_forces::ligaments::LigamentSpringLinear deepCopyNow(
        ligamentSpringLinear.DeepCopy());
    ...
    {
      SCALAR_TO_DOUBLE(
          ligamentSpringLinearStiffness, ligamentSpringLinear.stiffness());
  ...
  }
  {
    ...
    utils::Vector3d insertion(
        ligamentSpringLinear.position().insertionInLocal());
    insertion.set(0.5, 0.6, 0.7);
    const_cast<internal_forces::Geometry&>(ligamentSpringLinear.position())
        .setInsertionInLocal(insertion);
    utils::String oldName(insertion.utils::Node::name());
    utils::String newName("MyNewName");
    insertion.setName(newName);
...
TEST(springSecondOrder, copy) {
  {
    ...
    internal_forces::ligaments::LigamentSpringSecondOrder shallowCopy(
        ligamentSpringSecondOrder);
    internal_forces::ligaments::LigamentSpringSecondOrder deepCopyNow(
        ligamentSpringSecondOrder.DeepCopy());
  ...
  }
  {
    ...
    // again (length and insertion in Local)
    utils::Vector3d insertion(
        ligamentSpringSecondOrder.position().insertionInLocal());
    insertion.set(0.5, 0.6, 0.7);
    const_cast<internal_forces::Geometry&>(ligamentSpringSecondOrder.position())
        .setInsertionInLocal(insertion);
    utils::String oldName(insertion.utils::Node::name());
    utils::String newName("MyNewName");
    insertion.setName(newName);
