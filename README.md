...
In function of the type of actuator the parameters are different, in this example we choose the type Gauss3P and the parameters associate.
These different parameters were defined based on the relationship between torque and angular velocity (`Tmax`, `T0`, `wmax`, `wc`), 
the differential activation and angular velocity (`amin`, `wr`, `w1`), 
and torque and angle (`qopt`, `r`) describe by M. Jackson (The mechanics of the table contact phase of gymnastics vaulting, 2019).
```c
...
of the 3 axis (`x`, `y`, `z`).
#### direction
The `direction` of the torque can be positive or negative.
#### Tmax
...
#### qopt
The `qopt` tag allows to specify the optimum angle for torque production.
If the type actuator is `Gauss6P`, a `qopt2` will be asked.
#### r
The tag `r` represented the width of the curve based of the torque-angle relationship.
If the type actuator is `Gauss6P`, a `r2` will be asked.
